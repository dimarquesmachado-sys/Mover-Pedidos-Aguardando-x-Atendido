  ...
  try {
    ...
    if (Array.isArray(idsFullVistos) && idsFullVistos.length) {
      ...
      if (SIT_DESPACHADOS && Array.isArray(idsFullVistos) && idsFullVistos.length) {
        ...
        console.log(`[GOODBKP] move Full → DESPACHADOS(${SIT_DESPACHADOS}): ${movidos} movido(s)${falhas ? `, ${falhas} falha(s) (retenta no próximo ciclo)` : ''}${semBipagem ? `, ${semBipagem} NÃO movido(s) por não terem passado pela bipagem` : ''}`);
      }
    }
