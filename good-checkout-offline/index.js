...
function routes(readBody) {
  return async function handle(req, res, urlObj) {
    ...
    if (method === 'GET' && p === '/good-checkout-offline/custo-sync') {
      ...
      json(res, 200, { ok: true, iniciado: true, mensagem: 'custo-sync rodando em background (tartaruga anti-429) — ?status=1 p/ acompanhar', acompanhe: _urlStatus(req, '/good-checkout-offline/custo-sync') });
      return true;
    }
