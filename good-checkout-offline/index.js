...
const VERSAO     = 'good-checkout-offline v23/07 b12';
...
function routes(readBody) {
  return async function handle(req, res, urlObj) {
    ...
    if (method === 'GET' && p === '/good-checkout-offline/ir-shopee') {
      ...
      try {
        const ckIr = String(process.env.GOODBKP_SHOPEE_COOKIE || '').trim();
        if (!ckIr) {
          passosIr.push({ passo: 'cookie', erro: 'env GOODBKP_SHOPEE_COOKIE vazia ou ausente' });
        } else {
