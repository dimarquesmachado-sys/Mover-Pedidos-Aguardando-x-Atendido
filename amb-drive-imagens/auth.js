...
/**
 * Autenticação do módulo /amb-drive-imagens
 *
 * Usuários lidos da env var AMBIMG_USUARIOS no formato:
 *   AMBIMG_USUARIOS=Diego:senha1,Fulano:senha2
 *
...
function parseUsuarios() {
  const raw = process.env.AMBIMG_USUARIOS || '';
  if (!raw.trim()) return [];
  const lista = [];
...
function autenticar(usuario, senha) {
  if (USUARIOS.length === 0) {
    return { ok: false, erro: 'Nenhum usuário cadastrado. Configure AMBIMG_USUARIOS no Render.' };
  }
