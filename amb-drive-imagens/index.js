...
/**
 * Módulo /amb-drive-imagens — Sincroniza imagens do Google Drive → Bling (AMBTotal)
 *
 * Rotas:
 *   GET  /amb-drive-imagens                       → redirect /amb-drive-imagens/
 *   GET  /amb-drive-imagens/                       → frontend (login + app)
 *   GET  /amb-drive-imagens/health                 → status do módulo
 *   GET  /amb-drive-imagens/setup                  → página "Autorizar Bling" (aberta)
 *   GET  /amb-drive-imagens/oauth/callback         → recebe code Bling e salva tokens
 *   POST /amb-drive-imagens/login                  → autentica (JWT)
 *   POST /amb-drive-imagens/logout                 → encerra sessão
 *   GET  /amb-drive-imagens/api/status             → status Drive + Bling   [auth]
 *   GET  /amb-drive-imagens/api/pastas             → lista pastas Drive     [auth]
 *   POST /amb-drive-imagens/api/verificar-bling    → verifica SKUs no Bling [auth]
 *   POST /amb-drive-imagens/api/processar          → processa imagens       [auth]
 *   POST /amb-drive-imagens/api/enviar-bling        → envia URLs pro Bling   [auth]
 *   POST /amb-drive-imagens/api/processar-e-enviar  → tudo de uma vez        [auth]
 *   GET  /amb-drive-imagens/api/debug-produto/:cod  → debug produto Bling    [auth]
 */
...
const PUBLIC_DIR = path.join(__dirname, '..', 'public', 'amb-drive-imagens');
...
function routes(readBody) {
  return async function handle(req, res, urlObj) {
    ...
    if (p !== '/amb-drive-imagens' && !p.startsWith('/amb-drive-imagens/')) return false;

    // ── Raiz → frontend ──
    if (method === 'GET' && p === '/amb-drive-imagens') {
      res.writeHead(302, { Location: '/amb-drive-imagens/' });
      res.end();
      return true;
    }
    if (method === 'GET' && (p === '/amb-drive-imagens/' || p === '/amb-drive-imagens/index.html')) {
      servirArquivo(res, 'index.html');
      return true;
    }

    // ── Health (aberto) ──
    if (method === 'GET' && p === '/amb-drive-imagens/health') {
      json(res, 200, {
        ok: true, modulo: 'amb-drive-imagens',
    ...
    // ── Setup OAuth (aberto) ──
    if (method === 'GET' && p === '/amb-drive-imagens/setup') {
      const clientId = process.env.AMBIMG_BLING_CLIENT_ID;
      if (!clientId) { html(res, 500, '<h1>Erro</h1><p>AMBIMG_BLING_CLIENT_ID não configurado no Render.</p>'); return true; }
      ...
      html(res, 200, `
        <!doctype html><html><head><meta charset="utf-8"><title>Setup AMBTotal Imagens</title></head>
        <body style="font-family:system-ui;max-width:600px;margin:40px auto;padding:20px;">
          <h2>🖼️ Setup — AMBTotal Imagens (Bling)</h2>
          <p>Clique no botão para autorizar este app no Bling.</p>
          <p>⚠️ Faça login na conta <b>AMBTotal</b> antes de clicar.</p>
    ...
    if (method === 'GET' && p === '/amb-drive-imagens/oauth/callback') {
      ...
      try {
        ...
        html(res, 200, `
          ...
          <body style="font-family:system-ui;max-width:600px;margin:40px auto;padding:20px;">
            ...
            <p>Tokens salvos em disco. <a href="/amb-drive-imagens/">← Abrir o sistema</a></p>
          </body></html>
        `);
      } catch (e) {
    ...
    // ── LOGIN ──
    if (method === 'POST' && p === '/amb-drive-imagens/login') {
      ...
      console.log(`[amb-drive-imagens LOGIN] ${r.usuario} (${r.perfil})`);
      json(res, 200, { ok: true, token, usuario: r.usuario, perfil: r.perfil, expiraHoras: auth.SESSAO_HORAS });
      return true;
    }

    // ── LOGOUT ──
    if (method === 'POST' && p === '/amb-drive-imagens/logout') {
    ...
    // ════════════════════════════════════════════════════════════════
    if (p.startsWith('/amb-drive-imagens/api/')) {
      ...
      // ── status ──
      if (method === 'GET' && p === '/amb-drive-imagens/api/status') {
      ...
      // ── pastas ──
      if (method === 'GET' && p === '/amb-drive-imagens/api/pastas') {
        try {
      ...
      // ── verificar-bling ──
      if (method === 'POST' && p === '/amb-drive-imagens/api/verificar-bling') {
        const body = await readBody(req);
      ...
      // ── processar ──
      if (method === 'POST' && p === '/amb-drive-imagens/api/processar') {
        const body = await readBody(req);
        ...
        console.log(`[amb-drive-imagens PROCESSAR] ${quem}: ${items.length} item(s)`);
        json(res, 200, { resultados });
        return true;
      }

      // ── enviar-bling ──
      if (method === 'POST' && p === '/amb-drive-imagens/api/enviar-bling') {
        const body = await readBody(req);
        const items = body && body.items;
        if (!Array.isArray(items) || items.length === 0) { json(res, 400, { erro: 'Envie array items' }); return true; }
        const resultados = {};
        for (const it of items) resultados[it.sku] = await imagensService.enviarUm(it);
        console.log(`[amb-drive-imagens ENVIAR] ${quem}: ${items.length} item(s)`);
        json(res, 200, { resultados });
        return true;
      }

      // ── processar-e-enviar ──
      if (method === 'POST' && p === '/amb-drive-imagens/api/processar-e-enviar') {
        const body = await readBody(req);
        const items = body && body.items;
        if (!Array.isArray(items) || items.length === 0) { json(res, 400, { erro: 'Envie array items' }); return true; }
        const resultados = {};
        ...
        console.log(`[amb-drive-imagens DIRETO] ${quem}: ${items.length} item(s)`);
        json(res, 200, { resultados });
        return true;
      }

      // ── debug-produto ──
      if (method === 'GET' && p.startsWith('/amb-drive-imagens/api/debug-produto/')) {
        const codigo = decodeURIComponent(p.replace('/amb-drive-imagens/api/debug-produto/', ''));
        try {
    ...
    // ── Arquivos estáticos (favicon etc) em /amb-drive-imagens/{arquivo} ──
    if (method === 'GET' && /\.(ico|png|jpg|svg|json|css|js)$/i.test(p)) {
      const rel = p.replace('/amb-drive-imagens/', '');
...
function bootstrap() {
  setTimeout(() => {
    const st = tokenMgr.getStatus();
    console.log(`[amb-drive-imagens] Pronto. Bling configurado=${st.configurado} tokens_ok=${st.tokens_ok} | Drive=${drive.estaConfigurado()} | usuários=${auth.totalUsuarios}`);
    if (!st.configurado) console.warn('[amb-drive-imagens] ⚠️ Defina AMBIMG_BLING_CLIENT_ID e AMBIMG_BLING_CLIENT_SECRET no Render');
    else if (!st.tokens_ok) console.warn('[amb-drive-imagens] ⚠️ Tokens Bling não obtidos — acesse /amb-drive-imagens/setup');
  }, 4000);
}
