...
/**
 * Token Manager do módulo /amb-drive-imagens
 *
 * Gerencia tokens OAuth do Bling para o app "AMBTotal Imagens x SKU".
 * Tokens salvos em /data/amb-drive-imagens/bling-tokens.json (disco persistente Render).
 *
 * Env vars necessárias:
 *   AMBIMG_BLING_CLIENT_ID
 *   AMBIMG_BLING_CLIENT_SECRET
 *   AMBIMG_BLING_REDIRECT_URI  (default: https://mover-pedidos-aguardando-x-atendido.onrender.com/amb-drive-imagens/oauth/callback)
 */
...
const TOKEN_FILE = process.env.AMBIMG_TOKEN_FILE || '/data/amb-drive-imagens/bling-tokens.json';

const REDIRECT_URI = process.env.AMBIMG_BLING_REDIRECT_URI
  || 'https://mover-pedidos-aguardando-x-atendido.onrender.com/amb-drive-imagens/oauth/callback';
...
function lerTokens() {
  try {
    ...
    console.error('[amb-drive-imagens tokenManager] Erro ao ler tokens:', e.message);
    return {};
  }
}
...
function salvarTokens(access_token, refresh_token) {
  ...
  console.log('[amb-drive-imagens tokenManager] Tokens Bling salvos em disco ✓');
}

// ── OAuth helpers ────────────────────────────────────────────────────

function basicAuth() {
  const id  = process.env.AMBIMG_BLING_CLIENT_ID;
  const sec = process.env.AMBIMG_BLING_CLIENT_SECRET;
  if (!id || !sec) throw new Error('AMBIMG_BLING_CLIENT_ID / AMBIMG_BLING_CLIENT_SECRET não definidos');
...
async function refreshTokens() {
  const tk = lerTokens();
  if (!tk.refresh_token) throw new Error('refresh_token não disponível — autorizar via /amb-drive-imagens/setup');
...
async function garantirAccessToken() {
  const tk = lerTokens();
  if (!tk.access_token) throw new Error('access_token Bling não disponível — autorizar via /amb-drive-imagens/setup');
...
async function fetchComRetry(url, opts = {}) {
  ...
  if (r.status === 401) {
    console.log('[amb-drive-imagens tokenManager] 401 — tentando refresh…');
    await refreshTokens();
...
function getStatus() {
  const tk = lerTokens();
  return {
    configurado: !!process.env.AMBIMG_BLING_CLIENT_ID && !!process.env.AMBIMG_BLING_CLIENT_SECRET,
