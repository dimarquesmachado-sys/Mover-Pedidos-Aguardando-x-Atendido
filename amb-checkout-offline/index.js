...
const VERSAO     = 'amb-checkout-offline v25/08 b81';
let _sondaClonesJob = null;   // sonda temporária: a varredura roda em 2º plano; isto morre no restart (aceitável, é descartável)
      ...
      try {
        ...
        out.replay_filtro = {
          ...
          por_pedido: acc.map(p => {
            ...
            const un = String(raw || '');
            /* Três desfechos, não dois. Quando a venda TEM loja mas a lista omitiu a unidade
               (o Bling faz isso de forma intermitente), o ciclo vai buscar o DETALHE e pode
               classificar como Full — dizer bate:false aqui seria mentir. A sonda não repete
               a busca de detalhe (seria lenta e gastaria cota), então admite que não sabe. */
            if (!raw && p.loja) {
              return { numero: p.numero, un_raw: null, un_str: '', bate: null,
                       indeterminado: true,
                       motivo: 'a lista omitiu loja.unidadeNegocio; o ciclo resolveria pelo detalhe do pedido' };
            }
            return { numero: p.numero, un_raw: raw ?? null, un_str: un,
                     bate: !!(un && setFull.has(un)),
                     sem_loja: !p.loja || undefined };
          })
        };
    ...
    if (method === 'GET' && p === '/amb-checkout-offline/sonda-clones-full') {
      ...
      if (acaoS !== 'iniciar') {
        ...
        json(res, 200, {
          ...
          detalhe: j.rodando ? undefined : j.clones,
          descartadas_nao_autorizadas: j.rodando ? undefined : j.descartadas,
          nao_resolvidos: j.rodando ? undefined : j.naoResolvidos,
          erro: j.erro || undefined, aviso: j.aviso || undefined,
          inicio: j.inicio, fim: j.fim || undefined
        });
        return true;
      }

      if (_sondaClonesJob && _sondaClonesJob.rodando) { json(res, 200, { ok: false, erro: 'já existe uma varredura rodando — acompanhe sem &acao, ou espere terminar' }); return true; }
      ...
      const dataValida = (x) => {
        ...
        return !isNaN(dX.getTime()) && dX.toISOString().slice(0, 10) === x;   // Codex r3: 2026-02-30 normalizado não passa
      };
      ...
      const j = { rodando: true, janela: { de: deS, ate: ateS }, un: unS, pagina: 0, vistos: 0, daUnidade: 0,
                  clones: [], descartadas: [], naoResolvidos: [], erro: null, aviso: null,
                  inicio: new Date().toISOString(), fim: null };
      _sondaClonesJob = j;

      /* Codex r2: o prazo vence POR FORA (corrida) — o blingGet espera o garantirToken
         antes de olhar o sinal, e o fetch do token não tem sinal. */
      ...
      (async () => {
        try {
          for (let pag = 1; pag <= 40; pag++) {
            j.pagina = pag;
            let rS = null;
            try { rS = await comPrazoS(sig => blingGet(`/pedidos/vendas?dataInicial=${deS}&dataFinal=${ateMais1S}&pagina=${pag}&limite=100`, 3, sig)); } catch (e) { rS = null; }
            if (!rS || rS.ok === false) { j.erro = 'o Bling falhou na página ' + pag + ' — varredura INCOMPLETA, inicie de novo mais tarde'; return; }
            const arrS = (rS.data && rS.data.data) || null;
            if (!Array.isArray(arrS)) { j.erro = 'resposta do Bling em forma inesperada na página ' + pag + ' — varredura INCOMPLETA'; return; }   // Codex r5
            if (!arrS.length) break;
            for (const ped of arrS) {
              j.vistos++;
              const dataP = String(ped.data || '').slice(0, 10);
              if (dataP && (dataP > ateS || dataP < deS)) continue;   // Codex r4: o +1 dia não vaza pro resultado
              let unP = (ped.loja && ped.loja.unidadeNegocio && ped.loja.unidadeNegocio.id) || null;
              if (unP == null && ped.loja) {   // Codex r1: lista omite a unidade de vez em quando
                let detS = null;
                try { detS = await comPrazoS(sig => detalhePedido(ped.id, sig)); } catch (e) { detS = null; }
                await sleep(PAUSA_MS);
                if (!detS) { j.naoResolvidos.push({ pedido: ped.numero, motivo: 'lista sem unidade e o detalhe não veio — inicie de novo' }); continue; }
              ...
              if (nfS && nfS.serie != null) {
                if (String(nfS.serie) === '1') {
                  ...
                  if (sitNF === null && nfS.nfId) {   // Codex r6: resumo com série mas sem situação → hidrata
                    try {
                  ...
                  /* pedido do dono: cancelada NÃO entra. Autorizada=2 é o único código provado
                     na casa (nfFluxos.js); o resto vai pra descartadas com o código à vista. */
                  if (sitNF === 2) j.clones.push(linhaC);
                  else if (sitNF === null) j.naoResolvidos.push({ pedido: ped.numero, nf: nfS.numero, motivo: 'série 1, mas a situação da NF não veio nem no detalhe — inicie de novo' });
                  else j.descartadas.push(linhaC);
                }
              } else if (nfS && nfS.semNF) {
                // sem nota ainda: não é clone (o clone tem NF automática desde o salvar)
              } else {
                j.naoResolvidos.push({ pedido: ped.numero, motivo: 'série não lida (Bling falhou) — inicie de novo mais tarde' });
              }
            }
            if (pag === 40 && arrS.length === 100) { j.aviso = 'INCOMPLETA: mais de 4.000 pedidos no período — divida a janela com &de=/&ate='; break; }   // Codex r1
            if (arrS.length < 100) break;
            await sleep(PAUSA_MS);
          }
        } catch (e) {
          j.erro = String(e.message || e).slice(0, 180);
        } finally {
          j.rodando = false;
          j.fim = new Date().toISOString();
          try { console.log(`[SONDA-CLONES] fim: vistos=${j.vistos} full=${j.daUnidade} clones=${j.clones.length} descartadas=${j.descartadas.length} nao_resolvidos=${j.naoResolvidos.length}${j.erro ? ' ERRO=' + j.erro : ''}`); } catch (e2) {}
        }
      })();

      json(res, 200, { ok: true, iniciada: true, janela: { de: deS, ate: ateS }, unidade: unS,
        acompanhe: '/amb-checkout-offline/sonda-clones-full?k=ADMIN_KEY (sem acao) — recarregue até rodando:false' });
      return true;
    }
