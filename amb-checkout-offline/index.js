...
const VERSAO     = 'amb-checkout-offline v11/08 b43';
  ...
  if (!body.fresh && c && c.nome !== undefined && (Date.now() - (c.ts || 0)) < TTL && (c.custo != null || c.saldo != null)) {
    ...
    if (method === 'GET' && p === '/amb-checkout-offline/produto-fotos') {
      ...
      for (const sk of skusF) {
        ...
        else if (c && c.ts && (Date.now() - c.ts) < 12 * 3600 * 1000) fotos[sk] = null;   // já procuramos há pouco: não tem foto
      ...
      for (const sk of faltando.slice(0, 12)) {
        try {
          ...
          cacheF[sk] = { saldo: (antes.saldo != null ? antes.saldo : null), img: img || null, nome: (it0 && it0.nome) || antes.nome || null, ts: Date.now() };
          buscadas++;
