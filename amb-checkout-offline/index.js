...
const VERSAO     = 'amb-checkout-offline v13/08 b59';
...
async function magaluLinhas(de, ate, empresa, jaTem) {
  ...
  async function itensDoBlingPorNumeroLoja(codLoja, diaISO) {
    if (!codLoja || !/^\d{4}-\d{2}-\d{2}$/.test(String(diaISO || ''))) return null;
    if (!_diaCache[diaISO]) {
      const mapa = {};
    ...
    const idBling = _diaCache[diaISO][String(codLoja).trim()];
  ...
  for (const p of vivos) {
    ...
    if (!its.length || its.some(x => !x || !x.sku)) {
      let doBling = null;
      try { doBling = await itensDoBlingPorNumeroLoja(cod, dv); } catch (e) {}
