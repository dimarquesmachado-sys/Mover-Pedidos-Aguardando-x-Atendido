...
function routes(readBody) {
  return async function handle(req, res, urlObj) {
    ...
    if (method === 'POST' && p === '/amb-checkout-offline/etiqueta-anexar') {
      ...
      try { require('../lib/avisar-devolucoes')('amb', 'etiqueta_anexada', idA, { formato: formatoA, quem: (opSess && (opSess.usuario || opSess.nome || opSess.login)) || '' }); } catch (e) {}
    ...
    if (method === 'GET' && p === '/amb-checkout-offline/lista') {
      ...
      const semEtiq = ids
        ...
        .map(i => ({ id: i, numero: man[i].numero, cliente: man[i].cliente || '', nf_numero: man[i].nf_numero || null, marketplace: man[i].marketplace || 'outro', numero_loja: man[i].numero_loja || null, nf_emissao: man[i].nf_emissao || null, visto_em: man[i].visto_em || null, nf_id: man[i].nf_id || null }))   // numero_loja p/ o ↗ do canal; nf_emissao/visto_em p/ a data-hora; nf_id p/ o link da NF
