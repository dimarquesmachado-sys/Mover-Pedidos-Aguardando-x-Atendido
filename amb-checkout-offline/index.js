...
const VERSAO     = 'amb-checkout-offline v11/08 b45';
  ...
  if (!body.fresh && c && c.nome !== undefined && (Date.now() - (c.ts || 0)) < TTL && (c.custo != null || c.saldo != null)) {
    ...
    if (method === 'GET' && p === '/amb-checkout-offline/produto-fotos') {
      ...
      for (const sk of skusF) {
        ...
        else if (c && c.ts_img && (Date.now() - c.ts_img) < 12 * 3600 * 1000) fotos[sk] = null;
        else faltando.push(sk);
      }
      let buscadas = 0;
      for (const sk of faltando.slice(0, 12)) {
        try {
          const rb = await blingGet('/produtos?codigo=' + encodeURIComponent(sk) + '&criterio=5&limite=1');
          const it0 = (rb.ok && rb.data && rb.data.data && rb.data.data[0]) || null;
          let img = it0 ? primeiraImagem(it0) : null;
          if (it0 && img == null) {
            const dd = await blingGet('/produtos/' + it0.id);
            const det = (dd.ok && dd.data && dd.data.data) || null;
            if (det) img = primeiraImagem(det);
          }
          fotos[sk] = img || null;
    ...
    if (method === 'GET' && p === '/amb-checkout-offline/magalu-caca') {
      ...
      cacaMagalu(deM, ateM, 'amb').catch(e => console.log('[CACA-MAGALU] ' + (e.message || e)));
...
let _mgc = { rodando: false, em: null, de: null, ate: null, na_magalu: 0, ja_tinha: 0, inseridos: 0, linhas: 0, erro: null, parcial: false };

async function cacaMagalu(de, ate, empresa) {
  const dorme = ms => new Promise(r => setTimeout(r, ms));   // helper local (padrão do arquivo)
  empresa = empresa || 'amb';
  if (_mgc.rodando) return _mgc;
  _mgc = { rodando: true, em: new Date().toISOString(), de, ate, na_magalu: 0, ja_tinha: 0, inseridos: 0, linhas: 0, erro: null, parcial: false };
  try {
    const ADM = process.env.ADMIN_KEY || '';
    if (!ADM) throw new Error('sem ADMIN_KEY');
    const PORT_L = process.env.PORT || 3000;
    const url = 'http://127.0.0.1:' + PORT_L + '/magalu/pedidos-do-dia?empresa=' + encodeURIComponent(process.env.AMBBKP_MAGALU_EMPRESA || 'amb') +
                '&k=' + encodeURIComponent(ADM) + '&desde=' + de + '&ate=' + ate + '&paginas=40';
    const r = await fetch(url, { timeout: 120000 });
    const j = await r.json().catch(() => null);
    if (!j || !j.ok || !Array.isArray(j.pedidos)) throw new Error('rota da Magalu: ' + (j && j.erro ? j.erro : 'HTTP ' + r.status));
    _mgc.parcial = Boolean(j.parcial || j.truncado);
    _mgc.na_magalu = j.pedidos.length;

    // o que o histórico JÁ tem desse canal no período (pelo número da venda no marketplace)
    const jaTem = new Set();
    ...
    const pctMag = Number((cfgF.taxas && cfgF.taxas.magalu) || 0);   // 0 = não configurada: fica sem comissão em vez de inventar
    const custos = readJson(path.join(CACHE_DIR, '_custos.json'), {});
    ...
    for (const p of j.pedidos) {
      ...
      if (!cod || jaTem.has(cod)) continue;
      if (/cancel/i.test(String(p.status || ''))) continue;
      const dv = String(p.purchased_at || '').slice(0, 10);
      if (!/^\d{4}-\d{2}-\d{2}$/.test(dv) || dv < de || dv > ate) continue;
      const its = Array.isArray(p.itens) && p.itens.length ? p.itens : [{ sku: null, qtd: 1, valor: Number(p.total) || 0 }];
      ...
      for (const x of its) {
        ...
        const com = pctMag > 0 ? Math.round(vnota * pctMag / 100 * 100) / 100 : 0;
        const imp = Math.round(vnota * aq / 100 * 100) / 100;
        const mg = custo != null ? Math.round((vt - custo - com - imp) * 100) / 100 : null;
        linhas.push({ empresa, numero_pedido: 'MG-' + cod, numero_loja: cod, canal: 'magalu',
          data_venda: dv, sku: x.sku || null, descricao: null,
          quantidade: q2, valor_produto: vt, valor_nota: vnota, custo,
          comissao: com, frete_vendedor: 0, imposto: imp, margem: mg });
      }
      _mgc.inseridos++;
    }
    for (let i0 = 0; i0 < linhas.length; i0 += 200) {
      const lote = linhas.slice(i0, i0 + 200);
    ...
    if (_mgc.inseridos) console.log('[CACA-MAGALU] +' + _mgc.inseridos + ' pedido(s) / ' + _mgc.linhas + ' linha(s) que o Bling nao tinha (' + de + ' a ' + ate + ')');
  } catch (e) {
