...
const VERSAO     = 'amb-checkout-offline v11/08 b51';
...
function routes(readBody) {
  ...
  if (!body.fresh && c && c.nome !== undefined && (Date.now() - (c.ts || 0)) < TTL && (c.custo != null || c.saldo != null)) {
    ...
    const FOTO_V = 4;   // v4: cai pro SKU-BASE do kit quando o Bling devolve midia vazia   // v3: busca livre quando o código não acha + foto do componente do kit
    if (method === 'GET' && p === '/amb-checkout-offline/produto-fotos') {
      ...
      if (urlObj.searchParams && urlObj.searchParams.get('diag')) {
        ...
        try {
          const r1 = await blingGet('/produtos?codigo=' + encodeURIComponent(skD) + '&criterio=5&limite=1');
      ...
      for (const sk of faltando.slice(0, 16)) {
        try {
          // 1) pelo código exato
          let rb = await blingGet('/produtos?codigo=' + encodeURIComponent(sk) + '&criterio=5&limite=1');
          let it0 = (rb.ok && rb.data && rb.data.data && rb.data.data[0]) || null;
          ...
          if (it0 && img == null) {
            ...
            det = (dd.ok && dd.data && dd.data.data) || null;
          ...
          // 4) SKU-BASE do kit: neste catálogo os kits se chamam "NxSKU" (10xE14-5W-3000K-BIV
          //    é o kit de 10 do E14-5W-3000K-BIV). O diagnóstico de 11/08 provou que o Bling
          //    devolve `midia.imagens` VAZIA pra esses kits — externas [], internas [],
          //    imagensURL [] — mesmo com miniatura aparecendo na tela dele. Como o produto
          //    base TEM foto, usamos a dele: é a mesma lâmpada, só muda a quantidade.
          if (img == null) {
            const mBase = /^(\d+)\s*x\s*(.+)$/i.exec(sk);
            const skBase = mBase ? mBase[2].trim() : null;
            if (skBase && skBase !== sk) {
              const rB = await blingGet('/produtos?codigo=' + encodeURIComponent(skBase) + '&criterio=5&limite=1');
              const iB = (rB.ok && rB.data && rB.data.data && rB.data.data[0]) || null;
              let imgB = iB ? primeiraImagem(iB) : null;
              if (iB && imgB == null) {
                const dB = await blingGet('/produtos/' + iB.id);
                const detB = (dB.ok && dB.data && dB.data.data) || null;
                if (detB) imgB = primeiraImagem(detB);
              }
              if (imgB) img = imgB;
            }
          }
...
async function magaluLinhas(de, ate, empresa, jaTem) {
  ...
  for (const p of j.pedidos) {
    ...
    vivos.push(Object.assign({}, p, { _dv: dv, _cod: cod }));
  }
  ...
  for (const p of vivos) {
    ...
    const freTot = freteDe[cod] != null ? freteDe[cod] : 0;
    for (const x of its) {
