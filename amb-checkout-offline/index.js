...
const VERSAO     = 'amb-checkout-offline v11/08 b44';
  ...
  if (!body.fresh && c && c.nome !== undefined && (Date.now() - (c.ts || 0)) < TTL && (c.custo != null || c.saldo != null)) {
    ...
    if (method === 'GET' && p === '/amb-checkout-offline/produto-fotos') {
      ...
      for (const sk of skusF) {
        ...
        else if (c && (c.ts_img || c.ts) && (Date.now() - (c.ts_img || c.ts)) < 12 * 3600 * 1000) fotos[sk] = null;   // já procuramos há pouco: não tem foto
