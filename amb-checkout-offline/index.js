...
const VERSAO     = 'amb-checkout-offline v11/08 b37';
  ...
  try {
    ...
    try {
      ...
      if (SH_KEYd) {
        ...
        const rSd = await fetch(SH_URLd + '/' + (process.env.AMBBKP_SHOPEE_SYNC_LOJA || 'amb') + '/interno/pedidos-do-dia?horas=96&k=' + encodeURIComponent(SH_KEYd), { timeout: 90000 });   // b34: 96h = teto do serviço, cobre a janela [hoje-3, hoje+1]
        if (!rSd.ok) {
          const corpoSd = await rSd.text().catch(() => '');
          _vsy.shopee_direto = { erro: 'HTTP ' + rSd.status + ' — ' + corpoSd.slice(0, 90), url: SH_URLd, em: new Date().toISOString() };
