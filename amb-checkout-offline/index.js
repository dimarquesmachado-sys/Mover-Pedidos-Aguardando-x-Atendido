...
const VERSAO     = 'amb-checkout-offline v11/08 b38';
    ...
    try {
      ...
      if (_tkD) {
        ...
        if (sellerD) {
          ...
          for (const [k9, v9] of Object.entries(atual)) {
            if (!String(k9).startsWith('ml:') || !v9) continue;
            const oid9 = String(v9.numero_loja || '').trim();
            if (!oid9 || vistos.has(oid9)) continue;
            if (conferidas >= 40) break;                       // teto por rodada; o resto cai na próxima
            conferidas++;
            try {
