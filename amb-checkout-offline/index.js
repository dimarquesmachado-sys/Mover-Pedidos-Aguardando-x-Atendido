...
const VERSAO     = 'amb-checkout-offline v10/08 b31';
...
async function vendasSync() {
  ...
  try {
    ...
    try {
      ...
      if (_tkD) {
        ...
        if (sellerD) {
          ...
          for (let off = 0; off < 300 && off < totalD; off += 50) {
          ...
          _vsy.ml_direto = { novos: novosML, em: new Date().toISOString() };
    ...
    try {
      const SH_URLd = process.env.AMBBKP_SHOPEE_SYNC_URL || 'https://ambtotal-shopee-nf-sync-x-bling.onrender.com';
      ...
      if (SH_KEYd) {
        ...
        const rSd = await fetch(SH_URLd + '/' + (process.env.AMBBKP_SHOPEE_SYNC_LOJA || 'amb') + '/interno/pedidos-do-dia?horas=36&k=' + encodeURIComponent(SH_KEYd), { timeout: 90000 });
        ...
        if (jSd && jSd.ok && Array.isArray(jSd.pedidos)) {
          ...
          for (const pSd of jSd.pedidos) {
            ...
            if (/cancell/i.test(String(pSd.order_status || ''))) continue;
            atual['sh:' + osn] = {
    ...
    } catch (e) { console.log('[VENDAS-SYNC] fase shopee_direto falhou: ' + String(e.message || e).slice(0, 120)); }
    ...
    try {
      ...
      if (ADMd) {
        ...
        const rMd = await fetch('http://127.0.0.1:' + PORTd + '/magalu/pedidos-do-dia?empresa=' + MAG_EMPRESA + '&k=' + encodeURIComponent(ADMd), { timeout: 90000 });
        ...
        if (jMd && jMd.ok && Array.isArray(jMd.pedidos)) {
          ...
          for (const pMd of jMd.pedidos) {
            ...
            if (/cancel/i.test(String(pMd.status || ''))) continue;
            atual['mg:' + cod] = {
