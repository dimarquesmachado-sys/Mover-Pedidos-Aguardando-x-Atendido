'use strict';
// ════════════════════════════════════════════════════════════════════════
//  AMBTOTAL — PESCARIA RETROATIVA DE TARIFAS DO ML (porte da Girassol)
...
function rotasPescaria(ctx) {
  ...
  async function alvosDoPeriodo(de, ate) {
    ...
    for (let pag = 0; pag < 60; pag++) {
      const q = 'vendas_historico?empresa=eq.amb&canal=eq.ml' +
  ...
  async function gravarPedido(reg, fee) {
    ...
    for (const l of reg.linhas) {
      ...
      const r = await supaReq('amb', 'PATCH', 'vendas_historico?id=eq.' + encodeURIComponent(l.id), { comissao: com, margem: mg });
  ...
  return async function handlePescaria(req, res, urlObj) {
    ...
    if (method === 'GET' && p === '/amb-checkout-offline/pescar-tarifas-status') {
      if (!admOk(req, urlObj)) { json(res, 404, { error: 'not found' }); return true; }
      json(res, 200, Object.assign({ ok: true }, _pes));
      return true;
    }

    if (method === 'GET' && p === '/amb-checkout-offline/pescar-tarifas') {
      if (!admOk(req, urlObj)) { json(res, 404, { error: 'not found' }); return true; }
      ...
      const { url: uu, key: kk } = supaCfg('amb');
      if (!uu || !kk) { json(res, 500, { ok: false, erro: 'Supabase não configurado' }); return true; }

      // ── MODO 2: um pedido só, com antes/depois ──────────────────────────────
      if (umPedido) {
        const campos = 'id,numero_pedido,numero_loja,canal,data_venda,valor_produto,custo,imposto,frete_vendedor,comissao';
        const linhas = await supaGet('vendas_historico?empresa=eq.amb&numero_pedido=eq.' + encodeURIComponent(umPedido) + '&select=' + campos);
      ...
      if (massa && gravar) {
        ...
        json(res, 202, { ok: true, msg: 'pescando em segundo plano', de, ate, max, status: '/amb-checkout-offline/pescar-tarifas-status' });
        return true;
      }
    ...
    // ════════════════════════════════════════════════════════════════════════
    if (method === 'GET' && p === '/amb-checkout-offline/auditoria-ml') {
      if (!admOk(req, urlObj)) { json(res, 404, { error: 'not found' }); return true; }
      const q = urlObj.searchParams;
      const de = String((q && q.get('de')) || '').slice(0, 10);
      const ate = String((q && q.get('ate')) || '').slice(0, 10);
      ...
      let off = 0;
      for (let pag = 0; pag < 60; pag++) {
        const lote = await supaGet('vendas_historico?empresa=eq.amb&canal=eq.ml' +
          '&data_venda=gte.' + de + '&data_venda=lte.' + ate +
