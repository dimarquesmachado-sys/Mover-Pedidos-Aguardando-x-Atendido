'use strict';
// ════════════════════════════════════════════════════════════════════════
//  AMBTOTAL · CHECKOUT OFFLINE — MÓDULO DE HISTÓRICO E ANÁLISE (porte da Girassol)
...
function rotasHistorico(ctx) {
  ...
  return async function handleHistorico(req, res, urlObj) {
    ...
    // Uso: /amb-checkout-offline/previsao-vendas?base=180  (dias de histórico usados como base)
    ...
    if (method === 'GET' && p === '/amb-checkout-offline/previsao-vendas') {
      ...
      const { url: uP, key: kkP } = supaCfg('amb');
      ...
      try {
        while (offP < 120000) {
          const rq = await fetch(uP.replace(/\/+$/, '') + '/rest/v1/vendas_historico?empresa=eq.amb&data_venda=gte.' + dePV + '&data_venda=lte.' + atePV +
    ...
    // Uso: /amb-checkout-offline/historico-linhas?de=&ate=&off=0&lim=100
    if (method === 'GET' && p === '/amb-checkout-offline/historico-linhas') {
      ...
      const { url: uR, key: kkR } = supaCfg('amb');
      if (!uR || !kkR) { json(res, 500, { ok: false, erro: 'Supabase não configurado' }); return true; }
      const HH = { apikey: kkR, Authorization: 'Bearer ' + kkR };
      const BASE = uR.replace(/\/+$/, '') + '/rest/v1/vendas_historico?empresa=eq.amb&data_venda=gte.' + deR + '&data_venda=lte.' + ateR;
      ...
      const _aliqR = mes => { const a = _cfgR.aliquotas && _cfgR.aliquotas[mes];
        /* 19/08: 0% salvo era campo em branco gravado por engano — zero não é alíquota. */
    ...
    // Uso: /amb-checkout-offline/historico-longo?de=YYYY-MM-DD&ate=YYYY-MM-DD
    if (method === 'GET' && p === '/amb-checkout-offline/historico-longo') {
      ...
      try {
        while (offset < 60000) {
          const rq = await fetch(uL.replace(/\/+$/, '') + '/rest/v1/vendas_historico?empresa=eq.amb&data_venda=gte.' + deL + '&data_venda=lte.' + ateL + '&select=' + campos + '&order=data_venda.asc,numero_pedido.asc,sku.asc&limit=1000&offset=' + offset, { headers: H });
          if (!rq.ok) break;
          ...
          for (const l of linhas) {
            ...
            porCanal[cn].fat += vn; porCanal[cn].un += q; porCanal[cn].mar += (mg || 0) + (Number(l.credito_ml) || 0);
