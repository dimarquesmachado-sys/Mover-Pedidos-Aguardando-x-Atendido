...
const server = http.createServer(async (req, res) => {
  ...
  if (path.startsWith('/magalu/')) {
    const precisaAdmin = (path === '/magalu/conectar' || path === '/magalu/status' || path === '/magalu/teste' || path === '/magalu/sonda' || path === '/magalu/valores' || path === '/magalu/financeiro' || path === '/magalu/financeiro-lote' || path === '/magalu/reputacao');
    if (precisaAdmin) {
      if (!ADMIN_KEY || urlObj.searchParams.get('k') !== ADMIN_KEY) {
        return json(res, 404, { error: 'not found', path });
      }
