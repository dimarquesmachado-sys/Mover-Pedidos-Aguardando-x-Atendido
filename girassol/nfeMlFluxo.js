...
async function _fluxoNFeML(tokenBling) {
  ...
  // A Girassol gera MUITA NF por dia (centenas dentro da janela), e a lista
  // vem da MAIS ANTIGA para a mais NOVA. Uma NF que o Bling não enviou ao ML
  // é SEMPRE recente. Por isso: (1) descartamos as já resolvidas hoje e
  ...
  console.log(`[F3-NFeML] janela ${NF_JANELA_DIAS}d (${inicial}→${final}) | ${lista.length} autorizadas | ${pendentes.length} pendentes | processando ${batch.length} (mais recentes)`);
    ...
    try {
      ...
      if (msg.includes('não é invoice_pending') || msg.includes('invoice_pending')) {
        ...
        if (ESTADOS_FINAIS.includes(String(e.mlStatus || ''))) {
          ...
          console.log(`[F3-NFeML] NF ${nf.numero} (pedido ML ${numeroPedidoLoja}) — envio já em "${e.mlStatus}", não pede mais NF — marcando como ok`);
          marcarProcessado('F3', nfeId);
          _semPendenciaCount.delete(nfeId);
        } else {
        ...
        if (_perm.permanente) {
          ...
          marcarProcessado('F3', nfeId);
          console.error(`[F3-NFeML] NF ${nf.numero} (pedido ML ${numeroPedidoLoja}) TRAVADA — ${_perm.registro.motivo}. Parei de tentar; resolva no Bling e ela sai da lista.`);
        } else {
