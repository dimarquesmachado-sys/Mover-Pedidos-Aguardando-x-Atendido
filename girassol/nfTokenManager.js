...
async function postOAuthNF(body) {
  ...
  if (!resp.ok || !data || !data.access_token || !data.refresh_token) {
    throw new Error('NF OAuth: resposta inválida (HTTP ' + resp.status + '): ' + String(cru).slice(0, 120));
  }
...
async function garantirTokenNF() {
  ...
  if (resp.status === 403) {
    throw new Error('token NF sem a permissão de notas fiscais (403 na sonda /nfe) — reautorize o app NF em /setup-nf');
  }
  if (resp.status === 401) {
