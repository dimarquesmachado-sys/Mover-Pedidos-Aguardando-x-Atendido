...
async function getCidadeViaCEP(cepLimpo) {
  try {
    ...
    if (!municipio || !uf) return null;
    console.log(`[nfBlingApi] ViaCEP CEP=${cepLimpo} -> ${municipio}/${uf}`);
    return aplicarCorrecaoCidade(municipio, uf);
  } catch (e) {
    console.error('[nfBlingApi] Erro ViaCEP:', e.message);
    return null;
  }
}
...
// Ordem: cache -> BrasilAPI -> ViaCEP -> mapa manual FALLBACK_POR_CEP
