...
function _carregarIECache() {
  ...
  } catch (e) { console.error('[nfBlingApi] Erro lendo ie-cache:', e.message); }
  return {};
}
function _salvarIECache(cache) {
  ...
  } catch (e) { console.error('[nfBlingApi] Erro salvando ie-cache:', e.message); }
}
...
async function getNFsSituacaoConsulta(token) {
  ...
  console.log(`[nfBlingApi] getNFsSituacaoConsulta: ${recentes.length} NFs em Consulta situação (sit=10) nos últimos ${NF_JANELA_DIAS}d`);
  return recentes;
}
...
async function getCidadePorCEP(cep) {
  return _cepResolver.getCidadePorCEP(cep, "[nfBlingApi]");
}
...
async function getIEPorCNPJ(cnpj, uf) {
  ...
  if (hit && (Date.now() - hit.ts) < ttl) {
    ...
    if (hit.resultado) {
      ...
      console.log(`[nfBlingApi] IE do cache: ${chave} -> sem IE (negativo, não reconsulta)`);
    }
