 ...
 *
 * Esta é a Sessão 1 do projeto Lixas A COMBINAR: catalogar e listar variações filhas
...
async function tratar(req, res, urlObj) {
  ...
  if (method === 'GET' && p === '/lixas-combinar/catalogo') {
    json(res, 200, {
      ok: true,
      catalogo: lixasService.listarCatalogo()
    });
    return true;
  }
  ...
  if (method === 'GET' && p === '/lixas-combinar/oauth/callback') {
    ...
    try {
      const tokens = await tokenMgr.trocarCodePorTokens(code);
      html(res, 200, `
        <!doctype html>
  ...
  if (method === 'GET' && p.startsWith('/lixas-combinar/debug/produto/')) {
    ...
    try {
      ...
      // Já busca detalhes completos pra debug
...
async function iniciar() {
  ...
  if (!st.configurado) {
    console.log('[lixas-combinar] ⚠️ Defina LIXAS_BLING_CLIENT_ID e LIXAS_BLING_CLIENT_SECRET no Render');
  } else if (!st.tokens_ok) {
    console.log('[lixas-combinar] ⚠️ Tokens Bling não obtidos — acessar /lixas-combinar/setup');
  }
}
