...
/**
 * Autenticação do módulo /lixas-combinar
 *
 * Usuários são lidos da env var LIXAS_COMBINAR_USUARIOS no formato:
 *   LIXAS_COMBINAR_USUARIOS=Diego:senha1,Lucas:senha2,Ygor:senha3,Adriano:senha4
 *
 * O primeiro usuário da lista é considerado "admin", os demais "funcionario".
 * Pode-se forçar perfil usando ":admin" ou ":func" no final:
 *   LIXAS_COMBINAR_USUARIOS=Diego:senha1:admin,Lucas:senha2:func
 *
...
function parseUsuarios() {
  const raw = process.env.LIXAS_COMBINAR_USUARIOS || '';
  if (!raw.trim()) return [];
...
function autenticar(usuario, senha) {
  if (USUARIOS.length === 0) {
    return { ok: false, erro: 'Nenhum usuário cadastrado. Configure LIXAS_COMBINAR_USUARIOS no Render.' };
  }
