...
const VERSAO     = 'girassol-backup-offline v02/08 b88';
...
function routes(readBody) {
  ...
  if (!body.fresh && c && c.nome !== undefined && (Date.now() - (c.ts || 0)) < TTL && (c.custo != null || c.saldo != null)) {
    ...
    if (method === 'GET' && p === '/girassol-backup-offline/ml-billing-status') {
      ...
      json(res, 200, { ok: true, status: _mlb, tentativas: _mlb.tentativas || [], amostra_periodos: _mlb.amostra_periodos || null, atualizado: b.atualizado || null,
...
async function mlBillingSync(maxPeriodos) {
  ...
  try {
    ...
    const tentativas = [
      'https://api.mercadolibre.com/billing/integration/monthly/periods?group=ML',
      'https://api.mercadolibre.com/billing/integration/monthly/periods?group=ML&document_type=BILL',
      'https://api.mercadolibre.com/billing/integration/monthly/periods?group=ML&offset=0&limit=' + maxPeriodos,
      'https://api.mercadolibre.com/billing/integration/monthly/periods',
      'https://api.mercadolibre.com/billing/monthly/periods?group=ML'
    ];
    let dp = null;
    _mlb.tentativas = [];
    for (const u of tentativas) {
      ...
      try {
        ...
        st = rp.status;
        corpo = (await rp.text().catch(() => '')).slice(0, 400);
        if (rp.ok) { try { dp = JSON.parse(corpo); } catch (e) { dp = null; } }
    ...
    for (const key of periodos) {
      ...
      for (const docType of ['BILL', 'CREDIT_NOTE']) {
        ...
        for (let p = 0; p < 60; p++) {
          ...
          if (!r.ok) { if (r.status !== 404) _mlb.erros++; break; }
          const d = await r.json().catch(() => null);
          const its = (d && (d.results || d.details || d.data)) || [];
          if (!its.length) break;
          for (const it of its) {
            const idT = String(it.detail_id || it.id || it.charge_id || '');
            if (!idT) continue;
            const val = Number(it.amount != null ? it.amount : (it.charge_amount != null ? it.charge_amount : it.value)) || 0;
            const dia = String(it.date || it.charge_date || it.detail_date || '').slice(0, 10);
            const cat = _mlbCategoria(it.detail || it.description || it.concept || it.charge_type);
            const ord = String(it.order_id || it.sale_id || it.operation_id || '') || null;
            base.tarifas[idT] = { d: dia, v: Math.round(val * 100) / 100, c: cat, o: ord };
            _mlb.gravadas++;
          }
