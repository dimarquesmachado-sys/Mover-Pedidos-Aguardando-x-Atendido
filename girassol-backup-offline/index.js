...
const VERSAO     = 'girassol-backup-offline v02/08 b95';
...
function routes(readBody) {
  ...
  if (!body.fresh && c && c.nome !== undefined && (Date.now() - (c.ts || 0)) < TTL && (c.custo != null || c.saldo != null)) {
    ...
    if (method === 'GET' && p === '/girassol-backup-offline/ml-vendas-faltando') {
      ...
      const doML = new Map();   // numero_loja -> soma da comiss\u00e3o
      for (const t of Object.values(b.tarifas || {})) {
        if (!t.o || !t.d) continue;
        if (t.d < deF || t.d > ateF) continue;
        if (t.c !== 'comissao') continue;
        doML.set(String(t.o), Math.round(((doML.get(String(t.o)) || 0) + t.v) * 100) / 100);
      }
      ...
      const faltam = [];
      for (const [venda, com] of doML.entries()) if (!nosso.has(venda)) faltam.push({ venda, comissao_ml: com });
      ...
      json(res, 200, { ok: true, de: deF, ate: ateF,
        vendas_no_ml: doML.size, vendas_no_nosso_historico: nosso.size, faltando: faltam.length,
      ...
      for (const docType of ['BILL']) {
        ...
        for (let p = 0; p < 60; p++) {
          ...
          for (const it of its) {
            ...
            const ord = String(si.order_id || si.sale_id || si.pack_id || sh.shipment_id || '') || null;
            base.tarifas[idT] = { d: dia, v: Math.round(val * 100) / 100, c: _mlbCategoria(txt), o: ord, t: txt.slice(0, 60) };
            _mlb.gravadas++;
          }
