...
const VERSAO     = 'girassol-backup-offline v01/08 b85';
    ...
    if (method === 'GET' && p === '/girassol-backup-offline/historico-linhas') {
      ...
      if (!idx) {
        ...
        try {
          let o2 = 0;
          while (o2 < 80000) {
            const ri = await fetch(BASE + qCanal + '&select=numero_pedido&order=data_venda.desc,numero_pedido.desc&limit=1000&offset=' + o2, { headers: HH });
      ...
      let linhas = [];
      try {
        const rq = await fetch(BASE + qCanal + '&select=' + campos + '&order=data_venda.desc,numero_pedido.desc&limit=' + Math.max(1, qtdItens) + '&offset=' + off, { headers: HH });
