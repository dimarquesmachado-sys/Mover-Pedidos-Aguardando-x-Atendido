...
const VERSAO     = 'girassol-backup-offline v02/08 b93';
...
function routes(readBody) {
  ...
  if (!body.fresh && c && c.nome !== undefined && (Date.now() - (c.ts || 0)) < TTL && (c.custo != null || c.saldo != null)) {
    ...
    if (method === 'GET' && p === '/girassol-backup-offline/ml-billing-status') {
      ...
      json(res, 200, { ok: true, status: _mlb, tentativas: _mlb.tentativas || [], detalhes: _mlb.detalhes || [], amostra_item: _mlb.amostra_item || null, amostra_periodos: _mlb.amostra_periodos || null, atualizado: b.atualizado || null,
                       tarifas_guardadas: Object.keys(b.tarifas || {}).length, total_por_categoria: porCat,
...
async function backfillVendas(de, ate, empresa){
  ...
  try { await garantirSitCancel(async p2 => await blingGet(p2)); } catch (e) {}   // 01/08: IDs de cancelamento p/ o filtro abaixo
  const dorme = ms => new Promise(r=>setTimeout(r,ms));
  try {
    const custos = readJson(path.join(CACHE_DIR,'_custos.json'), {});
...
async function mlBillingSync(maxPeriodos) {
  ...
  try {
    ...
    for (const key of periodos) {
      ...
      for (const docType of ['BILL', 'CREDIT_NOTE']) {
        let fromId = 0;
        for (let p = 0; p < 60; p++) {
          ...
          const achar = (obj, re, ehNum) => {
            for (const [k, v] of Object.entries(obj || {})) {
              if (!re.test(k)) continue;
              if (ehNum && typeof v === 'number' && isFinite(v)) return v;
              if (!ehNum && v != null && typeof v !== 'object' && String(v).length) return String(v);
            }
            return null;
          };
          let pos = 0;
          for (const it of its) {
            pos++;
            const idT = String(it.detail_id || it.id || it.charge_id || achar(it, /(^|_)id$/i, false) || (key + '|' + docType + '|' + fromId + '|' + pos));
            const val = Number(it.amount != null ? it.amount : (it.charge_amount != null ? it.charge_amount :
                        (it.value != null ? it.value : achar(it, /amount|valor|charge|total|price/i, true)))) || 0;
            const dia = String(it.date || it.charge_date || it.detail_date || achar(it, /date|fecha|data/i, false) || '').slice(0, 10);
            const txt = it.detail || it.description || it.concept || it.charge_type || it.detail_type ||
                        achar(it, /detail|descri|concept|type|subtype|motivo|reason/i, false) || '';
            const ord = String(it.order_id || it.sale_id || it.operation_id || achar(it, /order|sale|venta|pack/i, false) || '') || null;
            base.tarifas[idT] = { d: dia, v: Math.round(val * 100) / 100, c: _mlbCategoria(txt), o: ord, t: String(txt).slice(0, 60) };
            _mlb.gravadas++;
          }
