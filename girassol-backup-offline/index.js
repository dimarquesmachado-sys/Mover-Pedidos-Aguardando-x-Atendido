...
const VERSAO     = 'girassol-backup-offline v04/08 b110';
...
async function backfillVendas(de, ate, empresa){
  ...
  try {
    ...
    const comBill = _mapaComissaoBilling();
    let tkFee = null;
    try { const { garantirTokenML: _gF } = require('../girassol/mlTokenManager'); tkFee = await _gF(); } catch (e) {}
      ...
      for(const p of lista){
        ...
        if ((!comissao || comissao <= 0) && canal === 'ml' && nl) {
          ...
          if (cB > 0) { comissao = cB; comFonte = 'billing'; }
            ...
            const f = await _feeMLLeve(nl, tkFee);
      ...
      if (empresa === 'girassol') {
        ...
        if (seller) {
          ...
          for (const t of Object.values(bill.tarifas || {})) {
            if (!t.o) continue;
            if (t.c === 'comissao' || t.c === 'mp') comDe[t.o] = Math.round(((comDe[t.o] || 0) + t.v) * 100) / 100;
