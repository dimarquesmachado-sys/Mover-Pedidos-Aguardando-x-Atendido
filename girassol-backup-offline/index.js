...
const VERSAO     = 'girassol-backup-offline v23/07 b22';
...
function routes(readBody) {
  return async function handle(req, res, urlObj) {
    ...
    if (method === 'GET' && p === '/girassol-backup-offline/ir-shopee') {
      ...
      try {
        const ckIr = String(process.env.GIRABKP_SHOPEE_COOKIE || '').trim();
        if (!ckIr) {
          passosIr.push({ passo: 'cookie', erro: 'env GIRABKP_SHOPEE_COOKIE vazia ou ausente' });
        } else {
