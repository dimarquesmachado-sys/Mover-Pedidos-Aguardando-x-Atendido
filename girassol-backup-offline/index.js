...
const VERSAO     = 'girassol-backup-offline v01/08 b87';
...
function routes(readBody) {
  ...
  if (!body.fresh && c && c.nome !== undefined && (Date.now() - (c.ts || 0)) < TTL && (c.custo != null || c.saldo != null)) {
    ...
    if (method === 'GET' && p === '/girassol-backup-offline/ml-billing-status') {
      ...
      json(res, 200, { ok: true, status: _mlb, atualizado: b.atualizado || null,
...
async function mlBillingSync(maxPeriodos) {
  ...
  try {
    const rp = await fetch('https://api.mercadolibre.com/billing/integration/monthly/periods?group=ML&limit=' + maxPeriodos, H);
    if (!rp.ok) throw new Error('periodos HTTP ' + rp.status);
    const dp = await rp.json();
    const periodos = (dp.results || dp.periods || dp || []).map(x => x.key || x.period_key || x).filter(x => typeof x === 'string');
