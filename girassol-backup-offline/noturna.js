...
function criarNoturna(ctx) {
  ...
  async function rotinaNoturna(disparo) {
    ...
    try {
      ...
      await etapa('devoluções do TikTok (' + DIAS_TIKTOK + ' dias)', async () => {
        const r = await devLib.coletarDevolucoesTikTok(ctxDev, 'girassol', DIAS_TIKTOK);
      ...
      await etapa('linha do tempo das devoluções do TikTok', async () => {
        const arqD = pathx.join(CACHE, '_tiktok_devolucoes_girassol.json');
        ...
        for (const d of faltam) {
          try {
            const r = await tkOauth.chamar('/return_refund/202309/returns/' + encodeURIComponent(d.id) + '/records', {}, {}, 'girassol');
    ...
    await etapa('varredura de cancelados (45 dias)', async () => {
      await varrerCancelados(45, 'girassol');
      return 'disparada';
    });
    await dorme(4000);
  ...
  function rotas(req, res, urlObj) {
    ...
    if (method === 'GET' && p === '/girassol-backup-offline/noturna-status') {
    ...
    if (method === 'GET' && p === '/girassol-backup-offline/rodar-noturna') {
      if (!adm) { json(res, 404, { error: 'not found' }); return true; }
      ...
      json(res, 202, { ok: true, msg: 'rotina noturna disparada em segundo plano', status: '/girassol-backup-offline/noturna-status' });
      return true;
    }
