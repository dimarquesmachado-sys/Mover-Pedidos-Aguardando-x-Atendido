   ...
   estado sem saída. Os dois são silenciosos — por isso a checagem grita no boot. */
{
  const _exp = require('../lib/checkout/conferir-expedicao').conferir({
    rotulo: 'GIRABKP',
    temExpedicao: require('../lib/empresas/registro').carregar({ servico: 'mover-pedidos' }).temCapacidade('girassol', 'expedicao') === true,
    SIT_VERIFICADO,
...
function routes(readBody) {
  ...
  const _rotasNfAnexar = require('../lib/checkout/rotas-nf-anexar').criar({
    prefixo: '/girassol-backup-offline', empresaId: 'girassol', json, readBody, readJson, writeJson, ensureDir, ehAdmin,
    ...
    for(let pg=1; pg<=500; pg++){
      ...
      for(const p of lista){
        ...
        /* 23/09 (retomada do #324 — esta é a parte GRAVE): a listagem ganhou 2/4/8 MINUTOS no
           ...
           SEGUIA até o DELETE, substituindo o histórico antigo por um novo COM BURACOS.
           Foram 30 assim em janeiro; numa rodada do ano seriam centenas.
           Agora o limite REAL aqui espera igual à listagem, e a varredura continua inteira. */
        let det=null;
        let esperas429det = 0;
        for (let td = 1; td <= 4; td++) {
          let _rdet = null;
          try { const rd = await _blingGet('/pedidos/vendas/'+p.id); _rdet = rd; det = (rd&&rd.ok&&rd.data&&rd.data.data)||null; } catch(e){}
          if (det) break;
          if (_rdet && _rdet.status === 429 && _rdet.limite !== false && esperas429det < 3) {
            ...
            await dorme(espL);
            td--;   // a espera longa não gasta uma das 4 tentativas normais
            continue;
          }
