...
const VERSAO     = 'girassol-backup-offline v06/08 b129';
...
async function varrerFornecedores(max) {
  ...
  // b128: o reset aqui NÃO tinha os campos novos do b127 (sem_custo, e os dois
  // contadores). Resultado: `_varFor.sem_custo.length` estourava no primeiro
  // produto sem fornecedor — "Cannot read properties of undefined".
  // Declarar o estado em dois lugares e esquecer um deles é o clássico; a lição
  // é manter o reset colado na declaração, com os MESMOS campos.
  _varFor = { rodando: true, fase: 'varrendo', vistos: 0, sem_fornecedor: 0, divergentes: 0, erros: 0,
              recuperados_por_componente: 0, sem_custo_mesmo: 0,
      ...
      for (const p0 of lista) {
        ...
        if (custoF <= 0) {
          ...
          for (const c2 of comps) {
            const idc = c2 && c2.produto && c2.produto.id;
            if (!idc) continue;
            const info = await custoDoProduto(idc, dorme);
            const q2 = Number(c2.quantidade || 1) || 1;
