...
const VERSAO     = 'girassol-backup-offline v06/08 b127';
...
const _noturna = criarNoturna({
  mlBillingSync, backfillVendas, mlSyncFees, varrerCancelados, canarioCron, podarExpedicao,
...
// b127 (06/08) — pergunta do Diego: "mesmo sem fornecedor, não dá pra pegar de outro
// lugar? pode ser produto de composição". Dá: num kit, o custo é a SOMA dos componentes,
// que é exatamente o que o Bling mostra como "Preço Total de Custo".
// Então, pra todo produto sem custo de fornecedor, eu abro a estrutura e somo os
// componentes. Cache por id porque o mesmo componente aparece em dezenas de kits.
...
async function varrerFornecedores(max) {
  ...
  const teto = Math.min(5000, Math.max(1, Number(max) || 1000));
  _varFor = { rodando: true, fase: 'varrendo', vistos: 0, sem_fornecedor: 0, divergentes: 0, erros: 0,
              inicio: new Date().toISOString(), fim: null, lista: [] };
  const dorme = ms => new Promise(r => setTimeout(r, ms));
  try {
    for (let pag = 1; pag <= 60 && _varFor.vistos < teto; pag++) {
      let lista = null;
      for (let tt = 1; tt <= 3 && !lista; tt++) {
        try { const rl = await blingGet('/produtos?pagina=' + pag + '&limite=100&criterio=2'); lista = (rl && rl.ok && rl.data && rl.data.data) || null; } catch (e) {}
        if (!lista) await dorme(3000 * tt);
      }
      ...
      for (const p0 of lista) {
        ...
        let d = null;
        try { const rd = await blingGet('/produtos/' + p0.id); d = (rd && rd.ok && rd.data && rd.data.data) || null; } catch (e) {}
        await dorme(430);
        if (!d) { _varFor.erros++; continue; }
        _varFor.vistos++;
        const sku = String(d.codigo || '').trim();
        const f = d.fornecedor || null;
        const custoF = Number((f && f.precoCusto) || 0) || 0;
        if (custoF <= 0) {
          // sem custo pelo fornecedor — tenta pela COMPOSIÇÃO antes de desistir
          _varFor.sem_fornecedor++;
          const comps = (d.estrutura && Array.isArray(d.estrutura.componentes)) ? d.estrutura.componentes : [];
          let soma = 0; const faltam = [];
          for (const c of comps) {
            const idc = c && c.produto && c.produto.id;
            if (!idc) continue;
            const info = await custoDoProduto(idc, dorme);
            const q2 = Number(c.quantidade || 1) || 1;
            if (info && info.custo > 0) soma += info.custo * q2;
            else faltam.push(idc);
          }
          soma = Math.round(soma * 100) / 100;
          if (soma > 0) _varFor.recuperados_por_componente++; else _varFor.sem_custo_mesmo++;
          if (_varFor.sem_custo.length < 300) _varFor.sem_custo.push({
            id: d.id, sku, nome: String(d.nome || '').slice(0, 70),
            tem_composicao: comps.length > 0, componentes: comps.length,
            custo_pelos_componentes: soma || null,
            componentes_sem_custo: faltam.length,
            fornecedor: (f && f.contato && f.contato.nome) || null
          });
          continue;
        }
        if (!f || !f.codigo) { continue; }
        const codF = String(f.codigo).trim();
        if (sku && codF && sku.toLowerCase() !== codF.toLowerCase()) {
          _varFor.divergentes++;
          if (_varFor.lista.length < 300) _varFor.lista.push({
            id: d.id, sku, codigo_no_fornecedor: codF,
            fornecedor: (f.contato && f.contato.nome) || null,
            custo: Number(f.precoCusto || 0), nome: String(d.nome || '').slice(0, 70)
          });
        }
      }
    }
  ...
  console.log('[FORNECEDORES] ' + _varFor.vistos + ' produtos · ' + _varFor.divergentes + ' com código de fornecedor diferente do SKU');
  return _varFor;
}
        ...
        if (canal === 'shopee' && nl && (SHOPEE_TODOS || !comissao || comissao <= 0)) {
          try {
            ...
            if (cEsc && cEsc.tarifa > 0) {
              ...
              // ⚠️ 06/08 — O FRETE FICA DE FORA POR ENQUANTO, DE PROPOSITO.
              // O dashboard ja tem uma cascata propria de frete (linha ~1808 do
              // dashboard.html): num pedido FLEX ele deduz o MOTOBOY da config (Shopee R$ 9),
              // porque o frete que o Bling importa num flex e o do COMPRADOR, nao custo da
              // loja. Se eu sobrescrevesse o frete_vendedor aqui com o liquido do escrow,
              // um pedido flex perderia o motoboy (caso real: 260805JQ6X1DUT, motoboy 9,00
              // na tela e liquido do escrow -8,00 — o certo seria 1,00, nao -8,00).
              // Entao: a COMISSAO vem do escrow (que e o ganho grande e esta provado) e o
              // frete continua como esta ate a gente tratar o Flex direito. O valor apurado
              // fica visivel no status pra nao perder a informacao.
              _backfill.shopee.frete_liquido_visto = Math.round(((_backfill.shopee.frete_liquido_visto || 0) + cEsc.frete_liquido_vendedor) * 100) / 100;
