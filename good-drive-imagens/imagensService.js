 ...
 *
 * Portado do projeto standalone (AMBTotal/GOOD Imagens), adaptado pro padrão
...
async function listarPastas() {
  ...
  if (!mae) throw new Error('GOODIMG_DRIVE_FOLDER_ID não configurado');
...
async function processarImagensRaw(imagens, contextoLog) {
  ...
  if (qtdTotal > MAX_IMAGENS_BLING) {
    ...
    console.log(`[good-drive-imagens] AVISO: ${aviso}`);
  }
  ...
  for (let i = 0; i < limitadas.length; i += 3) {
    ...
    await Promise.all(lote.map(async (img) => {
      ...
      catch (e) { console.log(`[good-drive-imagens] Aviso publicar ${img.name}: ${e.message}`); }
    }));
  }
...
async function processarUm({ sku, pastaId }) {
  ...
  try {
    ...
    console.log(`[good-drive-imagens] SKU ${sku} tem ${subpastas.length} subpasta(s) de variação`);
    const variacoes = {};
    ...
    for (const variacao of completo.variacoes) {
      ...
      if (!codigoVar) {
        try {
          ...
          console.log(`[good-drive-imagens] Erro buscando código da variação id=${variacao.id}: ${e.message}`);
        }
      }
      if (!codigoVar) {
