...
/**
 * blingProdutos.js — Consulta e atualiza produtos no Bling para /good-drive-imagens
 *
 * Funções:
...
const PAUSA_MS = Number(process.env.GOODIMG_BLING_PAUSA_MS || 400);
const MAX_RETRIES = 5;
...
async function blingReq(method, path, body) {
  ...
  for (let tentativa = 1; tentativa <= MAX_RETRIES; tentativa++) {
    ...
    if (r.status === 429) {
      ...
      console.log(`[good-drive-imagens blingProdutos] 429 em ${method} ${path} (${tentativa}/${MAX_RETRIES}) — aguardando ${waitMs}ms`);
      await sleep(waitMs);
      continue;
    }
    if (r.status === 503) {
...
async function atualizarImagens(idProduto, urls) {
  console.log(`[good-drive-imagens blingProdutos] atualizarImagens id=${idProduto}, ${urls.length} URLs`);
  ...
  console.log(`[good-drive-imagens blingProdutos] OK id=${idProduto}: ${externasDepois.length} externas confirmadas`);
  return {
    ok: true,
