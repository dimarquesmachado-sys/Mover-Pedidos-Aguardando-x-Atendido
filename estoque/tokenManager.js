 ...
 * Os tokens são salvos em /data/estoque/bling-tokens.json (disco persistente).
 *
 * Env vars necessárias:
 *   ESTOQUE_BLING_CLIENT_ID
 *   ESTOQUE_BLING_CLIENT_SECRET
 *   ESTOQUE_BLING_REDIRECT_URI
 *   ESTOQUE_TOKEN_FILE  (opcional, default /data/estoque/bling-tokens.json)
 */
...
const TOKEN_FILE = process.env.ESTOQUE_TOKEN_FILE || '/data/estoque/bling-tokens.json';
...
function lerTokens() {
  try {
    ...
    console.error('[estoque tokenManager] Erro ao ler tokens:', e.message);
    return {};
  }
}
...
function salvarTokens(access_token, refresh_token) {
  ...
  console.log('[estoque tokenManager] Tokens Bling salvos em disco ✓');
}

// ── OAuth helpers ────────────────────────────────────────────────────

function basicAuth() {
  const id  = process.env.ESTOQUE_BLING_CLIENT_ID;
  const sec = process.env.ESTOQUE_BLING_CLIENT_SECRET;
  if (!id || !sec) throw new Error('ESTOQUE_BLING_CLIENT_ID / ESTOQUE_BLING_CLIENT_SECRET não definidos');
...
async function gerarTokenInicial(auth_code) {
  ...
  const redirect_uri = process.env.ESTOQUE_BLING_REDIRECT_URI || '';
...
async function renovarToken() {
  ...
  _renovando = true;
  try {
    console.log('[estoque tokenManager] Renovando token Bling...');
    const { refresh_token } = lerTokens();
    if (!refresh_token) throw new Error('refresh_token ausente — rode /estoque/auth/bling primeiro');
    const redirect_uri = process.env.ESTOQUE_BLING_REDIRECT_URI || '';
    const data = await postOAuth({ grant_type: 'refresh_token', refresh_token, redirect_uri });
    salvarTokens(data.access_token, data.refresh_token);
    console.log('[estoque tokenManager] Token Bling renovado ✓');
...
async function garantirToken() {
  ...
  if (!access_token || access_token.length < 10) {
    console.log('[estoque tokenManager] Token Bling ausente — renovando');
    return renovarToken();
  }
  return access_token;
}
