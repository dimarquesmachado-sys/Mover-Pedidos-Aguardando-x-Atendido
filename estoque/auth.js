 ...
 *
 * Usuários são lidos da env var ESTOQUE_USUARIOS no formato:
 *   ESTOQUE_USUARIOS=Diego:senha1,Lucas:senha2,Ygor:senha3,Adriano:senha4
 *
 * O primeiro usuário da lista é considerado "admin", os demais "funcionario".
 * Pode-se forçar perfil usando ":admin" ou ":func" no final:
 *   ESTOQUE_USUARIOS=Diego:senha1:admin,Lucas:senha2:func
 *
...
function parseUsuarios() {
  const raw = process.env.ESTOQUE_USUARIOS || '';
  if (!raw.trim()) return [];
...
function autenticar(usuario, senha) {
  if (USUARIOS.length === 0) {
    return { ok: false, erro: 'Nenhum usuário cadastrado. Configure ESTOQUE_USUARIOS no Render.' };
  }
