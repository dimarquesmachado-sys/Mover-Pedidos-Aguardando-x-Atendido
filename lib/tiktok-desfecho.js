...
/* 29/08: tolerância de 1 centavo em todas as comparações — valores do TikTok vêm com 2
   casas e comparar contra 0 exato faria arredondamento virar "prejuízo" de R$ 0,00. */
const CENTAVO = 0.01;
...
function desfechoDoPedido(reg) {
  ...
  /* 29/08 — FALSO NEGATIVO ACHADO NO DADO REAL: pedidos com receita 0 e repasse NEGATIVO
     (ex.: -276,70, -47,19, -36,56 na Girassol) estavam saindo como 'limpa' com impacto 0,
     porque eu só olhava `ajustes_depois` — mas nesses registros o dinheiro saiu pelo próprio
     repasse (transação de REFUND/ajuste gravada sem receita). O líquido negativo É a perda,
     venha ela por onde vier: é dinheiro que saiu da conta da loja. */
  if (liquido < -CENTAVO && receita <= CENTAVO) {
    return { desfecho: 'debito_estorno', impacto: r2(liquido), receita, liquido,
             nota: 'lançamento negativo sem receita no período — estorno/débito puro (o valor saiu da conta)' };
  }
