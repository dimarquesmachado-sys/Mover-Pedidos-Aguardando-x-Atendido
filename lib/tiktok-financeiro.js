...
async function coletarFinanceiro(ctx, loja, dias, opts) {
  ...
  let truncou = false;   // teto de paginação atingido: o que veio NÃO é o período inteiro
  ...
  for (let volta = 0; volta < 60; volta++) {
    ...
    for (const st of lista) {
      ...
      for (let p2 = 0; p2 < 20; p2++) {
        ...
        if (!tk) break;
        if (p2 === 19) { truncou = true; console.log('[TIKTOK] extrato ' + id + ': mais de 20 páginas de transações — coleta truncada'); }
    ...
    // Codex (P2): estourar o teto de voltas sai do laço com token PENDENTE e sem marcar erro —
    // e eu registrava o intervalo inteiro como coletado. Uma calibração podia então usar só o
    // pedaço baixado, passar na checagem de cobertura e dar veredito conclusivo sobre dados
    // truncados. Teto atingido = coleta incompleta, e a cobertura não é registrada.
    if (volta === 59) { truncou = true; console.log('[TIKTOK] mais de 60 páginas de extratos — coleta truncada'); }
    await new Promise(r3 => setTimeout(r3, 250));
  }

  guardado.atualizado = new Date().toISOString();
  // Codex (P2): eu só escrevia no log do servidor e a rota devolvia ok:true limpo — quem chamou
  // (o Diego, ou a noturna) não tinha como saber que a coleta veio pela metade. O aviso sobe na
  // resposta.
  if (!erro && truncou) console.log('[TIKTOK] coleta truncada — cobertura NÃO registrada (o período pedido não foi baixado por inteiro)');
  guardado.truncou = !!truncou;
  if (!erro && !truncou) {
    guardado.ok_em = guardado.atualizado;
    // 19/08 (Codex, PR#139): quem sabe o que foi baixado é a COLETA, não quem lê o cache depois.
    // Sem este registro, a calibração só podia deduzir a cobertura pelo pedido mais antigo e o mais
    // novo — e duas coletas em janelas separadas deixam um buraco no meio que a dedução não vê.
    // Intervalos que se tocam são fundidos, pra lista não crescer sem fim.
    // Codex (P2): eu usava `dias` CRU. Pedir 800 dias baixa só 400 (o `total` limita), mas eu
    // registrava 800 como coletados — e a calibração de um período dentro dos 400 extras passava
    // na cobertura usando dados que nunca foram baixados. A janela registrada é a REAL.
    // Codex (P2, rodada seguinte): `desde` é um INSTANTE (meio-dia, por exemplo) e `hoje` ainda
    // está acontecendo — registrar os dois dias inteiros afirma ter baixado horas que não foram.
    // Só dias COMPLETOS entram: começa no dia seguinte ao instante inicial e termina ONTEM.
    // Codex (P2): eu calculava o dia em UTC, mas a calibração compara datas no fuso -03:00. Entre
    // 00:00 e 02:59 UTC o "ontem" do UTC ainda é HOJE no Brasil — e eu registrava como coberto um
    // dia que ainda está acontecendo. Tudo em -03:00, igual à calibração.
    const _diaBR = ms => new Date(ms - 3 * 3600000).toISOString().slice(0, 10);
    const _ini = Date.now() - total * 86400000;
    const _desde = _diaBR(_ini + 86400000);
    const _hoje = _diaBR(Date.now() - 86400000);
    // Codex (P2): a coleta pagina por DATA DE LIQUIDAÇÃO, mas eu registrava o intervalo como se
    // fosse de DATA DE PEDIDO — e a calibração seleciona por data de pedido. Um pedido feito num
    // dia já marcado como coberto pode liquidar semanas depois: a marca antiga diz "conferido" e a
    // calibração dá veredito conclusivo sem esses pedidos. A janela registrada recua uma margem de
    // liquidação (15 dias), que é o atraso típico do repasse: só afirma cobertura de pedidos cujo
    // prazo de liquidação já passou por inteiro dentro do que foi baixado.
    const _MARGEM_LIQ = 15;
    const _ateSeguro = _diaBR(Date.parse(_hoje + 'T12:00:00Z') - _MARGEM_LIQ * 86400000);
    const _lista = (Array.isArray(guardado.intervalos_coletados) ? guardado.intervalos_coletados.slice() : [])
      // coleta de 1 dia deixa `_desde` DEPOIS de `_hoje` (não há dia completo): não registra nada,
      // em vez de gravar um intervalo invertido que a checagem de cobertura leria como válido
      .concat(_desde <= _ateSeguro ? [{ de: _desde, ate: _ateSeguro, em: guardado.atualizado }] : [])
      .sort((a, b) => String(a.de).localeCompare(String(b.de)));
    const _fund = [];
    for (const it of _lista) {
      const u = _fund[_fund.length - 1];
      if (u && String(it.de) <= u.ate) { if (String(it.ate) > u.ate) u.ate = it.ate; }
      else _fund.push({ de: it.de, ate: it.ate, em: it.em });
    }
    guardado.intervalos_coletados = _fund;
  }
...
function calibrarRegraTarifa(ctx, loja, de, ate, pteInformado) {
  const g = ctx.readJson(arq(ctx, loja), { pedidos: {} });
  // Codex (P2): a coleta busca uma janela ROLANTE de dias e pode gravar resultado parcial depois de
  // um erro. Pedir a calibração de um intervalo maior do que foi coletado achava 30 pedidos e
  // carimbava o veredito como se cobrisse o período inteiro. Mede-se o que existe: o primeiro e o
  // último pedido guardados delimitam a cobertura REAL, e quando ela não alcança o pedido, o
  // veredito não é conclusivo.
  // Codex (P2): `Math.min(...array)` estoura o limite de argumentos do motor com cache grande
  // (~125 mil pedidos) e derrubaria a rota com RangeError. Redução iterativa, sem esse teto.
  let _tMin = Infinity, _tMax = -Infinity, _tN = 0;
  for (const p of Object.values(g.pedidos || {})) {
    const t = Number(p.criado_em) || 0;
    if (t > 0) { _tN++; if (t < _tMin) _tMin = t; if (t > _tMax) _tMax = t; }
  }
  const _dia = t => new Date(t * 1000).toISOString().slice(0, 10);
  // Codex (P2): eu DEDUZIA a cobertura do pedido mais antigo e do mais novo — mas coletas rolantes
  // em janelas separadas deixam buracos no meio (julho no cache + coleta só de agosto faz um pedido
  // de julho-a-agosto parecer coberto). Só a coleta sabe o que baixou; enquanto ela não registrar os
  // intervalos, esta rota NÃO afirma cobertura — reporta os extremos como informação e diz que o
  // intervalo coletado é desconhecido.
  const intervalos = Array.isArray(g.intervalos_coletados) ? g.intervalos_coletados : null;
  const cobreTudo = intervalos ? intervalos.some(i => String(i.de) <= de && String(i.ate) >= ate) : null;
  const cobertura = {
    primeiro_pedido: _tN ? _dia(_tMin) : null,
    ultimo_pedido: _tN ? _dia(_tMax) : null,
    coleta_ok_em: g.ok_em || null,
    intervalos_coletados: intervalos,
    cobre_o_periodo_pedido: cobreTudo
  };
  // Só o registro de intervalos decide. As comparações por extremos que eu tinha aqui eram a
  // heurística ANTIGA e agora atrapalhavam: um período legitimamente coletado onde só houve vendas
  // a partir do dia 10 era reprovado por "não ter pedido no dia 1" — ausência de venda não é
  // ausência de coleta.
  const coberturaFalha = !g.ok_em || !_tN || cobreTudo !== true;
  const ini = Date.parse(de + 'T00:00:00-03:00') / 1000, fim = Date.parse(ate + 'T23:59:59-03:00') / 1000;
  ...
  // Codex (P2): se TODOS os pedidos elegíveis tiverem tarifa zero, a amostra fica vazia e este
  // retorno diria "não havia pedidos" — escondendo o contraexemplo atrás de aparente falta de dado.
  if (!amostra.length) {
    // Codex (P2): sem cobertura confirmada, amostra vazia NÃO significa "não houve venda no
    // período" — significa que talvez nunca tenhamos baixado esses dias. Dizer "nenhum pedido"
    // faria o Diego ler ausência de coleta como ausência de venda, que é o pior tipo de engano.
    if (coberturaFalha) {
      return { ok: true, loja, de, ate, pedidos: 0, descartes, cobertura,
        veredito: { conclusivo: false, serve_para_estimar: false,
          motivo: 'não há pedidos na amostra E a coleta não cobre o período pedido — impossível dizer se não houve venda ou se os dados nunca foram baixados',
          o_que_fazer: 'rode a coleta do período e repita' } };
    }
    return zeradosLiquidados > 0
      ? { ok: true, loja, de, ate, pedidos: 0, descartes,
          veredito: { conclusivo: false, serve_para_estimar: false,
            motivo: 'todos os ' + zeradosLiquidados + ' pedido(s) liquidado(s) do período vieram com tarifa ZERO — a regra sempre prevê cobrança, então ou há isenção que não modelamos ou um campo de tarifa que não sabemos ler' } }
      : { ok: true, loja, de, ate, pedidos: 0, descartes, aviso: 'nenhum pedido liquidado sob a regra atual no período' };
  }
  ...
  const avalia = (comPTE, modoAfiliado) => {
    ...
    return {
      ...
      // valores SEM arredondamento, para o veredito decidir (os de cima são para leitura)
      bruto: {
        erro_relativo_medio: somaAbsRel / n * 100,
        dentro_de_10pct: dentro10 / n * 100,
        desvio_total: tarifaTot > 0 ? (previstoTot - tarifaTot) / tarifaTot * 100 : 0
      }
    };
  };
  ...
  // Codex (P2): eu ordenava pelo erro em REAIS, então pedidos caros ocupavam as dez vagas mesmo
  // errando pouco em percentual — e sumiam justamente os de ticket baixo com erro relativo enorme,
  // que são os que reprovam o modelo. O veredito decide por erro relativo; a lista também.
  }).sort((x, y) => (y.erro_relativo_pct || 0) - (x.erro_relativo_pct || 0)).slice(0, 10);
  ...
  return {
    ...
    veredito: (function(){
      ...
      if (amostra.length < MIN) {
        // Codex (P2): amostra pequena PORQUE a coleta não cobriu o período é outra história —
        // eu mandava "espere vender mais" quando o certo era "rode a coleta". A causa vem junto.
        return { conclusivo: false,
                 motivo: 'amostra pequena: ' + amostra.length + ' pedido(s) liquidado(s) sob a regra atual (mínimo ' + MIN + ')'
                       + (coberturaFalha ? ' — e a coleta não cobre o período pedido, então pode haver mais pedidos nunca baixados' : ''),
                 o_que_fazer: coberturaFalha ? 'rode a coleta do período e repita' : 'aguarde mais pedidos liquidarem no período',
                 cobertura,
      ...
      if (coberturaFalha) {
        return { conclusivo: false, serve_para_estimar: false,
          motivo: (cobreTudo === null
            ? 'a coleta ainda não registra QUAIS intervalos baixou, então não dá para afirmar que o período pedido foi coberto por inteiro — coletas em janelas separadas deixam buracos invisíveis'
            : 'a coleta não cobre o período pedido') + (g.ok_em ? '' : ' — e a última coleta não terminou com sucesso'),
          cobertura,
          o_que_fazer: 'rode /tiktok/financeiro-coletar com dias suficientes e repita' };
      }
      ...
      return {
        ...
        // Codex (P2): eu comparava os valores JÁ arredondados para exibição (uma decimal), então um
      // erro real de 5,04% virava 5,0% e passava no limite de 5%. O veredito usa os brutos.
      // Codex (P2): 27 acertos exatos e 3 erros de +69%, -29% e +11% passavam — a média fica baixa,
      // 90% ficam dentro de 10% e os desvios se cancelam no total. Mas um estimador que erra 69%
      // num pedido não serve pra decidir preço: teto explícito no PIOR caso.
      serve_para_estimar: e.bruto.erro_relativo_medio <= 5 && e.bruto.dentro_de_10pct >= 90
                       && Math.abs(e.bruto.desvio_total) <= 3 && e.pior_erro_relativo_pct <= 25,
