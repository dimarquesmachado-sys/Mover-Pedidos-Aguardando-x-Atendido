...
function calibrarRegraTarifa(ctx, loja, de, ate) {
  ...
  const amostra = [];
  let fora_regra_antiga = 0, fora_estorno = 0, fora_nao_fecha = 0;
  for (const p of Object.values(g.pedidos || {})) {
    const t = Number(p.criado_em) || 0;
    if (!(t >= ini && t <= fim)) continue;
    ...
    const tarifaLiq = Number(p.tarifa) || 0;
    if (!(tarifaLiq > 0)) continue;
    amostra.push({ order_id: p.order_id, receita, tarifa: tarifaLiq, afiliado: Number(p.afiliado) || 0 });
  }
  const descartes = { regra_antiga_antes_de_15_07_2026: fora_regra_antiga, com_estorno_de_tarifa: fora_estorno, identidade_nao_fechou: fora_nao_fecha };
  ...
  const melhorReal = comPTE.erro_medio_absoluto < semPTE.erro_medio_absoluto ? comPTE : semPTE;
  const comoVaiEstimar = avalia(melhorReal.com_pte, 'medio');   // ESTE é o estimador de verdade
  const melhor = melhorReal;
