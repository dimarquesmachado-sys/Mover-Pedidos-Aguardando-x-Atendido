...
async function coletarDevolucoesTikTok(ctx, loja, dias) {
  ...
  let vistas = 0, novas = 0, erro = null, token = '';
  for (let v = 0; v < 60; v++) {
    const params = { page_size: '50' };
    if (token) params.page_token = token;
    let r = null;
    try {
      r = await ctx.chamar('/return_refund/202309/return_orders/search', params,
        { metodo: 'POST', body: { create_time_ge: desde } }, loja);
    } catch (e) { erro = String(e.message || e).slice(0, 160); break; }
    if (!r || !r.ok || !r.corpo || r.corpo.code !== 0) {
      erro = 'return_orders: ' + ((r && r.corpo && r.corpo.message) || ('HTTP ' + (r && r.http)));
      break;
    }
    const lista = (r.corpo.data && (r.corpo.data.return_orders || r.corpo.data.refund_orders)) || [];
    if (!lista.length) break;
    for (const d of lista) {
      ...
      g.devolucoes[id] = {
        ...
        valor: _n((d.refund_amount && (d.refund_amount.refund_total || d.refund_amount.total)) || d.refund_total || 0),
    ...
    if (v === 59) { erro = 'lista de devoluções TRUNCADA: mais de 3.000 no período — reduza &dias='; break; }
