...
function calibrarRegraTarifa(ctx, loja, de, ate) {
  const g = ctx.readJson(arq(ctx, loja), { pedidos: {} });
  const ini = Date.parse(de + 'T00:00:00-03:00') / 1000, fim = Date.parse(ate + 'T23:59:59-03:00') / 1000;
  ...
  for (const p of Object.values(g.pedidos || {})) {
    ...
    if (Math.abs(p.confere || 0) >= 0.01) continue;  // identidade não fechou: fora da calibração
    const receita = Number(p.receita) || 0;
    if (receita <= 0) continue;
    const tarifaLiq = (p.tarifa_credito ? -p.tarifa : p.tarifa) - (p.tarifa_devolvida || 0);
    if (!(tarifaLiq > 0)) continue;                  // devolvido/estornado: fora
    amostra.push({ order_id: p.order_id, receita, tarifa: tarifaLiq, afiliado: Number(p.afiliado) || 0 });
  }
  if (!amostra.length) return { ok: true, loja, de, ate, pedidos: 0, aviso: 'nenhum pedido liquidado no período' };
  ...
  const avalia = comPTE => {
    let somaErro = 0, somaAbs = 0, dentro5 = 0, tarifaTot = 0, previstoTot = 0;
    for (const a of amostra) {
      const prev = regra(a.receita, 1, comPTE) + a.afiliado;   // afiliado real do próprio pedido
      const erro = prev - a.tarifa;
      somaErro += erro; somaAbs += Math.abs(erro);
      tarifaTot += a.tarifa; previstoTot += prev;
      if (a.tarifa > 0 && Math.abs(erro) / a.tarifa <= 0.05) dentro5++;
    }
  ...
  const semPTE = avalia(false), comPTE = avalia(true);
  const melhor = Math.abs(comPTE.erro_medio_absoluto) < Math.abs(semPTE.erro_medio_absoluto) ? comPTE : semPTE;
  ...
  const piores = amostra.map(a => {
    const prev = regra(a.receita, 1, melhor.com_pte) + a.afiliado;
    return { order_id: a.order_id, receita: a.receita, tarifa_real: a.tarifa,
             tarifa_prevista: prev, erro: Math.round((prev - a.tarifa) * 100) / 100 };
  ...
  return {
    ...
    veredito: {
      loja_parece_estar_no_pte: melhor.com_pte,
      erro_medio_do_melhor: melhor.erro_medio,
      acerto_dentro_de_5pct: melhor.dentro_de_5pct_percentual + '%',
      serve_para_estimar: Math.abs(melhor.desvio_total_pct) <= 3 && melhor.dentro_de_5pct_percentual >= 80
    },
    afiliado: {
    ...
    limitacoes: [
      ...
      'o afiliado usado e o REAL do proprio pedido; para estimar pedido sem extrato sera preciso usar a media medida aqui, que varia por anuncio',
      'pedidos com estorno/devolucao ficaram de fora'
    ]
  };
}
