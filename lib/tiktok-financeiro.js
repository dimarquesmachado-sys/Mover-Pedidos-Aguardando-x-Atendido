...
async function coletarFinanceiro(ctx, loja, dias, opts) {
  ...
  let truncou = false;   /* Codex #292: estourar as 60 páginas significa janela NÃO coberta */
  for (let volta = 0; volta < 60; volta++) {
    ...
    /* Codex #292 r2: truncamento só existe se a ÚLTIMA volta permitida terminar com página
       PENDENTE — quem fecha exatamente na 60 varreu tudo, e marcar antes tirava a cobertura
       de uma coleta boa. Aqui já sabemos que há token e que a janela não acabou. */
    if (volta === 59) truncou = true;
  ...
  if (!erro) {
    ...
    /* Codex #292: varredura TRUNCADA pelo limite de páginas (60×50 = 3.000 extratos) sai
       sem erro — declarar cobertura nesse caso seria mentir sobre o que foi visto. */
    if (truncou) {
      guardado.varreu_desde = null; guardado.varreu_dias = null; guardado.truncou_paginas = true;
    } else {
      guardado.varreu_desde = new Date(desde * 1000).toISOString();   /* base do INÍCIO */
      guardado.varreu_dias = Number(total) || null;
      guardado.truncou_paginas = false;
    }
  }
...
async function coletarDevolucoesTikTok(ctx, loja, dias) {
  ...
  if (!erro) {
    ...
    /* Codex #292: usa o `desde` calculado no INÍCIO da varredura, não Date.now() no fim —
       numa coleta longa o recálculo faria a janela declarada ficar menor que a pedida. */
    g.varreu_desde = new Date(desde * 1000).toISOString();
...
/* ─────────────────────────────────────────────────────────────────────────────
   COLETOR POR EMPRESA (fatia 6 da desduplicação, 13/09/2026).

   Este wrapper vivia copiado nos dois checkouts e a ÚNICA diferença entre as
   cópias era a palavra 'amb' / 'girassol' — o caso mais puro da regra da casa
   (peça nova nasce com a empresa como parâmetro). Copiado, ele obrigava a
   lembrar de mexer nos dois lados a cada conserto; aqui, empresa nova só passa
   a chave.

   Continua sem conhecer o mundo: quem entrega o módulo do TikTok e o acesso ao
   disco é o chamador, o que também deixa isto testável sem token nenhum.
   ──────────────────────────────────────────────────────────────────────────── */
function criarColetorDaEmpresa(empresa, deps) {
  if (!empresa) throw new Error('lib/tiktok-financeiro: empresa é obrigatória no coletor');
  /* Codex #395 (P1): a coleta entra por INJEÇÃO (com o padrão apontando pra função deste
     módulo). Antes, o teste tentava trocar o export pra espiar a chamada e não trocava
     nada — a função interna é chamada direto —, então aquele trecho não provava coisa
     alguma. Agora dá pra provar o que importa: que a EMPRESA chega certa na coleta. */
  const { carregarTikTok, fs, path, coletar } = deps || {};
  const coletarFn = typeof coletar === 'function' ? coletar : coletarFinanceiro;
  if (typeof carregarTikTok !== 'function') throw new Error('lib/tiktok-financeiro: falta a dependência carregarTikTok');
  return async function coletarFinanceiroTikTok(dias) {
    let tk = null;
    try { tk = carregarTikTok(); } catch (e) { return { ok: true, pulado: 'módulo do TikTok indisponível', pedidos_novos: 0, guardados: 0 }; }
    if (!tk || typeof tk.chamar !== 'function' || !tk.lerToken || !tk.lerToken(empresa)) {
    ...
    const ctxFin = {
      ...
      readJson: (a, p) => { try { return JSON.parse(fs.readFileSync(a, 'utf8')); } catch (e) { return p; } },
      writeJson: (a, v) => { try { fs.mkdirSync(path.dirname(a), { recursive: true }); } catch (e) {} fs.writeFileSync(a, JSON.stringify(v, null, 2)); },
      chamar: tk.chamar,
    };
    return coletarFn(ctxFin, empresa, dias || 35, {});
  };
}

module.exports.criarColetorDaEmpresa = criarColetorDaEmpresa;
