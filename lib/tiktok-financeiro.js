...
function calibrarRegraTarifa(ctx, loja, de, ate, pteInformado) {
  ...
  const _ts = Object.values(g.pedidos || {}).map(p => Number(p.criado_em) || 0).filter(t => t > 0);
  const _dia = t => new Date(t * 1000).toISOString().slice(0, 10);
  const cobertura = _ts.length
    ? { primeiro_pedido: _dia(Math.min(..._ts)), ultimo_pedido: _dia(Math.max(..._ts)), coleta_ok_em: g.ok_em || null }
    : { primeiro_pedido: null, ultimo_pedido: null, coleta_ok_em: g.ok_em || null };
  const coberturaFalha = !g.ok_em
    || !_ts.length
    || cobertura.primeiro_pedido > de
    || cobertura.ultimo_pedido < ate;
  const ini = Date.parse(de + 'T00:00:00-03:00') / 1000, fim = Date.parse(ate + 'T23:59:59-03:00') / 1000;
    ...
    veredito: (function(){
      ...
      if (coberturaFalha) {
        return { conclusivo: false, serve_para_estimar: false,
          motivo: 'a coleta não cobre o período pedido' + (g.ok_em ? '' : ' (a última coleta não terminou com sucesso)'),
          cobertura,
