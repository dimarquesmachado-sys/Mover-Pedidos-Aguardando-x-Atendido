...
async function coletarFinanceiro(ctx, loja, dias, opts) {
  ...
  if (!erro && !truncou) {
    ...
    const _ini = new Date(Date.now() - total * 86400000);
    const _desde = new Date(_ini.getTime() + 86400000).toISOString().slice(0, 10);
    const _hoje = new Date(Date.now() - 86400000).toISOString().slice(0, 10);
