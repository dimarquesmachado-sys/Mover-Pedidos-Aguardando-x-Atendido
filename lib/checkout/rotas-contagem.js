...
function criar(ctx) {
  ...
  /* Codex #509 (P1): o saldo é o dado que torna a divergência auditável, e ele precisa ser o
     ...
     Vai direto ao Bling de propósito: produtoDetalhe tem cache sem TTL, e saldo de horas atrás
     grava uma divergência errada pra sempre. */
  ...
  return async function rotasContagem(req, res, urlObj, method) {
    ...
    if (method === 'GET' && p === (prefixo + '/contagem-buscar-nome')) {
      ...
      json(res, 200, {
        ok: true,
        itens,
        /* Codex #509 (P2): "vazio" não é o mesmo que "incompleto", e a diferença muda o que o
           funcionário deve fazer. Basta UM produto ter passado por uma busca anterior pro
           índice deixar de estar vazio — e aí procurar qualquer outro produto dizia
           "nada encontrado", como se ele não existisse, quando na verdade o catálogo nunca
           foi indexado. Agora a tela sabe as duas coisas. */
        indice_vazio: Object.keys(idx).length === 0,
        indice_completo: !!(ctx.getIdxStatus && (ctx.getIdxStatus() || {}).fim),
        total: itens.length,
      });
      return true;
    }
    ...
    if (method === 'GET' && p === (prefixo + '/contagem-lista')) {
      ...
      const lista = de ? d.lancamentos.filter(l => diaSP(l.quando) >= de) : d.lancamentos;
