...
function criar(ctx) {
  ...
  /* dá nome aos componentes: o Bling às vezes devolve só o id, em uma de DUAS formas —
     `produto.id` (composição já vista aqui) ou `componente.id` (a mesma forma que
     amb-drive-imagens/blingProdutos.js lê/escreve ao preservar composição num PATCH). Sem
     tratar as duas, entradas reais no formato `componente` caíam sem código nem nome, o filtro
     abaixo (`sku || nome`) as descartava, e a recusa saía sem dizer o que contar — a mensagem
     genérica de antes. Resolve no máximo 6 — além disso a lista já não cabe na tela, e recusar
     um lançamento não pode custar uma rajada de chamadas. Quem não resolver sai com o id, que
  ...
  async function resolverComponentes(comps, sinal) {
    ...
    for (const c of comps) {
      ...
      let sku = (c.produto && c.produto.codigo) || (c.componente && c.componente.codigo) || c.codigo || '';
      let nome = (c.produto && c.produto.nome) || (c.componente && c.componente.nome) || c.nome || '';
      const id = (c.produto && c.produto.id) || (c.componente && c.componente.id) || c.idProduto || c.id || null;
