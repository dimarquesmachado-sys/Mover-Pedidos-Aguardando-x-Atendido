...
function criar(ctx) {
  ...
  return async function rotasContagem(req, res, urlObj, method) {
    ...
    if (method === 'POST' && p === (prefixo + '/contagem-excluir')) {
      ...
      /* Codex #524 (P2): excluir um lançamento JÁ aplicado esconde da lista a evidência de um
         movimento real no Bling — quem revisa depois não teria como saber que ele existiu. */
      if (l.aplicado_em) { json(res, 200, { ok: false, erro: 'este lançamento já foi aplicado no Bling em ' + l.aplicado_em + ' — excluir esconderia um movimento real já feito' }); return true; }
