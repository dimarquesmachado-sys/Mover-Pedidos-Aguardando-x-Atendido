...
/* ctx precisa de: prefixo, json, validarSessao, blingGet, produtoDetalhe,
   primeiraImagem, getPossiveisGtins, lerIndiceEan, CACHE_DIR, readJson,
   writeJson, empresa */
function criar(ctx) {
  ...
  return async function rotasContagem(req, res, urlObj, method) {
    ...
    if (method === 'GET' && p === (prefixo + '/contagem-lista')) {
      ...
      json(res, 200, {
        ok: true,
        lancamentos: lista.slice(-500).reverse(),
