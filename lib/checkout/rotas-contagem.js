...
function criar(ctx) {
  ...
  async function aplicarUm(id, sess, dep, reservaDoLote) {
    ...
    if (method === 'POST' && p === (prefixo + '/contagem-lancar-todas')) {
      ...
      if (foto) {
        const porId = new Map(foto.map(i => [String(i.id), Number(i.qtd)]));
        const sobrando = fila.filter(f => !porId.has(String(f.id)));
        const mudou = fila.filter(f => porId.has(String(f.id)) && porId.get(String(f.id)) !== f.qtd);
        const sumiu = foto.filter(i => !fila.some(f => String(f.id) === String(i.id)));
        if (sobrando.length || mudou.length || sumiu.length) {
          json(res, 200, { ok: false, desatualizado: true,
            erro: 'a lista mudou desde que você abriu a tela (' +
                  [sobrando.length ? sobrando.length + ' novo(s)' : '',
                   mudou.length ? mudou.length + ' com quantidade diferente' : '',
                   sumiu.length ? sumiu.length + ' que sumiu(ram)' : ''].filter(Boolean).join(', ') +
                  ') — nada foi lançado. Atualize a página e confira antes de lançar.' });
          return true;
        }
      }
