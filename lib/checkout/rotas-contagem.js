...
function criar(ctx) {
  ...
  async function aplicarUm(id, sess, dep) {
      let d;
    try { d = ler(); } catch (e) {
    ...
    if (method === 'POST' && p === (prefixo + '/contagem-lancar-todas')) {
      ...
      if (fila.length > 200) { json(res, 200, { ok: false, erro: 'são ' + fila.length + ' lançamentos — muitos pra uma rodada só. Lance por partes.' }); return true; }

      const itens = [];
      let feitos = 0, falhas = 0, seguidas = 0;
      for (const f of fila) {
        const r = await aplicarUm(f.id, sess, dep);
        itens.push({ sku: f.sku, qtd: f.qtd, ok: !!(r && r.ok), erro: (r && r.erro) || '', aviso: (r && r.aviso) || '' });
        if (r && r.ok) { feitos++; seguidas = 0; }
        else {
          falhas++; seguidas++;
          /* três seguidas é sinal de problema geral (token, cota, depósito), não do item —
             insistir só queimaria cota e demoraria mais pra ele descobrir */
          if (seguidas >= 3) { itens.push({ parou: true, erro: 'parei depois de 3 falhas seguidas — parece problema geral, não do item' }); break; }
        }
        await new Promise(s2 => setTimeout(s2, 400));   // respiro pra cota do Bling
      }
      json(res, 200, { ok: true, feitos, falhas, total: fila.length, itens });
      return true;
    }
