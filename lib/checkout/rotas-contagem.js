...
function criar(ctx) {
  ...
  const _estoqueEntrada = () => {
    /* Codex #526 (P2, r4): sem PAUSA_MS/sleep, a lib de entrada não tem como espaçar as
       chamadas de dentro de UM item (busca, cadastro, POST) — só o lote de fora espaçava entre
       itens. Se o contexto desta empresa ainda não os passa (a rota nasceu só pra Girassol),
       cai num respiro fixo em vez de derrubar a lib inteira. */
    if (!_libEstoque) _libEstoque = require('./estoque-entrada').criar({
      blingGet: ctx.blingGet, blingWrite: ctx.blingWrite,
      PAUSA_MS: ctx.PAUSA_MS || 350, sleep: ctx.sleep || (ms => new Promise(r => setTimeout(r, ms))),
    });
    return _libEstoque;
  };
  ...
  /* Codex #524 (P1): `ehAdmin` sozinho libera TODO MUNDO quando a env de admin está vazia —
     comportamento herdado de rotas menos arriscadas (reabrir/reverter pedido), documentado como
     "vazio = sem restrição". Nas DUAS rotas que tocam a escrita real no Bling (aplicar e a
     descoberta de depósito que ela depende) isso é errado demais pra herdar calado: uma
     implantação com operadores configurados mas sem admin explícito deixaria QUALQUER um lançar
     estoque de verdade. Aqui é fail-closed: sem admin configurado, ninguém passa. */
  const ehAdminExplicito = (sess) => {
    if (typeof ctx.ehAdmin !== 'function' || typeof ctx.lerAdmins !== 'function') return false;
    if (ctx.lerAdmins().length === 0) return false;
    return ctx.ehAdmin(String(sess.nome || sess || ''));
  };

  /* Codex #509 (P2): o carimbo `quando` é ISO (UTC); quem lê e decide (a lista do dia, e agora
     também a observação gravada no Bling) trabalha em dia/hora de São Paulo. Extraído aqui pra
     as duas pontas não divergirem. */
  const diaSP = (iso) => {
    try { return new Date(iso).toLocaleString('sv-SE', { timeZone: 'America/Sao_Paulo' }).slice(0, 10); }
    catch (e) { return String(iso).slice(0, 10); }
  };

  /* Codex #524 (P1): `aplicado_em` só existe DEPOIS que o Bling responde — enquanto
     /contagem-aplicar está esperando essa resposta, o lançamento carrega só `aplicando_em`, e as
     outras quatro rotas de mutação (ajustar, definir, excluir, trocar tipo) checavam apenas
     `aplicado_em`. Nessa janela outra aba podia mudar a quantidade ou o tipo por baixo do pedido
     em voo: o Bling recebe o número antigo, mas a rota relê o registro DEPOIS e carimba
     `aplicado_em` em cima do valor já editado — o registro aplicado passa a descrever uma
     quantidade que nunca foi a enviada. Mesmo TTL de 120s da trava de duplo-clique do aplicar:
     depois disso o lock é considerado morto (processo caiu no meio) e não deve travar a tela pra
     sempre. */
  const emAndamento = (l) => !!(l.aplicando_em && (Date.now() - Date.parse(l.aplicando_em)) < 120000);

  /* Codex #526 (P2, r3): duas abas (ou dois admins) chamando /contagem-lancar-todas quase
     juntas liam a mesma fila antes de qualquer uma reservar — cada uma via o pendente inteiro,
     recortava as 200 primeiras e disparava seu próprio laço SEQUENCIAL, mas os dois laços
     corriam em PARALELO um do outro: duas rodadas de chamadas ao Bling ao mesmo tempo pra
     mesma conta, exatamente o cenário de 429 que o laço sequencial existe pra evitar. Trava de
     processo, só pra este lote — não é a `trava-pesada` (essa é global entre as rotinas
     agendadas das três empresas; usá-la aqui prenderia o custo diário atrás de um clique
     manual de admin). */
  let _loteLancandoTodas = false;
  ...
  return async function rotasContagem(req, res, urlObj, method) {
    ...
    if (method === 'POST' && p === (prefixo + '/contagem-ajustar')) {
      ...
      /* Codex #524 (P2): já aplicado no Bling é um FATO consumado — editar o número aqui muda o
         registro auditado sem tocar no Bling, e quem revisar depois vai comparar um contra o
         outro sem saber qual valeu. */
      if (l.aplicado_em) { json(res, 200, { ok: false, erro: 'este lançamento já foi aplicado no Bling em ' + l.aplicado_em + ' — não é mais editável' }); return true; }
      if (emAndamento(l)) { json(res, 200, { ok: false, erro: 'este lançamento está sendo aplicado no Bling agora — espere terminar antes de editar' }); return true; }
    ...
    if (method === 'POST' && p === (prefixo + '/contagem-excluir')) {
      ...
      /* Codex #524 (P2): lançamento JÁ APLICADO no Bling vira registro histórico. Editar a
         ...
         de descrever o que foi enviado — e o estoque real não muda junto. Congela. Excluir um
         lançamento já aplicado também esconderia da lista a evidência de um movimento real. */
      if (l.aplicado_em) {
      ...
      if (emAndamento(l)) { json(res, 200, { ok: false, erro: 'este lançamento está sendo aplicado no Bling agora — espere terminar antes de excluir' }); return true; }
    ...
    /* ── marcar TODAS as de um dia como acréscimo ─────────────────────────────
       O dono lançou ~10 itens antes de os dois modos existirem, todos querendo SOMAR, e cada
       um ficou como contagem. Trocar um a um é o que existia; isto faz de uma vez.

       ⚠️ NÃO mexe em: lançamento já APLICADO no Bling (é fato consumado), lançamento em
       aplicação, e — de propósito — em nada fora do dia pedido. Um botão que varre o arquivo
       inteiro transformaria um inventário legítimo de outro dia em acréscimos, e isso ninguém
       desfaz depois porque a intenção original se perde.
       Devolve quantas mudaram, pra ele conferir contra o que via na tela. */
    if (method === 'POST' && p === (prefixo + '/contagem-marcar-todas')) {
      const sess = validarSessao(req.headers['cookie']);
      if (!sess) { json(res, 401, { ok: false, erro: 'precisa estar logado' }); return true; }
      let body = {};
      try { body = await ctx.readBody(req); } catch (e) { body = {}; }
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(dia)) {
        json(res, 200, { ok: false, erro: 'informe o dia (AAAA-MM-DD) — sem ele isto varreria o arquivo inteiro' });
        return true;
      }
      ...
      const quem = String(sess.nome || sess || '');
      const quando = new Date().toISOString();
      let mudadas = 0, puladas = 0;
      for (const l of d.lancamentos) {
        if (!l || l.excluido) continue;
        if (diaSP(l.quando) !== dia) continue;
        if ((l.tipo || 'contagem') === novoTipo) continue;
        if (l.aplicado_em || emAndamento(l)) { puladas++; continue; }
        ...
        l.tipo_trocas.push({ de: antes, para: novoTipo, quem, quando, em_lote: true });
        mudadas++;
      }

      if (mudadas && !gravar(d)) {
        json(res, 200, { ok: false, erro: 'não consegui salvar — nada foi alterado' });
        return true;
      }
      json(res, 200, { ok: true, mudadas, puladas });
      return true;
    }
    ...
    if (method === 'POST' && p === (prefixo + '/contagem-tipo')) {
      ...
      if (emAndamento(l)) { json(res, 200, { ok: false, erro: 'este lançamento está sendo aplicado no Bling agora — espere terminar antes de trocar o tipo' }); return true; }
    ...
    /* ── quais são os depósitos desta empresa ────────────────────────────────
       ...
       Codex #524 (P2): expõe a mesma metadata privilegiada (ids/nomes de depósito) da descoberta
       de ids, que lá exige chave de admin — aqui exigia só sessão válida. Mesma trava explícita
       da rota de aplicar. */
    if (method === 'GET' && p === (prefixo + '/contagem-depositos')) {
      const sess = validarSessao(req.headers['cookie']);
      if (!sess) { json(res, 401, { ok: false, erro: 'precisa estar logado' }); return true; }
      if (!ehAdminExplicito(sess)) {
        json(res, 200, { ok: false, erro: 'só o responsável pode consultar os depósitos do Bling' });
        return true;
      }
    ...
    /* ── APLICAR no estoque do Bling ──────────────────────────────────────────
       ...
       usarem, muda tudo. */
  /* 24/09 — O MIOLO DE APLICAR, EXTRAÍDO. Existe porque agora há DOIS caminhos que escrevem no
     estoque (um item, ou todos do dia), e travas duplicadas divergem: uma delas deixaria passar
     o que a outra recusa, e o erro só apareceria como estoque errado no Bling — sem desfazer.
     Uma função, as mesmas recusas, os dois caminhos. */
  /* `reservaDoLote`: quando o lote já reservou a fila inteira, a reserva dele NÃO é
     concorrência alheia — sem isto a própria `aplicarUm` recusaria cada item com "já há um
     lançamento deste item em andamento". */
  async function aplicarUm(id, sess, dep, reservaDoLote) {
      let d;
    try { d = ler(); } catch (e) {
      return { ok: false, erro: 'o arquivo de contagens está ilegível — avise o responsável' };
    }
    if (carimbarIds(d)) gravar(d);
    const l = d.lancamentos.find(x => x && x.id === id && !x.excluido);
    ...
    if (l.aplicado_em) {
      return { ok: false, ja: true, erro: 'este lançamento já foi aplicado no Bling em ' + l.aplicado_em + ' — aplicar de novo somaria em dobro' };
    }
    if (l.tipo !== 'somar') {
      return { ok: false, erro: 'só lançamento do tipo SOMAR entra no estoque: uma CONTAGEM diz quanto TEM, e somar esse número duplicaria o saldo' };
    }
    /* trava de concorrência: dois cliques quase juntos não podem virar duas entradas */
    if (l.aplicando_em && l.aplicando_em !== reservaDoLote && (Date.now() - Date.parse(l.aplicando_em)) < 120000) {
      return { ok: false, erro: 'já há um lançamento deste item em andamento — espere terminar' };
    }
    l.aplicando_em = new Date().toISOString();
    if (!gravar(d)) { return { ok: false, erro: 'não consegui reservar o lançamento — nada foi enviado ao Bling' }; }

    const lib = _estoqueEntrada();
    const r = await lib.entrada({
      sku: l.sku,
      quantidade: Number(l.contado),
      /* Codex #524 (P2): `quando` é ISO/UTC; a observação é lida em SP junto com o resto da
         tela — fatiar o ISO cru rotulava lançamentos das 21h-23h59 de SP com o dia seguinte. */
      observacao: 'Contagem ' + diaSP(l.quando) + ' por ' + String(l.quem || ''),
      depositoId: dep,
    });
    ...
    if (l2) {
      ...
      if (r.ok) {
        ...
        /* Codex #524 (P2): uma falha anterior ficava registrada pra sempre — depois de um
           retry bem-sucedido a lista mostrava "✅ no Bling" E "⚠ falhou ao lançar" juntos,
           pro mesmo lançamento, sem dizer qual valia. */
        delete l2.ultima_falha;
        delete l2.ultima_falha_em;
        delete l2.ultima_falha_ambigua;
      } else {
        l2.ultima_falha = String(r.erro || '').slice(0, 300);
        l2.ultima_falha_em = new Date().toISOString();
        /* Codex #526 (P1): marca SEPARADA pra escrita ambígua (timeout ou sem resposta no
           POST). `ultima_falha` é só texto pra ler; o lote precisa DECIDIR com base nisso, e
           decidir por texto é o caminho pra reapresentar um item que talvez já tenha entrado. */
        if (r.ambiguo) l2.ultima_falha_ambigua = true; else delete l2.ultima_falha_ambigua;
      }
      marcado = gravar(d);
    }
    /* Codex #524 (P1): o Bling já aceitou a entrada — não dá pra "recusar" isso agora. Mas se
       o marcador local (`aplicado_em`) não persistiu, nada impede o mesmo lançamento de ser
       aplicado de novo daqui a pouco (ou depois de um restart) e duplicar o estoque de
       verdade. Reporta com um aviso explícito em vez de um `ok:true` mudo, pra alguém
       confirmar manualmente antes de qualquer novo clique. */
    if (r.ok && !marcado) {
      return {
        ok: true, link: r.link, quantidade: r.quantidade, custo: r.custo,
        aviso: 'aplicado no Bling, mas não consegui gravar o marcador local — avise o responsável e NÃO aplique este lançamento de novo antes de conferir',
      };
    }
    return r.ok ? { ok: true, link: r.link, quantidade: r.quantidade, custo: r.custo } : r;
  }

    /* ── lançar TODOS os acréscimos do dia, de uma vez ───────────────────────
       O dono: "esses eu lancei de manhã quando eu achava que já tava certo... só pra eu não
       perder tempo de novo". Dez cliques no 📦 é o que existia.

       ⚠️ São N ESCRITAS IRREVERSÍVEIS seguidas. Por isso NÃO tem trava própria: chama a mesma
       `aplicarUm` do botão individual, uma por vez. Duplicar as recusas aqui seria o caminho
       mais curto pra uma delas divergir e deixar passar o que a outra recusa — e o erro só
       apareceria como estoque errado no Bling.
       Sequencial de propósito: a cota é da conta, e em paralelo o Bling recusa por limite. */
    if (method === 'POST' && p === (prefixo + '/contagem-lancar-todas')) {
      const sess = validarSessao(req.headers['cookie']);
      if (!sess) { json(res, 401, { ok: false, erro: 'precisa estar logado' }); return true; }
      if (!ehAdminExplicito(sess)) {
      ...
      if (!dep) {
        json(res, 200, { ok: false, erro: 'depósito desta empresa não configurado — veja /contagem-depositos antes de lançar' });
        return true;
      }

      /* Codex #526 (P2, r3): serializa o LOTE em si — ver comentário da declaração de
         `_loteLancandoTodas` acima. Recusa em vez de esperar: esperar formaria fila de
         requisições HTTP penduradas, e quem clicou de novo não tem como saber que já tem um
         lote seu rodando (a tela não mostra "em andamento" entre um clique e outro). */
      if (_loteLancandoTodas) {
        json(res, 200, { ok: false, erro: 'já há um lote de lançamento em andamento — espere terminar antes de clicar de novo' });
        return true;
      }
      _loteLancandoTodas = true;
      try {
        let d;
        try { d = ler(); } catch (e) {
          json(res, 200, { ok: false, erro: 'o arquivo de contagens está ilegível — avise o responsável' });
        ...
        /* a lista é montada AGORA e não muda: aplicarUm relê o arquivo a cada item, então
           trabalhar sobre uma lista viva faria o mesmo item entrar duas vezes. */
        const fila = d.lancamentos
          .filter(l => l && !l.excluido && l.id && l.tipo === 'somar' && !l.aplicado_em && !emAndamento(l))
        /* Codex #526 (P1): quando um POST perde a resposta, a `aplicarUm` grava a falha e NÃO
           marca `aplicado_em` — mas o Bling PODE ter processado. Sem esta linha, a rodada
           seguinte pegaria o mesmo item e somaria DE NOVO no estoque real.
           Escrita ambígua sai da fila automática: quem decide relançar é o dono, item a item,
           depois de conferir no Bling. O 📦 individual continua disponível pra isso. */
        .filter(l => !l.ultima_falha_ambigua)
          .filter(l => diaSP(l.quando) === dia)
          .map(l => ({ id: l.id, sku: l.sku, qtd: Number(l.contado) }));

        /* Codex #526 (P1): a tela manda a FOTO do que confirmou (ids e quantidades). Se o que
         está no arquivo agora não bate — outra pessoa criou, editou ou o dono mexeu numa linha
         depois de carregar a página —, RECUSA em vez de lançar algo diferente do que ele viu.
         Numa escrita sem desfazer, divergir do confirmado é o erro mais caro possível. */
      /* 24/09 — A FOTO AGORA ESCOLHE, não só confere. Antes ela era só validação (o servidor
         montava a fila do dia inteiro e recusava se divergisse); com as caixinhas de seleção,
         ela passa a DEFINIR quais vão. As duas coisas na mesma estrutura de propósito: a
         quantidade enviada continua sendo comparada com a que ele viu na tela, item a item —
         escolher 3 de 10 não pode abrir mão da conferência que evita lançar número diferente
         do confirmado, numa escrita sem desfazer. */
      const foto = Array.isArray(body.itens) ? body.itens : null;
      /* Codex #527 (P2): "sem marcar nada, vão todos" é uma promessa sobre a fila INTEIRA, não
         só sobre a foto — sem o modo, um item elegível que surgiu depois que a tela carregou
         (alguém criou ou converteu um novo acréscimo) não estava na foto e simplesmente não
         entrava em `escolhidos`, sem aviso: "todos" saía incompleto e reportava sucesso. */
      const modoSelecao = body.modo === 'selecao';
      if (foto) {
        if (!foto.length) { json(res, 200, { ok: false, erro: 'nenhum item selecionado' }); return true; }
        const elegiveis = new Map(fila.map(f => [String(f.id), f]));
        /* Codex #527 (P1): a foto é `body.itens`, controlado pelo cliente — sem isto, um id
           duplicado virava DUAS entradas na fila. A primeira recusaria a segunda só se
           `aplicado_em` já tivesse persistido; numa falha ambígua (Bling aceitou, marcador
           local não gravou) `aplicado_em` continua vazio e a segunda ocorrência repetia a
           MESMA escrita irreversível antes de alguém conferir o aviso. */
        const vistos = new Set();
        const problemas = [];
        const escolhidos = [];
        for (const i2 of foto) {
          const id2 = String(i2.id || '');
          if (vistos.has(id2)) continue;
          vistos.add(id2);
          const f = elegiveis.get(id2);
          if (!f) { problemas.push('um item selecionado já não está disponível pra lançar'); continue; }
          if (Number(i2.qtd) !== f.qtd) { problemas.push(f.sku + ': a quantidade mudou (você viu ' + Number(i2.qtd) + ', agora é ' + f.qtd + ')'); continue; }
          escolhidos.push(f);
        }
        if (!modoSelecao) {
          const vistosFila = new Set(escolhidos.map(f => String(f.id)));
          const sobrando = fila.filter(f => !vistosFila.has(String(f.id)));
          if (sobrando.length) problemas.push(sobrando.length + ' novo(s) acréscimo(s) pronto(s) que a tela ainda não mostrava');
        }
        if (problemas.length) {
          json(res, 200, { ok: false, desatualizado: true,
            erro: 'a lista mudou desde que você abriu a tela — nada foi lançado:\n· ' + [...new Set(problemas)].join('\n· ') +
                  '\nAtualize a página e confira antes de lançar.' });
          return true;
        }
        fila.length = 0;
        fila.push(...escolhidos);
      }

      if (!fila.length) { json(res, 200, { ok: true, feitos: 0, falhas: 0, itens: [] }); return true; }
        /* Codex #526 (P2): eu recusava acima de 200 mandando "lance por partes", mas não havia
           COMO lançar por partes — a rota sempre remontava a fila inteira e recusava de novo.
           Agora ela própria fatia: pega as 200 primeiras e informa quantas ficaram, pra a tela
           oferecer a rodada seguinte. */
        const restantes = Math.max(0, fila.length - 200);
        if (restantes) fila.length = 200;

        /* Codex #526 (P1): só o item DA VEZ recebia `aplicando_em`. Os outros da fila ficavam
           editáveis enquanto as chamadas ao Bling rodavam — e a `aplicarUm` relê o registro, então
           mexer no + / − de uma linha ainda não enviada mandava pro Bling um número DIFERENTE do
           que ele confirmou na tela. Reserva a fila inteira antes de começar. */
        const marca = new Date().toISOString();
        for (const l of d.lancamentos) {
          if (fila.some(f => f.id === l.id)) l.aplicando_em = marca;
        }
        if (!gravar(d)) {
          json(res, 200, { ok: false, erro: 'não consegui reservar a fila — nada foi enviado ao Bling' });
          return true;
        }

        const itens = [];
        let feitos = 0, falhas = 0, seguidas = 0;
        /* Codex #526 (P1, r2): a fila inteira ganha UM carimbo (`marca`), mas o TTL de 120s do
           `emAndamento` foi pensado pra detectar um APLICAR ÚNICO que caiu no meio — não pra
           cobrir um lote de até 200 itens, cada um com chamadas ao Bling que podem levar segundos
           (mais ainda sob 429). Passados 120s do início do lote, os itens que AINDA esperam a vez
           voltavam a aparecer como "não em andamento" pras rotas de edição (ajustar, definir,
           excluir, trocar tipo) — reabrindo a MESMA janela que a reserva upfront existe pra
           fechar. Renova o carimbo do resto da fila a cada item: a folga entre um item e o
           seguinte nunca chega perto do TTL, então o lock nunca esfria enquanto o lote avança. */
        let marcaAtual = marca;
        function renovarReserva() {
          try {
            const dh = ler();
            const agora = new Date().toISOString();
            let mexeu = false;
            for (const l of dh.lancamentos) {
              if (l && l.aplicando_em === marcaAtual && !l.aplicado_em) { l.aplicando_em = agora; mexeu = true; }
            }
            if (mexeu && gravar(dh)) marcaAtual = agora;
          } catch (e) {}
        }
        for (const f of fila) {
          renovarReserva();
          /* Codex #526 (P1, r4): a renovação de cima só roda ENTRE itens — mas cada etapa da
             `entrada` agora tem teto de 45s (comTeto), e um item pode encadear até três
             (busca, cadastro, POST): 135s possíveis num item só, mais que o TTL de 120s. Nesse
             intervalo, os OUTROS itens ainda na fila ficavam com o carimbo velho, `emAndamento`
             os via como livres, e as rotas de editar/excluir aceitavam mexer numa linha que o
             lote ainda ia processar — mandando pro Bling um número diferente do confirmado.
             Renova também DURANTE o item em andamento, não só antes dele. */
          const batendo = setInterval(renovarReserva, 60000);
          let r;
          try {
            r = await aplicarUm(f.id, sess, dep, marcaAtual);
          } finally {
            clearInterval(batendo);
          }
          /* o `id` vai junto (Codex #527, P1): é como a tela sabe quais das selecionadas
             foram DE FATO tentadas nesta rodada, pra não perder de vista o resto quando o
             lote for maior que 200 e `restantes` mandar clicar de novo. */
          itens.push({ id: f.id, sku: f.sku, qtd: f.qtd, ok: !!(r && r.ok), erro: (r && r.erro) || '', aviso: (r && r.aviso) || '' });
          if (r && r.ok) { feitos++; seguidas = 0; }
          else {
            falhas++;
            /* Codex #526 (P2): eu parava em 3 falhas SEGUIDAS supondo problema geral, mas as
               recusas mais comuns são DO ITEM (kit, sem custo, sem SKU, quantidade inválida) —
               três kits em sequência abortavam todos os válidos que vinham depois, e a nova
               tentativa parava no mesmo lugar. Agora só falha GERAL interrompe.
               Codex #526 (P1, r3): "não recebi confirmação do Bling (falha de rede)" — de
               estoque-entrada.js — NÃO batia aqui. É o pior caso possível: o Bling pode já ter
               recebido a escrita, só a confirmação que se perdeu, e sem contar como geral o
               laço seguia mandando as próximas 197 escritas sobre a MESMA conexão ruim. */
            /* Codex #526 (P1, r4): o texto sozinho não cobre tudo — o teto de 45s por etapa
               (`comTeto`, em estoque-entrada.js) devolve "a resposta do Bling não chegou a
               tempo" e "não obtive resposta do Bling", que não batem em nenhuma palavra do
               regex. Confiar só no texto deixava a rodada seguir mandando as próximas 197
               escritas sobre a MESMA conexão travada. `r.ambiguo` é a marca estruturada que
               cobre qualquer caminho ambíguo, escrito hoje ou amanhã. */
            const geral = (r && r.ambiguo) || /não consegui consultar|HTTP 401|HTTP 403|HTTP 429|depósito|token|falha de rede/i.test(String((r && r.erro) || ''));
            seguidas = geral ? seguidas + 1 : 0;
            if (seguidas >= 3) { itens.push({ parou: true, erro: 'parei depois de 3 falhas de conexão/permissão seguidas — parece problema geral, não dos itens' }); break; }
          }
          await new Promise(s2 => setTimeout(s2, 400));   // respiro pra cota do Bling
        }
        /* libera a reserva de quem não chegou a ser processado (parada por falha geral) */
        try {
          const d2 = ler();
          let mexeu = false;
          for (const l of d2.lancamentos) {
            if (l && l.aplicando_em === marcaAtual && !l.aplicado_em) { delete l.aplicando_em; mexeu = true; }
          }
          if (mexeu) gravar(d2);
        } catch (e) {}
        json(res, 200, { ok: true, feitos, falhas, total: fila.length, restantes, itens });
        return true;
      } finally {
        _loteLancandoTodas = false;
      }
    }
    ...
    if (method === 'POST' && p === (prefixo + '/contagem-aplicar')) {
      ...
      /* Codex #527 (P2): o lote reserva a fila inteira e serializa as chamadas ao Bling pra
         evitar tráfego paralelo (ver `_loteLancandoTodas` acima) — mas este lançamento
         individual não olhava o mesmo trava. Enquanto um lote roda, outra linha (não
         reservada por ele) ainda podia ser lançada por aqui ao mesmo tempo, reabrindo o
         paralelismo que a trava existe pra impedir. */
      if (_loteLancandoTodas) {
        json(res, 200, { ok: false, erro: 'há um lote de lançamento em andamento — espere terminar antes de lançar individualmente' });
        return true;
      }
      let body = {};
      try { body = await ctx.readBody(req); } catch (e) { body = {}; }
      ...
      json(res, 200, await aplicarUm(id, sess, dep));
      return true;
    }
    ...
    if (method === 'GET' && p === (prefixo + '/contagem-lista')) {
      ...
      /* Codex #509 (P2): o carimbo `quando` é ISO (UTC) e o `de` chega como dia de SÃO PAULO.
         ...
         (helper `diaSP` definido no topo do módulo). */
      ...
      /* 24/09 — o ID DO PRODUTO NO BLING, pra a tela poder linkar o SKU direto pra tela de
         estoque (estoque.php?buscaid=ID). O dono pediu pra conferir sem procurar no Bling.
         Vem do ÍNDICE LOCAL, que já tem id por produto: zero chamada ao Bling, e a cota é da
         conta. Lançamento já aplicado traz o seu próprio (`bling_produto_id`), que é o mais
         confiável — foi com ele que a entrada foi feita.
         Sem id conhecido, o campo fica vazio e a tela mostra o SKU como texto: link que abre a
         tela errada é pior que link nenhum. */
      const idx = lerIndiceEan() || {};
      const idPorSku = Object.create(null);   // Codex #528 (P2): SKU tipo "constructor"/"__proto__" não pode
                                               // resolver pra propriedade herdada de {}
      for (const chave of Object.keys(idx)) {
        const it = idx[chave];
        if (it && it.sku && it.id && !idPorSku[it.sku]) idPorSku[it.sku] = it.id;
      }
      const comId = pagina.map(l => (l && !l.bling_produto_id && idPorSku[l.sku])
        ? Object.assign({}, l, { bling_produto_id: idPorSku[l.sku] })
        : l);
