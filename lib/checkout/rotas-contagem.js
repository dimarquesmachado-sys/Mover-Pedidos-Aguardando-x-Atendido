    ...
    if (method === 'POST' && p === (prefixo + '/contagem-lancar-todas')) {
      ...
      if (!fila.length) { json(res, 200, { ok: true, feitos: 0, falhas: 0, itens: [] }); return true; }
        ...
        for (const f of fila) {
          ...
          const r = await aplicarUm(f.id, sess, dep, marcaAtual);
          ...
          if (r && r.ok) { feitos++; seguidas = 0; }
            ...
            const geral = /não consegui consultar|HTTP 401|HTTP 403|HTTP 429|depósito|token|falha de rede/i.test(String((r && r.erro) || ''));
