...
function criar(ctx) {
  ...
  async function acharProduto(sku) {
    ...
    const r = await ctx.blingGet('/produtos?codigo=' + encodeURIComponent(alvo) + '&limite=5');
  ...
  async function podeReceberEntrada(produtoId) {
    ...
    if (!det) return { pode: true, sabido: false };        // não bloqueia por falha de rede
  ...
  async function listarDepositos() {
    ...
    const l = (r && r.ok && r.data && r.data.data) || [];
  ...
  async function entrada({ sku, quantidade, observacao, depositoId }) {
    ...
    const r = await ctx.blingWrite('POST', '/estoques', corpo);
    if (!r || !r.ok) {
