...
function criarFaseDireta(cfg) {
  for (const nome of ['empresa', 'mlTokenManager', 'shopeeKey', 'shopeeUrlEnv', 'adminKey', 'porta']) {
    if (cfg == null || cfg[nome] == null) throw new Error('lib/checkout/fase-direta: falta ' + nome);
  }
  const { log = console.log } = cfg;
    ...
    const MAG_EMPRESA = magEmpresa;
      ...
      try {
        ...
        if (SH_KEYd) {
          ...
          const rSd = await fetch(SH_URLd + '/' + (cfg.empresa) + '/interno/pedidos-do-dia?horas=168&k=' + encodeURIComponent(SH_KEYd), { timeout: 90000 });   // b38: 168h = teto novo do serviço, cobre a retenção de 6 dias (Codex PR#14)
          if (!rSd.ok) {
