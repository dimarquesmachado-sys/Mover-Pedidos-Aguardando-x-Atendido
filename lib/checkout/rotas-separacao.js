...
function criar(cfg) {
  ...
  return async function handle(req, res, urlObj, method) {
    ...
    if (method === 'POST' && p === (prefixo + '/salvar-localizacao')) {
      ...
      const item = busca.data && busca.data.data && busca.data.data[0];
      if (!item || !item.id) { json(res, 200, { ok: false, erro: 'produto não encontrado p/ SKU ' + sku + ' (o Bling respondeu, e não há produto com esse código)' }); return true; }
