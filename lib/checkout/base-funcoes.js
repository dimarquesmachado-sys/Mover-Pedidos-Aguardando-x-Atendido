...
function criar(cfg) {
  ...
  /* 24/09 (Codex #524, P1) — `tentativas` OPCIONAL, padrão 3 como sempre foi. Existe porque a
   repetição automática é errada pra POST que NÃO é idempotente: a entrada de estoque, por
   exemplo. Se o Bling grava e a conexão cai antes da resposta, repetir soma em dobro no estoque
   real. Quem chama sabe se pode repetir; esta função não. */
async function blingWrite(method, pathUrl, body, opcoes) {
  const _tentativas = Math.max(1, Number((opcoes && opcoes.tentativas) != null ? opcoes.tentativas : 3) || 3);
  let token;
  try { token = await garantirToken(); }
  catch (e) { return { ok: false, status: 401, data: null, erro: 'token: ' + e.message }; }
  for (let t = 0; t < _tentativas; t++) {
