...
/* ─────────────────────────────────────────────────────────────────────────────
   HISTÓRICO DE VENDAS — código único das empresas (14/09/2026).

   Fase 3, passo 3. E este não é só desduplicação: **a GOOD não tem histórico
   nenhum**, e é por isso que ela não tem dashboard de vendas — o que o dono citou
   como objetivo. A peça que falta lá é exatamente a que estava duplicada nas
   outras duas.

   Medido sem comentários, a diferença real era pequena: 17 linhas só na AMB e 39
   só na Girassol — e as 39 eram a própria parametrização que ela já tinha feito em
   25/08, depois de um bug em que outra empresa lia o cache, o admin e o cliente
   Bling DA GIRASSOL. Esse desenho virou este arquivo.

   Tudo vem do ctx, sem cair em módulo local: dentro da lib, um `|| ctx.X` faria
   toda empresa herdar silenciosamente os dados da Girassol — que foi exatamente o
   bug de agosto.
   ──────────────────────────────────────────────────────────────────────────── */

function rotasHistorico(ctx) {
  /* a lista saiu do LINT de órfãos: cada nome é um que a extração deixaria solto e que só
     apareceria na primeira chamada, em produção. `base` não entra — dentro da lib ele faria
     toda empresa herdar os dados da Girassol, que foi o bug de agosto. */
  for (const n of ['CACHE_DIR', 'CONFERIDOS_FILE', 'readJson', 'writeJson', 'ehAdmin', 'blingGet',
                   'json', 'sleep', 'supaCfg', 'validarSessao', 'pecas', 'PAUSA_MS']) {
    if (ctx == null || ctx[n] == null) throw new Error('lib/checkout/historico: falta ' + n + ' no ctx');
  }
  ...
  /* Codex #436 (P1): a coluna credito_ml (bônus de envio Flex) só existe no vendas_historico
     da AMB — selecioná-la pra Girassol/GOOD faria o PostgREST responder 400. A AMB somava esse
     crédito na margem antes da extração; a unificação tinha derrubado isso em silêncio. */
  const TEM_CREDITO_ML = EMPRESA === 'amb';
  ...
  /* 14/09: estas duas linhas escaparam com require('./nf') e require('./ciclo') — dentro da
     lib isso aponta pra lib/checkout/, não pra pasta da empresa, e o boot quebrou na hora.
     Vêm das peças injetadas, como todo o resto. */
  const nfDoPedido      = ctx.nfDoPedido    || ctx.pecas.nf.nfDoPedido;
  const detalhePedido   = ctx.detalhePedido || ctx.pecas.ciclo.detalhePedido;
  ...
  const PAUSA_MS        = ctx.PAUSA_MS;   // ritmo da API é por empresa (exigido no ctx, cada fachada injeta o base.PAUSA_MS da própria pasta)
  ...
  return async function handleHistorico(req, res, urlObj) {
    ...
    if (method === 'GET' && p === R('historico-longo')) {
      ...
      const campos = 'numero_pedido,numero_loja,canal,data_venda,sku,descricao,quantidade,valor_produto,valor_nota,custo,comissao,frete_vendedor,imposto,margem,uf'   // 17/08: sem isto o historico-longo lia uf vazio em TODA linha (havia dois `campos` no arquivo e o uf tinha entrado no outro);
        + (TEM_CREDITO_ML ? ',credito_ml' : '');   // Codex #436 (P1): só a AMB tem a coluna — ver TEM_CREDITO_ML
      ...
      try {
        while (offset < 60000) {
          ...
          for (const l of linhas) {
            ...
            if (mg != null) T.mar += mg;
            // Codex #436 (P1): bônus de envio Flex (gravado na 1ª linha do pedido) entra na margem agregada — só a AMB tem a coluna
            const crdL = TEM_CREDITO_ML ? (Number(l.credito_ml) || 0) : 0;
            if (crdL) T.mar += crdL;
            ...
            porCanal[cn].fat += vn; porCanal[cn].un += q; porCanal[cn].mar += (mg || 0) + crdL;
    ...
    if (method === 'GET' && p === R('buscar-lucro')) {
      ...
      for (const l of linhasB) {
        ...
        // Codex #436 (P1): bônus de envio Flex entra na M.C. igual ao historico-longo — só a AMB tem a coluna (ver TEM_CREDITO_ML)
        if (TEM_CREDITO_ML) g.credito = r2c(g.credito + (Number(l.credito_ml) || 0));
