...
/* ─────────────────────────────────────────────────────────────────────────────
   HISTÓRICO DE VENDAS DO CHECKOUT — as empresas (14/09/2026).

   Passo 3 da Fase 3. Vale dizer o que isto destrava: a GOOD NÃO TINHA histórico
   nenhum — por isso não tem dashboard de vendas —, e a peça que falta lá é
   exatamente a que estava duplicada na AMB e na Girassol.

   O trabalho pesado já estava feito: em 25/08 a Girassol foi inteira parametrizada
   por empresa (rotas por prefixo, Supabase por filtro, dependências por contexto),
   depois de um bug em que outra empresa lia CACHE_DIR, CONFERIDOS_FILE, o admin e
   o cliente Bling DA GIRASSOL. A AMB tinha a mesma lógica com tudo cravado. Aqui
   as duas passam a usar a versão parametrizada.
   ──────────────────────────────────────────────────────────────────────────── */
...
const { lerChaveAdmin } = require('../http/chave-admin')  /* 14/09: o caminho encurtou ao sair de <empresa>/ pra lib/checkout/ */;
// ════════════════════════════════════════════════════════════════════════
...
function rotasHistorico(ctx) {
  /* 14/09 — na lib NÃO existe fallback pro módulo local. Na pasta da Girassol o `|| base.X`
     era rede de segurança pra chamador antigo; aqui ele seria a porta pra uma empresa herdar
     em silêncio o cache, o admin e o cliente Bling de OUTRA — que foi exatamente o bug do
     Codex #197 registrado neste arquivo. Falta de dependência derruba no boot. */
  for (const n of ['empresa', 'modulo', 'validarSessao', 'supaCfg', 'CACHE_DIR', 'CONFERIDOS_FILE',
                   'readJson', 'writeJson', 'ehAdmin', 'blingGet', 'nfDoPedido', 'detalhePedido',
                   'sleep', 'json', 'histCache', 'PAUSA_MS']) {
    if (!ctx || ctx[n] == null) throw new Error('lib/checkout/historico: falta ' + n);
  }
  const { sleep, json } = ctx;
  ...
  const EMPRESA = String(ctx.empresa);
  const MODULO  = String(ctx.modulo);
  /* dependências DA EMPRESA — ctx manda; sem ctx, cai no módulo local (Girassol) */
  const CACHE_DIR = ctx.CACHE_DIR;
  const CONFERIDOS_FILE = ctx.CONFERIDOS_FILE;
  const readJson = ctx.readJson;
  const writeJson = ctx.writeJson;
  const ehAdmin = ctx.ehAdmin;
  const blingGet = ctx.blingGet;
  const nfDoPedido      = ctx.nfDoPedido;
  const detalhePedido   = ctx.detalhePedido;
  ...
  /* Codex #225 r2 (P1): o completar do frete Magalu virou FÁBRICA, usada pelo /historico-longo
     (linha a linha) e pelo /historico-linhas (pedido agrupado) — os cards e a lista têm que
     mostrar o MESMO número (o padrão do custo manual, de novo). Regras:
     · banco e tabela dão o frete do PACOTE (o magaluFreteSkuGravar grava o freteCopart do pedido
       inteiro no 1º SKU) → o pedido leva UM frete, de UMA fonte — a 1ª linha que resolver
       (banco: média×qtd, o formato de sempre; senão tabela: valor do pacote, sem ×qtd);
     · SKU renomeado resolve pela mesma cadeia do custo manual (_sku-depara.json);
     · memo por SKU + orçamento (8 consultas / 6s) por requisição — o resto entra nas próximas. */
  const _freteMagaluFabrica = () => {
    const banco = (() => { try { return readJson(path.join(CACHE_DIR, '_magalu_frete_sku.json'), {}) || {}; } catch (e) { return {}; } })();
    ...
    const dpIdx = {};
    for (const kD of Object.keys(depara)) { const kU = String(kD).toUpperCase(); if (dpIdx[kU] === undefined) dpIdx[kU] = depara[kD]; }
    const resolve = (sku) => {
      let a = String(sku || '').trim();
      const vistos = new Set([a.toUpperCase()]);
      for (let i = 0; i < 5; i++) {
        const r = dpIdx[a.toUpperCase()];
    ...
    const _bancoDe = (s0, s) => {
      const b = bancoIdx[String(s0 || '').toUpperCase()] || bancoIdx[String(s || '').toUpperCase()];
      const m = b && Number(b.media);
      /* Codex r4 (P1): o banco guarda o freteCopart do PEDIDO INTEIRO (magaluFreteSkuGravar) —
         a média é do PACOTE, então SEM ×qtd. (Até a r3 multiplicava, herdado do completar de
         20/08; pedido de 2 unidades pagava dois pacotes.) */
      return (m > 0) ? Math.round(m * 100) / 100 : null;
    };
    /* Decisão por PEDIDO, determinística (Codex r4 P1): /historico-longo e /buscar-lucro leem
       sku.asc e /historico-linhas lê sku.desc — "a 1ª linha que resolver" escolhia fontes
       diferentes pro MESMO pedido. Agora os itens são ordenados por SKU aqui dentro e o banco de
       QUALQUER item vence a tabela; a resposta diz qual SKU venceu (pra margem descontar no lugar
       certo). UM frete por pedido, do pacote. */
    const porPedido = async (canal, itens, pedido) => {
      const ck = String(canal || '').toLowerCase();
      if (ck !== 'magalu') return null;          // só a Magalu tem previsão de frete hoje
    ...
    return { porPedido, pendentes: () => pendentes };
  };
  /* o ritmo da API é por empresa (cota do Bling é da conta) e não tem fallback: herdar o
     ritmo de outra empresa é o caminho pra estourar a cota dela. */
  const PAUSA_MS        = ctx.PAUSA_MS;
  ...
  return async function handleHistorico(req, res, urlObj) {
    ...
    if (method === 'GET' && p === R('historico-linhas')) {
      ...
      for (const l of linhas) {
        ...
        /* 15/09 — a `descricao` era PEDIDA ao banco (está em `campos`) e jogada fora aqui: o
           item ia só com sku e qtd, e o front lê `i.descricao` pra mostrar o nome. Resultado:
           no período longo (mês anterior, 30 dias, ano) o dashboard mostrava só o código do
           produto, enquanto hoje/ontem/7 dias — que vêm do cache, não do banco — mostravam o
           nome. A rota irmã `historico-longo` já montava certo (linha ~1008); esta não.
           Buscar o dado e descartar é o pior dos dois mundos: paga a consulta e não entrega. */
        o.itens.push({ sku: l.sku || '', descricao: l.descricao || '', qtd: q });
      ...
      /* Codex #225 r2 (P1): a LISTA tem que mostrar o mesmo frete que o agregado completa — senão
         ...
         pelo índice): a 1ª linha que resolver leva o frete; entra no frete e SAI da margem, como lá. */
      const _freteMgL = _freteMagaluFabrica();
    ...
    if (method === 'GET' && p === R('historico-longo')) {
      ...
      /* 18/09 — a tela precisa saber DE ONDE veio a alíquota de cada mês, não só o imposto
         final. O dono olhou o card "15% do faturamento" logo depois de eu carregar as
         alíquotas apuradas da GOOD e teve que perguntar se elas tinham pegado — tinham, mas
         agosto não está na lista dele e caiu no padrão, que é o comportamento certo.
         Card que não diz a origem do número obriga a perguntar; e perguntar toda vez é o
         mesmo custo de não ter o número. */
      const _aliqOrigem = {};   // '2026-08' → { valor, fonte: 'painel'|'apurada'|'padrao' }
      const _aliqL = mes => {
        const a = _cfgL.aliquotas && _cfgL.aliquotas[mes];
        /* 19/08: mês salvo como 0% era campo em BRANCO gravado por engano — zero não é
           alíquota, cai no padrão. */
        if (a != null && isFinite(Number(a)) && Number(a) > 0) {
          _aliqOrigem[mes] = { valor: Number(a), fonte: 'painel' };
          return Number(a);
        }
        if (DEFAULT_ALIQ_BK && DEFAULT_ALIQ_BK[mes] != null) {
          _aliqOrigem[mes] = { valor: Number(DEFAULT_ALIQ_BK[mes]), fonte: 'apurada' };
          return Number(DEFAULT_ALIQ_BK[mes]);
        }
        /* sem alíquota conhecida: quem chama aplica o próprio padrão, e a tela precisa poder
           DIZER isso — é a diferença entre "o imposto é 15%" e "não sei o imposto deste mês". */
        _aliqOrigem[mes] = { valor: null, fonte: 'padrao' };
        return null;
      };
      let _impRecalc = 0;
      ...
      const _freteMg = _freteMagaluFabrica();   // Codex #225 r2: mesma fábrica das duas rotas
      const _pedComFrete = new Set();   // Codex r3: pedidos magalu com frete gravado em QUALQUER linha
      const _frCands = [];              // linhas zeradas candidatas — a decisão fica pro FIM do laço
      ...
      try {
        while (offset < 60000) {
          ...
          for (const l of linhas) {
            ...
            /* Codex #225 r3 (P2): a decisão do frete previsto SAI do laço — o backfill distribui o
               frete real entre as linhas do pedido e uma linha pode arredondar a 0 tendo IRMÃ com o
               frete (que ainda nem chegou no stream). Aqui só registra; decide no fim, com o
               período inteiro conhecido. */
            let _frCand = null;
            ...
            if (_frCand) { _frCand.temMg = (mg != null); _frCand.pc = porCanal[cn]; _frCand.pd = dd ? porDia[dd] : null; _frCand.ps = porSku[sk]; _frCand.pu = porUF[kUF]; _frCands.push(_frCand); }
          }
          if (linhas.length < 1000) break;
          offset += 1000;
        }
      } catch (e) { json(res, 500, { ok: false, erro: String(e.message || e) }); return true; }
      /* Codex #225 r3: aplica o frete previsto AGORA, com o período inteiro conhecido — pedido que
         tem frete gravado em qualquer linha não ganha estimativa por cima. A fábrica segue
         garantindo UM frete por pedido, de-para e orçamento; o previsto entra no frete e SAI das
         margens exatamente onde a linha tinha somado (total, canal, dia, SKU, UF). */
      ...
      const dados = { ok: true, de: deL, ate: ateL, fonte: 'supabase', paginas, ufs, pedidos_sem_uf: pedSemUF,
        ...
        canais, dias, skus };
      /* Codex #225 r3: resultado PARCIAL (o orçamento do frete acabou com SKU na fila) não congela
         por 10 min — sem cachear, a próxima abertura avança o backlog em mais uma leva. */
      if (_freteMg.pendentes() === 0) _histCache[cacheKey] = { ts: Date.now(), dados };
      json(res, 200, dados);
      return true;
    }
    ...
    if (method === 'GET' && p === R('buscar-lucro')) {
      ...
      /* Codex #225 r3 (P1): a busca global mostra o MESMO frete que o Mês/Ano completa — pedido
         magalu sem NENHUM frete gravado ganha o previsto na 1ª linha que resolver (g.frete>0 já
         cobre a irmã com o frete do pedido, porque aqui o pedido chega inteiro e agrupado).
         Entra no frete e sai da M.C., como lá. Só os 15 exibidos — orçamento não vaza pro resto. */
      const gruposB = numsB.map(nk => porPed.get(nk)).filter(Boolean).slice(0, 15);
      ...
      if (_freteMgB.pendentes() === 0) _histCache[ckB] = { ts: Date.now(), dados: dadosB };   // parcial não congela (Codex r3)
      json(res, 200, dadosB);
      return true;
    }
