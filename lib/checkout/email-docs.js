...
/* ─────────────────────────────────────────────────────────────────────────────
   E-MAIL DE DOCUMENTOS — código único das três empresas (14/09/2026).

   Último dos arquivos espelhados grandes. As seis linhas que diferiam eram o nome
   do módulo no cabeçalho e o prefixo das envs citado nas MENSAGENS de erro — e
   essas mensagens importam: elas dizem qual env criar no Render, então apontar a
   env da empresa errada mandaria o dono configurar o lugar errado.

   Tudo o que vem da pasta da empresa (base, nf, etiquetas, danfe, fusão) entra por
   injeção: são arquivos que ainda vivem lá, e require relativo dentro da lib
   apontaria pro lugar errado.
   ──────────────────────────────────────────────────────────────────────────── */

function criar(cfg) {
  for (const n of ['tag', 'base', 'pecas']) {
    if (!cfg || cfg[n] == null) throw new Error('lib/checkout/email-docs: falta ' + n);
  }
  for (const p of ['nf', 'etiquetas', 'danfe-simplificado', 'fusao-etiqueta']) {
    if (!cfg.pecas[p]) throw new Error('lib/checkout/email-docs: falta a peça ' + p);
  }
  const { tag } = cfg;
  //  amb-checkout-offline · módulo email-docs  (extraído do index.js — Lote 1)
  // ════════════════════════════════════════════════════════════════════════
  'use strict';
  const fs    = require('fs');
  ...
  const { etiquetaPdf, zplParaPdf } = cfg.pecas['etiquetas'];
  const { dadosNFSimp, nfDoPedido } = cfg.pecas['nf'];
  const { gerarDanfeSimplificado, gerarTiraNF } = cfg.pecas['danfe-simplificado'];
  const { fundirEtiquetaComDanfe } = cfg.pecas['fusao-etiqueta'];
  ...
  if (!EMAIL_USER || !EMAIL_PASS) return { ok: false, erro: `email não configurado (faltam ${tag}_EMAIL_USER / ${tag}_EMAIL_PASS no Render)` };
  if (!EMAIL_DEST) return { ok: false, erro: `destinatário não configurado — crie a env ${tag}_EMAIL_DEST no Render (aceita vários e-mails separados por vírgula)` };
