...
function criar(cfg) {
  ...
  if (obj.tributacao) {
    /* 23/09 — A REDE VARRIA `tributacao` INTEIRA e pescava o NCM junto. NCM tem exatamente 8
       dígitos, que é o tamanho de um EAN-8, então ele entrava como se fosse código de barras.
       O dono viu na tela ("está trazendo a informação do NCM"), mas o estrago é maior: o índice
       de EAN é CHAVEADO pelo código, e vários produtos dividem o mesmo NCM — um sobrescrevia o
       outro em silêncio, e uma busca por aquele número devolvia o produto errado.
       A rede continua (existe pra pescar GTIN em campo de nome inesperado); o que sai são os
       campos que a gente SABE que não são código de barras. */
    const NAO_E_GTIN = /^(ncm|cest|origem|spedtipoitem|codigolistaservicos|codigoenquadramentoipi|percentualtributos|valorbaseicmsst|valoricmsst|grupoproduto)$/i;
    Object.entries(obj.tributacao).forEach(([k, v]) => {
      if (NAO_E_GTIN.test(String(k))) return;
      if (typeof v === 'string' && v.length >= 8) c.push(v);
    });
  }
  ...
  try {
    ...
    /* Codex #513 (P2): faltava `img` aqui — quem abrisse um resultado da busca por nome
       apagava a foto que a indexação completa tinha gravado, e o item voltava a aparecer
       "sem foto" até a próxima reindexação total. */
