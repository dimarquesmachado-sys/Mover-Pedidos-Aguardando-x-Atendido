...
/* ─────────────────────────────────────────────────────────────────────────────
   Utilidades comuns do checkout — código único das três empresas (14/09/2026).
...
function criar(cfg) {
  for (const n of ['tag', 'base']) {
    if (!cfg || cfg[n] == null) throw new Error('lib/checkout/comum: falta ' + n);
  }
  const { tag } = cfg;
  //  amb-checkout-offline · módulo comum  (extraído do index.js — Lote 1)
  // ════════════════════════════════════════════════════════════════════════
  'use strict';
  const fs    = require('fs');
  ...
  return { servicoDoPedido, ehFlex, cronDeveriaTerRodado, kitIncompletoNoCache, zplEscape, bannerVolumeZpl };
