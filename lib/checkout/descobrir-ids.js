  ...
  async function _provarCanalML(canais, sellerId) {
    ...
    for (const c of semNome) {
      for (const url of ['/orders/' + c._amostra, '/packs/' + c._amostra]) {
        ...
        if (!vendedor && d && Array.isArray(d.orders) && d.orders[0] && d.orders[0].id) {
          try {
            const r2 = await fetch('https://api.mercadolibre.com/orders/' + d.orders[0].id, { headers: { Authorization: 'Bearer ' + token } });
            if (r2.ok) {
              const d2 = await r2.json();
              vendedor = (d2 && d2.seller && d2.seller.id) || null;
              if (vendedor) _diagProva.push({ canal: c.id, url: '/packs → /orders/' + d.orders[0].id, leia: 'o vendedor veio do pedido DENTRO do pacote' });
            } else {
              _diagProva.push({ canal: c.id, url: '/orders/' + d.orders[0].id + ' (pedido de dentro do pacote)', status: r2.status });
            }
          } catch (e) { _diagProva.push({ canal: c.id, url: 'pedido de dentro do pacote', erro: String(e.message || e).slice(0, 80) }); }
        }
        ...
        if (String(vendedor) !== String(sellerId)) {
          _diagProva.push({ canal: c.id, url, vendedor_no_pedido: vendedor, nosso_seller: sellerId,
