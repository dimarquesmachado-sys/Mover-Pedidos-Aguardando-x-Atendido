...
function criar(cfg) {
  ...
  async function _provarCanalML(canais, sellerId) {
    if (!cfg.garantirTokenML || !sellerId) return null;
    let token;
    try { token = await cfg.garantirTokenML(); } catch (e) { return null; }
    ...
    for (const c of semNome) {
      for (const url of ['/orders/' + c._amostra, '/packs/' + c._amostra]) {
        let d;
        try {
          const r = await fetch('https://api.mercadolibre.com' + url, { headers: { Authorization: 'Bearer ' + token } });
          if (!r.ok) continue;
          d = await r.json();
        } catch (e) { continue; }
        const vendedor = (d && d.seller && d.seller.id) || (d && d.orders && d.orders[0] && d.orders[0].seller_id) || null;
        if (vendedor && String(vendedor) === String(sellerId)) {
          c.nome = 'Mercado Livre';
          c.obs = 'PROVADO: o pedido ' + c._amostra + ' deste canal pertence ao seller ' + sellerId + ' (' + url + ')';
          return c;
        }
      }
    }
  ...
  const FORMATOS = [
    ...
    [/^\d{6}[A-Z0-9]{8,}$/i, 'Shopee (formato do pedido)'],
    [/^LU-?\d/i, 'Magalu (formato do pedido)'],
    [/^\d{3}-\d{7}-\d{7}$/, 'Amazon (formato do pedido)'],
    [/^[0-9]{18,19}$/, 'TikTok (formato do pedido)'],
  ];
  ...
  function _sugerirEnvs(prefixo, situacoes, canais, canalMlProvado) {
    const acha = (padrao) => {
    ...
    let confira;
    if (canalMlProvado) {
      env[prefixo + 'ME_LOJA_IDS'] = String(canalMlProvado.id);
      confira = 'ME_LOJA_IDS PROVADO — ' + canalMlProvado.obs;
    } else {
    ...
    return { colar_no_render: env, confira, nao_encontrei: naoAchei };
  }
  ...
  async function descobrir() {
    ...
    let provado = null;
    if (cn && cn.ok && ml && ml.ok) provado = await _provarCanalML(cn.itens, ml.seller_id);
    if (cn && cn.ok) for (const c of cn.itens) delete c._amostra;   // ruído pro leitor
    if (cfg.prefixoEnv != null) {
      saida.sugestao = _sugerirEnvs(cfg.prefixoEnv, (st && st.ok) ? st.itens : [], (cn && cn.ok) ? cn.itens : [], provado);
    }
    return saida;
  }
