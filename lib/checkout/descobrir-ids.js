...
function criar(cfg) {
  ...
  function _sugerirEnvs(envNomes, situacoes, canais, canalMlProvado) {
    ...
    const env = {};
    /* 15/09 — o destino do pedido CONFERIDO depende da capacidade `expedicao`:
         · COM app de Expedição (só a Girassol): conferido → VERIFICADO (24); o app move pra
           DESPACHADOS depois, quando a equipe bipa na entrega à transportadora;
         · SEM Expedição (AMB e GOOD): não existe etapa seguinte, então conferido → DESPACHADOS.
       Antes disso eu sugeria VERIFICADO pra todo mundo e a conferência acusava AMB e GOOD como
       "valor diferente" — eu levei ao dono como possível engano, e era o desenho certo. */
    const mapa = [
      ['atendido', /^atendido$/i],
      ['aguardando', /^aguardando$/i],
      ['despachados', /^despachados?$/i],
      ['verificado', cfg.temExpedicao ? /^verificado$/i : /^despachados?$/i],
    ];
    const naoAchei = [];
    for (const [chave, padrao] of mapa) {
      const nome = envNomes && envNomes[chave];
      if (!nome) continue;                       // empresa que não expõe essa env
      if (chave === 'verificado' && !cfg.temExpedicao) {
        /* deixa explícito POR QUE o destino não é o VERIFICADO — senão parece erro de novo */
        naoAchei.push(undefined);   // marcador removido abaixo; o aviso vai no `confira`
        naoAchei.pop();
      }
    ...
    for (const [nome, valor] of Object.entries(env)) {
      ...
      if (ehLista(nome)) {
        ...
        /* 16/09 (P2 do Codex): a conferência tem que usar a MESMA régua do consumidor. O
           blingApi passou a RECUSAR a lista inteira quando um item é inválido (senão a
           empresa ignora em silêncio os pedidos do canal digitado errado), e aqui a checagem
           continuava frouxa: `203296034,abc` apareceria em `ok` — dizendo "está configurado"
           sobre um valor que faz o F1 e o F3 se recusarem a rodar. Conferência que diverge do
           consumidor é pior que conferência nenhuma. */
        const invalidos = lista.filter(x => !/^\d+$/.test(x));
        if (invalidos.length) {
          conferencia.diferente.push({ env: nome, no_render: atualLimpo, descoberto: String(valor),
            leia: 'a lista tem item INVÁLIDO (' + invalidos.map(x => '"' + x.slice(0, 20) + '"').join(', ') +
                  ') — o F1 e o F3 recusam a lista INTEIRA, então esta empresa não move pedido nenhum' });
          continue;
        }
    ...
    const sobreExpedicao = cfg.temExpedicao
      ? 'esta empresa TEM app de Expedição: o pedido conferido vai pra VERIFICADO e o app move pra DESPACHADOS quando a equipe bipa na entrega'
      : 'esta empresa NÃO tem app de Expedição: o pedido conferido já vai pra DESPACHADOS, porque não há etapa depois da conferência';
    return { colar_no_render: env, confira, nao_encontrei: naoAchei, conferencia, sobre_expedicao: sobreExpedicao };
  }
