...
function criar(cfg) {
  ...
  function _sugerirEnvs(envNomes, situacoes, canais, canalMlProvado) {
    ...
    const conferencia = { ok: [], diferente: [], ausente_mas_padrao_ok: [], ausente_precisa_criar: [] };
    for (const [nome, valor] of Object.entries(env)) {
      ...
      if (atual == null || String(atual).trim() === '') {
        ...
        if (padrao != null && String(padrao) === String(valor)) conferencia.ausente_mas_padrao_ok.push({ env: nome, valor, leia: 'não existe no Render, mas o padrão do código já é este valor — criar não muda nada' });
        else conferencia.ausente_precisa_criar.push({ env: nome, valor, padrao_do_codigo: padrao != null ? String(padrao) : '(desconhecido)' });
        continue;
      }
      if (String(atual).trim() === String(valor)) conferencia.ok.push({ env: nome, valor });
      else conferencia.diferente.push({ env: nome, no_render: String(atual).trim(), descoberto: String(valor), leia: 'VALOR DIFERENTE — confira qual está certo antes de mexer' });
    }
