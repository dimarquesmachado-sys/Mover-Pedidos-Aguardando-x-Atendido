  ...
  return async function handle(req, res, urlObj, method) {
    ...
    if (method === 'GET' && p === (prefixo + '/buscar-produto-nome')) {
      ...
      for (const chave of Object.keys(idx)) {
        ...
        /* 23/09 — devolve a FOTO (gravada na indexação) e deixa de devolver o "ean": o número
           que aparecia ali era NCM na maioria dos casos, porque a rede de GTIN varria a
           tributação inteira. Corrigido na origem, mas o índice já gravado ainda tem os
           errados — e mostrar um código de barras errado é pior que não mostrar nenhum.
           O que a pessoa confere contra a etiqueta é o SKU. */
        itens.push({ sku: it.sku, nome: it.nome || '', id: it.id || null, img: it.img || '' });
