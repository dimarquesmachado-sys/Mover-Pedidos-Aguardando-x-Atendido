  ...
  return async function handle(req, res, urlObj, method) {
    ...
    if (method === 'GET' && p === (prefixo + '/buscar-produto-nome')) {
      ...
      for (const chave of Object.keys(idx)) {
        ...
        /* 23/09 — ERA AQUI O BUG. Este `break` cortava em 30 ANTES da ordenação logo abaixo,
           então o que sobrava eram os 30 primeiros na ordem interna do índice — que é a ordem
           de inserção, ou seja, arbitrária. Com 200 lixas o funcionário via 30 SORTEADAS, não
           as 30 primeiras: variações que existiam no índice, com nome completo, simplesmente
           não caíam no sorteio. O dono viu "30 resultado(s)" e faltando o que ele procurava.
           Agora acumula tudo (com um teto alto só pra não estourar memória), ordena, e só
           então pagina. */
      ...
      /* 23/09 — o dono pediu TUDO de uma vez por padrão, com a opção de ver em partes.
         `limite` ausente ou 0 = manda todos os que casaram (limitados só pelo teto de memória
         lá em cima). Com `limite`, corta e informa o resto — é o modo em partes, que ele usa
         quando a lista fica grande demais pro celular do galpão. */
      const off = Math.max(0, parseInt(urlObj.searchParams.get('offset') || '0', 10) || 0);
      const lim = Math.max(0, parseInt(urlObj.searchParams.get('limite') || '0', 10) || 0);
      const pagina = lim > 0 ? itens.slice(off, off + lim) : itens.slice(off);
      json(res, 200, {
        ok: true,
        itens: pagina,
        total: itens.length,          // quantos CASARAM, não quantos vieram nesta página
        offset: off,
        tem_mais: off + pagina.length < itens.length,
