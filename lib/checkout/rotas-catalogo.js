  ...
  return async function handle(req, res, urlObj, method) {
    ...
    if (method === 'GET' && p === (prefixo + '/buscar-produto-nome')) {
      ...
      for (const chave of Object.keys(idx)) {
        ...
        /* 23/09 — ERA AQUI O BUG: um `break` cortava em 30 (depois em 2000, Codex #521 P2)
           ANTES da ordenação logo abaixo. Sobravam os N primeiros na ordem interna do índice
           — ordem de inserção, arbitrária —, não os N primeiros alfabéticos. Sem teto nenhum
           aqui: o laço já percorre TODO o índice pra filtrar por nome/SKU, então não juntar
           tudo em `itens` antes de ordenar só reintroduzia o mesmo bug num teto mais alto — e
           era pior ainda com o modo "tudo" virando padrão: a tela diria "2000 de 2000
           resultado(s)" como se fosse tudo, escondendo exatamente os que faltavam. O catálogo
           cabe de sobra em memória (a varredura completa da Girassol, a maior, é da ordem de
           milhares de produtos). */
      }
      /* nome sozinho empata entre variações do mesmo produto pai — desempata por SKU pra dar
         uma ordem TOTAL e estável, que é o que o cursor abaixo depende pra não pular nem
         repetir item quando o catálogo muda entre uma página e a próxima (Codex #521 P2). */
      itens.sort((a, b) => {
        const c = String(a.nome).localeCompare(String(b.nome), 'pt-BR');
        return c !== 0 ? c : String(a.sku).localeCompare(String(b.sku), 'pt-BR');
      });

      /* 23/09 — o dono pediu TUDO de uma vez por padrão, com a opção de ver em partes.
         `limite` ausente ou 0 = manda todos os que casaram a partir do cursor. Com `limite`,
         corta e informa o resto — é o modo em partes, que ele usa quando a lista fica grande
         demais pro celular do galpão.

         Paginação por CURSOR (nome, sku) do último item mostrado, não por offset numérico:
         offset é posição na lista de AGORA, e o índice muda sozinho (indexação em background,
         produto resolvido por outra busca) entre um "ver mais" e o outro. Um produto inserido
         ou removido antes da posição pedida deslocava offset numérico — repetindo ou pulando
         item sem avisar. O cursor reposiciona pelo CONTEÚDO do último item já mostrado, que
         uma mudança alheia no índice não desloca (Codex #521 P2). */
      const aposNome = String(urlObj.searchParams.get('apos_nome') || '');
      const aposSku = String(urlObj.searchParams.get('apos_sku') || '');
      let inicio = 0;
      if (aposNome || aposSku) {
        inicio = itens.findIndex((it) => {
          const c = String(it.nome).localeCompare(aposNome, 'pt-BR');
          return c > 0 || (c === 0 && String(it.sku).localeCompare(aposSku, 'pt-BR') > 0);
        });
        if (inicio === -1) inicio = itens.length;   // cursor já era o último: acabou
      }
      const lim = Math.max(0, parseInt(urlObj.searchParams.get('limite') || '0', 10) || 0);
      const pagina = lim > 0 ? itens.slice(inicio, inicio + lim) : itens.slice(inicio);
      const ultimo = pagina[pagina.length - 1] || null;
      json(res, 200, {
        ok: true,
        itens: pagina,
        /* Codex #521 (P2, retomado): a correção anterior aqui era `inicio + itens.slice(inicio).length`
           — que é `inicio + (itens.length - inicio)`, ou seja, sempre `itens.length` de novo. Um
           no-op: exatamente o número que o apontamento pedia pra TIRAR da conta continuava entrando,
           só escrito de um jeito que parecia diferente. `itens.length` conta TAMBÉM quem ficou pra
           trás do cursor — inclusive item inserido pela indexação depois que a pessoa começou a
           rolar —, e o cursor nunca alcança esse item em nenhuma página seguinte. A tela dizia
           "30 de 51" e o "ver mais" nunca chegava aos 51: a página final vinha vazia (0 itens, pois
           não sobra nada depois do cursor) e caía no branco de "nada encontrado" no cliente, que
           SUBSTITUÍA a lista de 30 já mostrada por uma mensagem de erro. `total` agora é só o que
           é alcançável A PARTIR deste cursor — o cliente soma com o que já mostrou antes pra exibir
           o total acumulado (ver `buscarPorNome` em contagem.html). */
        total: itens.length - inicio,
        tem_mais: inicio + pagina.length < itens.length,
        proximo_apos_nome: ultimo ? ultimo.nome : '',
        proximo_apos_sku: ultimo ? ultimo.sku : '',
