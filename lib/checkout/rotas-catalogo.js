  ...
  return async function handle(req, res, urlObj, method) {
    ...
    if (method === 'GET' && p === (prefixo + '/buscar-produto-nome')) {
      ...
      json(res, 200, {
        ok: true,
        itens: pagina,
        /* Codex #521 (P2): com cursor, `itens.length` conta TAMBÉM o que ficou pra trás dele
           — inclusive item inserido pela indexação depois que a pessoa começou a rolar. Aí a
           tela diria "30 de 51" e o "ver mais" nunca alcançaria os 51, porque o cursor pula o
           que foi inserido antes dele. O total tem que ser o que AINDA É ALCANÇÁVEL daqui. */
        total: inicio + itens.slice(inicio).length,
    ...
    if (method === 'GET' && p === (prefixo + '/buscar-produto')) {
      ...
      json(res, 200, { ok: true, produto: {
        id: prod.id || null,   // Codex #528 (P2): sem isto o caminho de SKU/EAN escaneado não tinha
                                // como linkar pro estoque no Bling — só a busca por nome recebia o id
        sku: prod.codigo || '',
        nome: prod.nome || '',
