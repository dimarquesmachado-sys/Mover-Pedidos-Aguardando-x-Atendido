...
function cruzar(cancelados, reversasPorCodigo, saiuPorCodigo) {
  ...
  for (const c of (cancelados || [])) {
    ...
    if (voltou) {
    ...
    } else if (c.classe === 'entregue_apos_estorno') {
      /* 30/08: a CLASSE já sabe — veio das datas do shipping (#302). A ponta da "etiqueta do
         checkout" que eu tinha planejado virou desnecessária: o próprio Magalu diz se saiu
         (shipped_at) e se foi entregue (delivered_at), então não há o que consultar aqui. */
      situacao = 'entregue_apos_estorno';
      acao = 'ENTREGUE DEPOIS DO ESTORNO: cliente ficou com a mercadoria e com o dinheiro — prejuízo integral, vale contestar com o Magalu';
    } else if (c.classe === 'entregue_e_cancelado') {
      situacao = 'entregue_e_cancelado';
      acao = 'foi ENTREGUE ao cliente e cancelado depois, sem devolução registrada — conferir se o produto voltou antes de dar baixa';
    } else if (c.classe === 'saiu_e_nao_entregou') {
      situacao = 'saiu_e_nao_entregou';
      acao = 'saiu e NÃO foi entregue (insucesso/recusa) — o produto deve ter voltado: conferir no recebimento e emitir devolução COM entrada de estoque';
    } else if (c.classe === 'nf_sem_saida') {
      situacao = 'nf_sem_saida';
      acao = 'produto NUNCA saiu do CD — cancelar a NF (se no prazo) ou devolução SEM entrada de estoque, só pra recuperar o imposto';
    } else if (despachou) {
      ...
      situacao = 'indefinido';
      acao = 'sem as datas do shipping e sem reversa — recolete os cancelados pra classificar';
    } else {
      situacao = 'nf_sem_saida';
  ...
  return {
    ...
    /* prejuízo com produto perdido: o entregue-após-estorno é o caso mais claro disso */
    valor_prejuizo_real: soma(l => l.situacao === 'saiu_e_nao_voltou' || l.situacao === 'entregue_apos_estorno'),
    valor_conferir_retorno: soma(l => l.situacao === 'entregue_e_cancelado' || l.situacao === 'saiu_e_nao_entregou' || l.situacao === 'estorno_sem_remessa'),
