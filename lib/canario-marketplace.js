  ...
  for (const canal of alvo) {
    ...
    if (faltando.length && typeof ctx.packDaVenda === 'function') {
      ...
      for (const venda of faltando) {
        if (idas >= LIMITE) { resolvidos.push(venda); continue; }
        let rp = null;
        ...
        if (typeof ctx.ordensDoPack !== 'function' || idas >= LIMITE) { resolvidos.push(venda); continue; }
        let ri = null;
        ...
        if (Array.isArray(irmas) && irmas.some(o => doBling.has(String(o).trim()))) continue;
