...
async function conferir(ctx, dias, canais, opts) {
  ...
  for (const canal of alvo) {
    ...
    if (faltando.length && typeof ctx.packDaVenda === 'function') {
      const LIMITE = 25;
      const conferir = faltando.slice(0, LIMITE);
      const aindaFalta = faltando.slice(LIMITE);
      for (const venda of conferir) {
        let pack = null;
        try { pack = await ctx.packDaVenda(canal, venda); } catch (e) { pack = null; }
        /* pack null = não deu pra saber; mantém como faltante (não inventa presença) */
        if (pack && doBling.has(String(pack).trim())) continue;
        aindaFalta.push(venda);
      }
      faltando = aindaFalta;
    }
