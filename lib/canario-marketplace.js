...
async function conferir(ctx, dias, canais, opts) {
  ...
  for (const canal of alvo) {
    ...
    if (faltando.length && typeof ctx.packDaVenda === 'function') {
      /* Codex #335 (1): o lote de 25 pegava SEMPRE os mesmos primeiros, porque o resultado do
         ML vem em ordem cronológica e `faltando` é remontado do zero a cada rodada. Com mais
         de 25 sem pack na busca, do 26º em diante nunca seria consultado — falso alerta
         permanente. Agora o que já foi consultado e resolvido não volta pra fila: o cache do
         ctx responde na hora (não gasta chamada) e o lote avança para os ainda desconhecidos. */
      const LIMITE = 25;
      const resolvidos = [];
      let gastos = 0;
      for (const venda of faltando) {
        if (gastos >= LIMITE) { resolvidos.push(venda); continue; }
        let pack = null;
        try { pack = await ctx.packDaVenda(canal, venda); } catch (e) { pack = null; }
        /* só conta como consulta quando de fato foi ao ML; o ctx serve do cache sem custo */
        gastos++;
        if (!pack) { resolvidos.push(venda); continue; }   /* não deu pra saber: segue faltante */
        const packS = String(pack).trim();
        if (doBling.has(packS)) continue;                  /* o Bling guardou pelo pacote */
        /* Codex #335 (2): num carrinho de vários itens o Bling pode ter gravado o número de
           uma IRMÃ, não o pack nem esta venda. Só comparar o pack deixaria a venda acusada —
           justamente quando o mapa do billing ainda não conhece a relação, que é a condição
           que faz este caminho existir. Então pergunta ao ML quais são as ordens do pacote. */
        let irmas = null;
        if (typeof ctx.ordensDoPack === 'function') {
          try { irmas = await ctx.ordensDoPack(canal, packS); } catch (e) { irmas = null; }
        }
        if (irmas && irmas.some(o => doBling.has(String(o).trim()))) continue;
