...
function mudos() {
  ...
  const estado = ler();
  /* Codex #307 r3: o filtro tinha caído em OUTRA função, onde `vivas` nem existe — quebraria
     ao rodar, e o node --check não pega ReferenceError. Aqui é o lugar certo. */
  for (const ch of Object.keys(estado)) {
    const e = String((estado[ch] && estado[ch].empresa) || '').toLowerCase();
    if (e && vivas.indexOf(e) < 0) delete estado[ch];
  }
  const out = [];
