...
/* 15/09 (Codex #314): id→nome das situações de VENDA no Bling desta empresa (mesma descoberta
   que garantirSitCancel já faz nos index.js, só que pra TODAS as situações, não só cancelado —
   sem isso o dono vê só o número da situação em por_situacao e precisa abrir o Bling pra saber
   se é "Cancelado", "Bonificação" ou outra coisa.
   25/09 (Codex #529 P2): /situacoes/modulos/{id} é PAGINADO — lib/checkout/descobrir-ids.js já
   levou um susto com isso (uma situação real, "Checkout parcial", sumia lendo só a 1ª página).
   Uma lista parcial aqui não avisa que está incompleta, então percorre até a página vir vazia
   ou repetida. */
async function buscarNomesSituacao(blingGet) {
  const nomes = {};
  try {
    const rm = await blingGet('/situacoes/modulos');
    const mods = (rm && rm.ok && rm.data && rm.data.data) || [];
    const modVenda = mods.find(m => /venda/i.test(String(m.nome || m.descricao || ''))) || mods[0];
    if (modVenda && modVenda.id) {
      for (let pag = 1; pag <= 10; pag++) {
        const rs = await blingGet('/situacoes/modulos/' + modVenda.id + '?pagina=' + pag + '&limite=100');
        const sits = (rs && rs.ok && rs.data && rs.data.data) || [];
        if (!sits.length) break;
        let novos = 0;
        for (const s of sits) {
          if (!s || s.id == null) continue;
          const k = String(s.id);
          if (!(k in nomes)) novos++;
          nomes[k] = s.nome || s.descricao || null;
        }
        if (sits.length < 100 || !novos) break;
      }
    }
  } catch (e) { /* sem nome não é fatal — por_situacao segue com o id numérico */ }
  return nomes;
}
...
async function procurar(blingGet, de, ate, opcoes) {
  ...
  while (pagina <= limitePaginas) {
    ...
    /* 25/09 (Codex #529 P2): um `ok:true` com corpo vazio/malformado (o wrapper devolve
       `{ok:true, data:null}` quando o 2xx não é JSON válido) caía no mesmo `[]` de "página
       sem pedidos" e a busca terminava calada de novo — a lista VÁLIDA vazia (period sem
       pedidos) e o corpo QUEBRADO precisam ser distinguíveis, senão o conserto do #314 não
       vale nada pra esse caso. */
    const corpo = r && r.data;
    let lista;
    if (corpo && Array.isArray(corpo.data)) lista = corpo.data;
    else if (Array.isArray(corpo)) lista = corpo;
    else { erro = 'Bling respondeu OK mas o corpo veio vazio ou fora do formato esperado'; break; }
    ...
    for (const p of lista) {
      pedidos.push({
        ...
        situacao: (p.situacao && p.situacao.id) || null,
      });
    }
  ...
  /* 15/09 (Codex #314): a chave era p.situacao.valor — o 0/1 "ativo?" que a LISTAGEM manda,
     documentado em amb-checkout-offline/index.js:6241-6244 — e não o id da situação de fato.
     Cancelado, aberto e qualquer outra situação "inativa" mandam valor=0, que virava "0" e,
     como 0 é falsy, o `|| null` da leitura acima convertia tudo isso em sem_situacao — então
     cancelado, transferência e bonificação caíam no MESMO balde, e o por_situacao não
     distinguia nada, que era o ÚNICO motivo dele existir. Agora agrupa por situacao.id
     (fixado na leitura acima) e resolve o nome pra quem for ler o relatório não precisar
     abrir o Bling pra saber o que cada id significa. */
  const porSituacao = {};
  for (const p of pedidos) {
  ...
  if (!erro && pedidos.length) {
    const nomesSituacao = await buscarNomesSituacao(blingGet);
    for (const k of Object.keys(porSituacao)) if (nomesSituacao[k]) porSituacao[k].nome = nomesSituacao[k];
  }
