...
function conferir(empresa) {
  ...
  const blingVar = require('./empresas').envBling(emp);   /* 30/08: nome derivado, com compat pros históricos */
  /* Codex #307: derivar a env NÃO cria a integração. As três empresas atuais têm módulo
     próprio (girassol/, good/, ambtotal/) com tokenManager; empresa nova teria a env e
     nenhum código consumindo. Dizer 'ok' aqui seria prometer o que não existe. */
  const temModulo = (() => { try { require('../' + (emp === 'amb' ? 'ambtotal' : emp) + '/tokenManager'); return true; } catch (x) { return false; } })();
  itens.push({ nome: 'Bling', ok: !!blingVar && tem(blingVar) && temModulo,
    falta: !temModulo ? 'módulo do Bling da empresa (pasta ' + emp + '/ com tokenManager) — a env sozinha não basta' : blingVar,
    como: temModulo ? 'autorizar uma vez em /<empresa>/bling/callback'
        : 'empresa nova precisa do módulo de Bling portado; sem ele, o embarque cobre só os marketplaces' });

  /* Marketplaces — cada um com o seu sinal de "está ligado" */
  /* Codex #307: eu deixei o fallback ANTIGO aqui — empresa nova em EMPRESAS apareceria como
     'falta TikTok' mesmo com o módulo já reconhecendo ela. */
  const lojasTk = String(process.env.TIKTOK_LOJAS || require('./empresas').lista().join(',')).split(',').map(s => s.trim());
  itens.push({ nome: 'TikTok', ok: lojasTk.indexOf(emp) >= 0,
    falta: 'nome da empresa em TIKTOK_LOJAS', como: 'acrescentar na env e autorizar em /tiktok/conectar?loja=' + emp });
  itens.push({ nome: 'Magalu', ok: require('./empresas').valida(emp),
