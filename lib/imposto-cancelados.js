    ...
    if (typeof ctx.canceladosNoMarketplace === 'function') {
      try {
        ...
        for (const sn of (snsCancelados || [])) {
          ...
          if (!v || String(sn).startsWith('__')) continue;
          if (!v.cancelado_mkt) { v.cancelado_mkt = 1; v.situacao = v.situacao || 'Cancelado na Shopee'; }
          v.escrow_final = 1;                      /* cancelado não gera escrow: sai da fila */
          const n = String(v.numero || '').trim();
          if (n && !nums.has(n)) { nums.add(n); _varre.da_shopee++; }
        }
