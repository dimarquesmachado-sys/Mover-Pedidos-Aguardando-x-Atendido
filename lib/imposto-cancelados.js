...
async function varrerCancelados(ctx, dias, empresa) {
  ...
  /* Codex #391: sem situação de cancelamento descoberta no Bling, a função voltava ANTES
     de perguntar à Shopee — ou seja, a fonte da verdade só era consultada quando o espelho
     já tinha achado algo. Agora segue: o bloco do Bling é pulado e o da Shopee roda. */
    ...
    if (typeof ctx.canceladosNoMarketplace === 'function') {
      try {
        ...
        for (const sn of (snsCancelados || [])) {
          ...
          const v = porSn && porSn[String(sn).trim()];
          if (v) {
            ...
            snsForaDoIndice.push(String(sn).trim());
          }
        }
