...
const _reapVazio  = () => ({ rodando:false, meses:[], fila:[], mesAtual:null, linhas:0, atualizadas:0, erros:0, inicio:null, fim:null, msg:'' });
...
async function reaplicarImposto(ctx, meses, empresa){
  ...
  if (_reap.rodando) {
    /* Codex #498 (P1): salvar uma alíquota nova ENQUANTO a rodada anterior ainda busca ou
       grava perdia o segundo pedido em silêncio — o arquivo já tinha a config mais nova, mas
       o histórico persistido ficava preso na alíquota do primeiro save, sem erro nenhum, até
       alguém salvar de novo por acaso. Agora entra na fila da rodada em curso: ela reprocessa
       estes meses (com a config já atualizada, lida de novo por mês) assim que terminar os
       que já estava fazendo. */
    const novos = meses.filter(m => _reap.fila.indexOf(m) === -1);
    if (novos.length) { _reap.fila = _reap.fila.concat(novos); _reap.msg = 'na fila: ' + _reap.fila.join(', '); }
    return _reap;
  }
  ...
  // lido POR MÊS (não uma vez só no início): com a fila, um mês pode ser processado depois de
  // um novo save ter mudado a config — ler uma vez só serviria a alíquota velha pro atrasado.
  const aliqDe = m => { const cfg = ctx.readJson(ctx.path.join(ctx.CACHE_DIR, '_config-fiscal.json'), { aliquotas: {} });
    const a = cfg.aliquotas && cfg.aliquotas[m];
  ...
  _setReap({ rodando:true, meses:meses.slice(), fila:[], mesAtual:null, linhas:0, atualizadas:0, erros:0,
            inicio:new Date().toISOString(), fim:null, msg:'' });
  console.log('[FISCAL] reaplicando imposto em: ' + meses.join(', '));
  try {
    let fila = meses.slice();
    while (fila.length) {
      const mes = fila.shift();
      if (_reap.meses.indexOf(mes) === -1) _reap.meses.push(mes);
      _reap.mesAtual = mes;
      const aq = aliqDe(mes);
      if (aq == null) { console.log('[FISCAL] ' + mes + ': sem alíquota — pulando'); }
      else {
      const ini = mes + '-01';
      ...
      if (!fila.length && _reap.fila.length) {
        fila = _reap.fila.splice(0);
        console.log('[FISCAL] fila recebida durante a rodada — retomando: ' + fila.join(', '));
      }
    }
...
// 13/09 - O CONTADOR DE APAGADOS SEMPRE DEU ZERO. As duas limpezas do ano fecharam com
// apagados:0 e erros:0 (bom demais pra ser verdade) porque o codigo lia o lado errado do
// Content-Range: num DELETE com count=exact o PostgREST poe o total DEPOIS da barra, e a
// leitura pegava o pedaco antes dela (um asterisco, que vira NaN e soma zero). Ou seja,
// mesmo apagando linhas o relatorio dizia zero e nao dava pra saber se a limpeza do
// historico funcionou. Le os dois formatos e devolve zero so quando nao ha cabecalho.
function _apagadosDoCabecalho(cr) {
  const txt = String(cr || '').trim();
  if (!txt) return 0;
  const depois = txt.split('/')[1];
...
async function varrerCancelados(ctx, dias, empresa) {
  ...
  try {
    ...
    if (typeof ctx.canceladosNoMarketplace === 'function') {
      /* PR #392 (Codex, P2): o cliente do sync tinha um teto de 60 dias por chamada — era
         o serviço externo, não esta lib, quem impunha isso, e por isso a varredura avisava
         quando `dias` (pode chegar a 400 pela rota manual) passava de 60. O serviço foi
         ajustado em par com o cliente (fatia de 15 em 15 dias e pagina por dentro; o que
         chega aqui já é a lista completa do período pedido), então o teto do cliente
         (Math.min(400, ...), linha acima) voltou a ser a única fronteira real — não sobrou
         excedente pra avisar. */
      try {
      ...
      if (url && key) {
        ...
        for (let i = 0; i < arr.length; i += 80) {
          ...
          try {
            ...
            if (rd.ok) { _varre.apagados += _apagadosDoCabecalho(rd.headers.get('content-range')); }
            else _varre.erros++;
