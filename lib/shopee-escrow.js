...
function contasDoEscrow(resp) {
  ...
  return {
    ...
    itens: (function(){
      ...
      const somaIt = its.reduce((s, i2) => s + (num(i2.discounted_price) || num(i2.selling_price) || num(i2.original_price)) * qtdAtiva(i2), 0);
      ...
      return its.filter(function(i2){ return qtdAtiva(i2) > 0; }).map(function(i2){
