...
function contasDoEscrow(resp) {
  ...
  return {
    ...
    itens: (function(){
      ...
      const somaQtd = ativos.reduce((s, i2) => s + qtdAtiva(i2), 0);   /* Codex #312 r3: base do rateio quando não há preço */
      ...
      return ativos.map(function(i2){
        ...
        /* Codex #312 r3: sem preço, o rateio caía por LINHA e ignorava a quantidade — 1
           unidade e 5 unidades levavam a mesma tarifa, o que anula justamente o efeito que
           este PR quer ter (cancelar unidades mudar o rateio). Sem preço, pondera pela
           quantidade ativa; só se nem quantidade houver é que divide por linha. */
        const parte = somaIt > 0 ? (v2 / somaIt)
                    : (somaQtd > 0 ? (q2 / somaQtd)
                    : (ativos.length ? 1 / ativos.length : 0));
