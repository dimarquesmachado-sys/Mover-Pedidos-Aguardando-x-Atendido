...
function resumoDevolucoesML(ctx, de, ate) {
  ...
  let qtd = 0, custo = 0, semRetorno = 0, aindaRetido = 0;
  const lista = [];
  for (const d of Object.values(dev)) {
    ...
    porStatus[d.status_retorno || d.status || '?'] = (porStatus[d.status_retorno || d.status || '?'] || 0) + 1;
    if (String(d.dinheiro || '') === 'retained') aindaRetido++;
    const info = (typeof ctx.skuDoPedido === 'function' && d.order_id) ? (ctx.skuDoPedido(d.order_id) || null) : null;
  ...
  return {
    ...
    nota: 'o custo REAL da devolucao do ML continua vindo do faturamento (categoria devolucao) — o return-cost da API veio zero nos casos abertos. Aqui esta O QUE voltou e por que.',
