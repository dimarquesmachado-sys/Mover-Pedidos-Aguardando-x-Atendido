    ...
    for (let off = 0; off < 2000 && !parar; off += 50) {
      ...
      try {
        ...
        if (!rc.ok) {
          ...
          if (rc.status === 403) {
            // 14/08 — na AMB o 403 persistiu MESMO com "Venda e envios" em leitura+escrita e
            // token reconsentido (a pesca de tarifas passou a funcionar com o mesmo token).
            // Então parei de adivinhar qual permissão falta: o corpo da resposta do ML diz o
            // motivo exato ("scope", "not allowed", "invalid_token"…). Ele vai junto no erro.
            const appId = (String(ctx.token || '').match(/APP_USR-(\d+)-/) || [])[1] || 'não identificado';
            let corpo = '';
            try { corpo = JSON.stringify(await rc.json()).slice(0, 300); }
            catch (e) { try { corpo = (await rc.text()).slice(0, 300); } catch (e2) { corpo = '(sem corpo)'; } }
            erro = 'HTTP 403 em claims/search · app ' + appId + ' · vendedor ' + sellerId +
                   ' · resposta do ML: ' + corpo +
                   ' — se falar em escopo/permissão, habilite TODAS as permissões funcionais em Leitura+escrita no painel do app e reconsinta; ' +
                   'se falar em token, o problema é o token, não a permissão.';
            break;
          }
...
async function resumoDevolucoesML(ctx, de, ate) {
  ...
  for (const d of Object.values(dev)) {
    ...
    // 14/08 (regra do Diego): a fonte é o MARKETPLACE. Bipe é evento de operação, não fonte
    // de dado — o fallback que lia o cache do checkout saiu daqui. Ordem: pedido no ML
    // (seller_sku, título, preço) → histórico (que também nasce do marketplace, pelo order ou
    // pelo pack). Sem cache de conferência no meio.
