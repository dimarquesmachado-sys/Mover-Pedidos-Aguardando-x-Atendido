...
// 14/08 — textos confirmados pelo Diego direto na tela do ML, um pedido de cada vez.
// Sem isso o card mostraria "PDD9939" pro estoquista.
const MOTIVO_ML = {
...
// Contá-las como devolução infla o número — mesmo erro que a Shopee tinha com as CANCELLED.
const STATUS_SEM_EFEITO = ['expired', 'cancelled'];

async function coletarDevolucoesML(ctx, dias) {
  ...
  for (const st of ['opened', 'closed']) {
    ...
    for (let off = 0; off < 2000 && !parar; off += 50) {
      ...
      try {
        ...
        if (!rc.ok) { erro = erro || ('claims/search ' + st + ': HTTP ' + rc.status); break; }
  ...
  const pendentes = Object.values(arq.devolucoes).filter(d => !d.detalhe_em).slice(0, 120);
  ...
  return { ok: !erro, dias_pedidos: total, vistas, novas, com_retorno: comRetorno, guardadas: Object.keys(arq.devolucoes).length, erro };
}
...
async function resumoDevolucoesML(ctx, de, ate) {
  ...
  const doHistorico = (typeof ctx.buscarNoHistorico === 'function')
    ...
    : {};
  let qtd = 0, custo = 0, semRetorno = 0, aindaRetido = 0, semEfeito = 0;
  const lista = [];
  for (const d of Object.values(dev)) {
    ...
    // devolução que não se concretizou: o ML liberou o dinheiro de volta pra loja
    if (STATUS_SEM_EFEITO.indexOf(st) >= 0) { semEfeito++; continue; }
    qtd++;
    custo = Math.round((custo + _num(d.custo_retorno)) * 100) / 100;
    const mot = d.motivo || 'sem motivo';
    const motTxt = MOTIVO_ML[mot] || mot;
    porMotivo[motTxt] = (porMotivo[motTxt] || 0) + 1;
    if (String(d.dinheiro || '') === 'retained') aindaRetido++;
    const info = (doHistorico && doHistorico[String(d.order_id || '')])
  ...
  return {
    ...
    // não entram no total: o ML devolveu o dinheiro pra loja (comprador não enviou ou desistiu)
    ...
    nota: 'o custo REAL segue vindo do faturamento (categoria devolucao). No ML, arrependimento com produto OK normalmente NAO tem custo de retorno cobrado do vendedor — por isso a maioria vem zero. Devolucao expirada/cancelada NAO entra no total: o ML liberou o dinheiro de volta.',
