...
function responderCusto(opcoes) {
  ...
  const tDev = (dev.ok_em || dev.atualizado) ? Date.parse(dev.ok_em || dev.atualizado) : null;
  const tFin = (fin.ok_em || fin.atualizado) ? Date.parse(fin.ok_em || fin.atualizado) : null;
  ...
  return { http: 200, corpo: Object.assign({
    ok: true,
    cache_atualizado_em: dev.ok_em || dev.atualizado || null,
    horas_desde_a_coleta: horas,          /* medidas desde a última coleta BEM-SUCEDIDA */
    coleta_falhando: !!((dev.atualizado && dev.ok_em && dev.atualizado > dev.ok_em) || (fin.atualizado && fin.ok_em && fin.atualizado > fin.ok_em)),
