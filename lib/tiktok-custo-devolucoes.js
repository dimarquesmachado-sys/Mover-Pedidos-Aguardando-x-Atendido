...
function responderCusto(opcoes) {
  ...
  const janela = (cache) => cache.varreu_desde ? Math.floor(Date.parse(cache.varreu_desde) / 1000) : null;
  let devDesde = janela(dev), finDesde = janela(fin);
  /* sem janela gravada (cache de antes deste conserto), cai no registro mais antigo */
  if (devDesde == null) for (const d of Object.values(dev.devolucoes)) {
    const t = Number(d && d.criado_em) || 0;
    if (t && (!devDesde || t < devDesde)) devDesde = t;
  }
