...
function responderCusto(opcoes) {
  ...
  /* Codex #291 r2: `atualizado` é gravado a CADA tentativa, mesmo falhando (lib/tiktok-
     financeiro.js só avança `ok_em` quando dá certo). Usar o atualizado mostrava dado
     fresco com coleta quebrada há dias — exatamente o silêncio que perseguimos hoje. */
  /* Codex #291 r3 (P1): o fallback pro `atualizado` transformava FRACASSO em coleta fresca.
     Quando a primeira coleta falha, a lib grava um mapa válido e `atualizado`, mas deixa
     `ok_em` NULO de propósito — é o sinal de que nunca houve sucesso. Sem ok_em não há
     coleta boa: nada de fallback. */
  const tDev = dev.ok_em ? Date.parse(dev.ok_em) : null;
  const tFin = fin.ok_em ? Date.parse(fin.ok_em) : null;
  const nuncaColetou = [];
  if (!dev.ok_em) nuncaColetou.push('devoluções');
  if (!fin.ok_em) nuncaColetou.push('financeiro');
  ...
  /* Codex #291 r2: a cobertura tem que ser a INTERSEÇÃO das duas coletas. O financeiro
     varre menos dias que as devoluções (120 × 180), então um período coberto pelas
     devoluções mas não pelo extrato saía como completo e sem custo — pior que avisar. */
  /* Codex #291 r4: a cobertura verdadeira é a JANELA que a última varredura bem-sucedida
     cobriu — não o registro mais antigo que sobrou em cache de coletas passadas. */
  /* Codex #292: varredura truncada não declara cobertura — sem varreu_desde, a cobertura
     é DESCONHECIDA e o período sai como parcial (admitir a dúvida > garantir o que não sei). */
  const janela = (cache) => (cache.varreu_desde && !cache.truncou_paginas) ? Math.floor(Date.parse(cache.varreu_desde) / 1000) : null;
  let devDesde = janela(dev), finDesde = janela(fin);
  /* Sem janela declarada (cache antigo ou varredura truncada): a cobertura é DESCONHECIDA.
     Deduzi-la do registro mais antigo seria otimista — aquele registro pode ser resto de uma
     coleta longa anterior, e o período apareceria como completo sem ter sido varrido. */
  const coberturaDesconhecida = (devDesde == null || finDesde == null);
  if (finDesde == null) for (const p of Object.values(fin.pedidos)) {
    const t = Number(p && (p.criado_em || p.data)) || 0;
    if (t && (!finDesde || t < finDesde)) finDesde = t;
  }
  const coberturaDesde = (devDesde && finDesde) ? Math.max(devDesde, finDesde) : (devDesde || finDesde);
  const parcial = coberturaDesconhecida || !!(coberturaDesde && ini < coberturaDesde);

  return { http: 200, corpo: Object.assign({
    ok: true,
    cache_atualizado_em: dev.ok_em || null,
    horas_desde_a_coleta: horas,          /* desde a última coleta BEM-SUCEDIDA; null se nunca houve */
    /* falhando = tentou depois do último sucesso, OU nunca teve sucesso nenhum */
    coleta_falhando: !!(nuncaColetou.length || (dev.atualizado && dev.ok_em && dev.atualizado > dev.ok_em) || (fin.atualizado && fin.ok_em && fin.atualizado > fin.ok_em)),
    nunca_coletou: nuncaColetou.length ? nuncaColetou : null,
    periodo_parcial: parcial,
    cobertura_desconhecida: coberturaDesconhecida,
