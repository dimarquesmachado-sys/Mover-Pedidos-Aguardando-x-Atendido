...
async function coletarAds(ctx, dias, loja) {
  ...
  let novos = 0, vistos = 0, janelas = 0, erro = null, limiteApi = null;
  const hoje = new Date();
  for (let off = 0; off < total; off += 30) {
    ...
    if (!Array.isArray(resp)) {
      // 14/08 — MEDIDO na Girassol pedindo 226 dias: a Shopee recusa relatório de ads mais
      // antigo que ~180 dias com `ads.performance.error_date_too_old`. Isso é LIMITE DA API,
      // não falha nossa: registrar como erro fazia a etapa noturna reprovar sem motivo e
      // sugeria que faltava dado recuperável. Janelas mais antigas nem são tentadas.
      if (/date_too_old/i.test(String(motivo))) {
        limiteApi = limiteApi || ('a Shopee não fornece relatório de ads anterior a ' + iso(ini) + ' (limite ~180 dias) — histórico mais antigo só pelo painel');
        break;
      }
      erro = erro || ('janela ' + iso(ini) + '..' + iso(fim) + ': ' + String(motivo).slice(0, 160));
      continue;
    }
    for (const l of resp) {
  ...
  return { ok: !erro || vistos > 0, dias_pedidos: total, janelas, dias_vistos: vistos, dias_novos: novos, erro, limite_da_api: limiteApi };
}
