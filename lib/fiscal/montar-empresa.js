...
function montarEmpresa(idOuAlias, opcoes) {
  ...
  /* 15/09 — empresa NOVA não pode herdar o canal de venda de outra. Nas existentes o padrão
     herdado fica (não dá pra ver o Render daqui, e tirar quebraria quem depende dele), mas
     quem nasce agora declara o próprio: sem isso, o F1 julgaria os pedidos dela pelo canal
     alheio e ignoraria tudo — em silêncio, que é o pior jeito. */
  if (!process.env[env('ME_LOJA_IDS')]) {
    throw new Error('lib/fiscal/montar-empresa: ' + e.id + ' precisa de ' + env('ME_LOJA_IDS') +
                    ' (os ids do canal do Mercado Livre no Bling desta empresa) — sem isso o F1 usaria o canal de outra');
  }
