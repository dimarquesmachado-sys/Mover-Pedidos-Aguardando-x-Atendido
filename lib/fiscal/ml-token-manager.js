...
/* ─────────────────────────────────────────────────────────────────────────────
   ...
   Duas coisas que a classificação mostrou:
     • as diferenças eram prefixo de env, caminho do arquivo e rótulo de log —
       nenhuma regra;
     • a Girassol, de novo, usa envs SEM prefixo (ML_CLIENT_ID) e um redirect
       próprio: valores lidos do arquivo dela, não deduzidos.
...
function criarMlTokenManager(cfg) {
  ...
  const ML_REDIRECT_URI  = process.env[cfg.envRedirect];
  ...
  if (resp.status !== 401 && resp.status !== 403) {
    throw new Error('AMB ML: sonda inconclusiva (HTTP ' + resp.status + '); refresh preservado — transitório, tente de novo');
  }
