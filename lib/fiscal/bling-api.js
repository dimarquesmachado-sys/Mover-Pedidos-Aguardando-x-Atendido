...
function criarBlingApi(cfg) {
  ...
  /* 15/09 — ATENÇÃO, isto decide quais pedidos o F1 considera do Mercado Livre. O padrão
     '206017293' é o id de UMA empresa (o histórico, da primeira que existiu no serviço), e
     ele valia pra qualquer empresa que não definisse a env — ou seja, uma empresa nova
     usaria o canal de venda de OUTRA pra decidir o que é dela. É a mesma classe do CNPJ
     trocado na DANFE: funciona, e funciona errado.
     Não removi o padrão porque não tenho como ver o Render: se alguma empresa atual depende
     dele, tirar quebraria o F1 dela hoje. O que dá pra fazer sem risco é a verdade aparecer
     no boot — quem usa env própria e quem está no padrão herdado. */
  const _lojaIdsBrutos = process.env[cfg.envMeLojaIds];
  const ME_LOJA_IDS = (_lojaIdsBrutos || '206017293').split(',').map(Number);
  if (!_lojaIdsBrutos) {
    console.warn('[' + rotulo + ' blingApi] ' + cfg.envMeLojaIds + ' NÃO está definida — usando o id ' +
                 'herdado 206017293 pra decidir o que é venda do Mercado Livre. Se este não for o canal ' +
                 'DESTA empresa, o F1 está julgando os pedidos dela pelo canal de outra.');
  } else {
