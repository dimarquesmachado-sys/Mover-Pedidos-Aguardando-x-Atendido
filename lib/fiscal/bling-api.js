...
function criarBlingApi(cfg) {
  ...
  if (!_lojaIdsBrutos) {
    ...
    const _saida = _emp
      ? 'defina SKIP_EMPRESAS=' + _emp.id + ' pra o resto do servidor subir sem ela, rode ' +
        '/' + _emp.slugDescobrirIds + '/descobrir-ids (prova o canal contra a conta do ML), ' +
        'cole o valor em ' + cfg.envMeLojaIds + ' no Render e tire o SKIP_EMPRESAS'
    ...
    throw new Error('[' + rotulo + '] falta ' + cfg.envMeLojaIds + ': sem ela o F1 não sabe quais ' +
      'canais de venda são do Mercado Livre, e sem ela o servidor inteiro não sobe (falha alto ' +
      'de propósito, ver config/empresas.js). ' + _saida[0].toUpperCase() + _saida.slice(1) + '.');
  }
