...
/* 16/09 (P2 do Codex) — a mensagem do throw abaixo precisa do id de contrato (pro
   SKIP_EMPRESAS) e do slug HTTP real da rota /descobrir-ids desta empresa, e nenhum dos
   dois dá pra derivar do `rotulo` por fórmula (o slug do checkout-offline não segue o
   prefixo de env). As três são as únicas que passam por aqui hoje — a lista some inteira
   nos testes de espelho se um rótulo novo aparecer sem entrada aqui. */
const _EMPRESA_POR_ROTULO = {
  GIRASSOL: { id: 'girassol', slugDescobrirIds: 'girassol-backup-offline' },
  AMB:      { id: 'ambtotal', slugDescobrirIds: 'amb-checkout-offline' },
  GOOD:     { id: 'good',     slugDescobrirIds: 'good-checkout-offline' },
};
...
function criarBlingApi(cfg) {
  ...
  /* 16/09 — O ID HERDADO SAIU. Ele decidia quais pedidos o F1 considera do Mercado Livre, e
     ...
     16/09 (P2 do Codex) — a mensagem original mandava rodar "/<empresa>/descobrir-ids", uma
     rota que não existe (o slug real é o do checkout-offline de cada empresa), E que fica
     INALCANÇÁVEL nesse exato cenário: config/empresas.js exige TODAS as lojas ativas de uma
     vez (é o mesmo "falha alto, não vira fantasma" desta nota), então este throw derruba o
     boot do servidor INTEIRO — não só desta empresa —, e sem o servidor de pé nenhuma rota
     HTTP responde. A saída que funciona ANTES do boot é tirar só esta empresa da lista com
     SKIP_EMPRESAS: o resto sobe, a própria rota de descoberta DELA sobe junto (ela não lê
     ME_LOJA_IDS pra existir, só pra o F1 rodar), e dá pra pegar o valor sem editar código. */
  const _lojaIdsBrutos = String(process.env[cfg.envMeLojaIds] || '').trim();

  /* 16/09 — A PROTEÇÃO FICA NO USO, NÃO NO BOOT. O conserto anterior derrubava o boot quando
     a env faltava, e isso tinha dois custos: o CI não passa essas envs (deixando o ai-safety
     vermelho, e corrigir exigiria o dono aplicar mudança de workflow à mão, porque meu token
     não tem escopo `workflow`), e derrubar o servidor inteiro por UMA empresa sem canal leva
     as outras duas junto.
     O que protege de verdade é a função que DECIDE se o pedido é do Mercado Livre: sem canal
     declarado ela não pode responder "não é do ML" pra tudo — esse era exatamente o silêncio
     que o id herdado causava. Ela recusa, e só aquela empresa fica parada, com a causa no log. */
  /* 16/09 (P2 do Codex) — o F3 (nfe-ml-fluxo.js) tem o PRÓPRIO guarda de canal vazio, porque
     lê ME_LOJA_IDS exportado direto em vez de passar por canaisDeclarados(). Sem esta função
     exportada, aquele guarda não tinha como saber a rota real de /descobrir-ids desta empresa
     nem o nome da env — e caiu de volta no molde genérico "/descobrir-ids do checkout-offline",
     que é exatamente o que este comentário já tinha corrigido aqui. Uma função só, os dois
     guardas usam. */
  function mensagemCanalFaltando() {
    /* a rota REAL desta empresa, não o molde "/<empresa>/descobrir-ids" que ninguém consegue
       colar — o slug do checkout não segue o prefixo de env (conserto do Codex nesta rodada). */
    const _emp = _EMPRESA_POR_ROTULO[rotulo];
    return '[' + rotulo + '] falta ' + cfg.envMeLojaIds + ': sem ela o F1 não sabe quais ' +
      'canais de venda são do Mercado Livre e NÃO vai mover pedido nenhum desta empresa. ' +
      (_emp ? 'Rode /' + _emp.slugDescobrirIds + '/descobrir-ids (prova o canal contra a conta do ML) ' +
              'e cole o valor em ' + cfg.envMeLojaIds + ' no Render.'
            /* 16/09 (P2 do Codex): rótulo fora da tabela é empresa MONTADA PELO CONTRATO, que
               não tem pasta de checkout — logo, não tem rota /descobrir-ids própria. Mandá-la
               "rodar o /descobrir-ids desta empresa" é mandar o dono a um 404, e ele ia
               procurar o que não existe em vez de resolver. */
            : 'Esta empresa é montada pelo contrato (sem pasta), então não tem rota ' +
              '/descobrir-ids própria: pegue o id do canal do Mercado Livre direto no Bling dela ' +
              'e cole em ' + cfg.envMeLojaIds + ' no Render.');
  }

  let _avisouSemCanal = false;
  function canaisDeclarados() {
    if (ME_LOJA_IDS.length) return ME_LOJA_IDS;
    if (!_avisouSemCanal) {
      _avisouSemCanal = true;
      console.error(mensagemCanalFaltando());
    }
    return null;
  }
  /* 16/09 (P2 do Codex) — DESCARTAR O ITEM INVÁLIDO EM SILÊNCIO É PIOR QUE RECUSAR TUDO.
     `203296034,abc` virava uma lista "válida" com um id só: o F1 seguia normalmente e
     ignorava todos os pedidos do canal que foi digitado errado — sem erro, sem aviso, e com
     o dono achando que configurou os dois. Um item ruim invalida a lista inteira. */
  const _itens = _lojaIdsBrutos.split(',').map(x => String(x).trim()).filter(Boolean);
  const _invalidos = _itens.filter(x => !/^\d+$/.test(x));
  const ME_LOJA_IDS = _invalidos.length ? [] : _itens.map(Number);
  if (_invalidos.length) {
    console.error('[' + rotulo + '] ' + cfg.envMeLojaIds + ' tem item inválido: ' +
      _invalidos.map(x => '"' + x.slice(0, 20) + '"').join(', ') +
      ' — a lista inteira foi RECUSADA. Aceitar os válidos e descartar o resto faria esta empresa ' +
      'ignorar os pedidos do canal digitado errado, em silêncio.');
  }
  /* 16/09 — este throw também saiu do boot, pelo mesmo motivo do outro: env vazia ou com lixo
     não pode derrubar as TRÊS empresas. Sem id válido, ME_LOJA_IDS fica vazio e o guarda
     canaisDeclarados() abaixo recusa a decisão do F1 com a causa no log — só esta empresa
     para. Foi exatamente isto que o boot do CI, sem env nenhuma, pegou. */
  if (ME_LOJA_IDS.length) console.log('[' + rotulo + ' blingApi] canais do ML: ' + ME_LOJA_IDS.join(', ') + ' (de ' + cfg.envMeLojaIds + ')');
  ...
  if (!p) return false;
  const canais = canaisDeclarados();
  /* sem canal declarado NÃO responde "não é do ML": isso faria o F1 ignorar tudo em silêncio,
     que é o bug que o id herdado causava. */
  if (!canais) throw new Error('[' + rotulo + '] ' + cfg.envMeLojaIds + ' não declarada — não dá pra dizer se o pedido é do Mercado Livre');
  return canais.includes(p.loja?.id);
  }
