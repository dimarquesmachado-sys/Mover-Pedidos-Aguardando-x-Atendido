...
/* ─────────────────────────────────────────────────────────────────────────────
   ...
   Passo 2.5 do plano multiloja, com o caminho seguro que o próprio plano prevê:
   AMB e GOOD diferiam por UM COMENTÁRIO, então viram uma peça só; a Girassol fica
   fora por enquanto, e não por preguiça — ela tem comportamento a mais que
   ninguém decidiu portar:

     • devolve `status` do shipment além do substatus;
     • tem `getShipmentRaw`, que as outras não têm;
     • declara SITE_ID.

   Unificar as três aqui significaria escolher, sem decisão do dono, entre apagar
   isso ou impor às outras duas. O registro dessas diferenças está em
   docs/fase2-diferencas-girassol.md.
   ──────────────────────────────────────────────────────────────────────────── */
...
function criarMlApi(cfg) {
  ...
  return { getShipmentInfo, getShipmentSubstatus, enviarNFeParaML, baixarXmlNFe };
