...
/* ─────────────────────────────────────────────────────────────────────────────
   TOKEN DO BLING (app principal) — as três empresas (13/09/2026).

   Passo 2.8, o último da fila mecânica. Depois do porte da renovação proativa
   (PR #413), as três já se comportavam igual: guardam o vencimento e renovam
   antes de vencer, em vez de gastar uma sonda por operação.

   ⚠️ O caminho do arquivo continua POR EMPRESA, e isso é regra, não descuido: a
   Girassol grava relativo ao módulo e as outras no disco persistente. Unificar o
   caminho faria a empresa perder o token e exigir nova autorização no Bling no
   meio do expediente — sintoma que ninguém liga a uma refatoração.
   ──────────────────────────────────────────────────────────────────────────── */

function criarTokenManager(cfg) {
  for (const nome of ['rotulo', 'envTokenFile', 'arquivoToken', 'envBlingClientId', 'envBlingClientSecret', 'envBlingRedirectUri']) {
    if (!cfg || cfg[nome] == null) throw new Error('lib/fiscal/token-manager: falta ' + nome);
  }
  const { rotulo } = cfg;
  ...
  /* ⚠️ o caminho vem do chamador e NÃO é unificado: a Girassol grava relativo ao módulo e as
     outras no disco persistente. Trocar isso faz a empresa perder o token e pedir nova
     autorização no Bling no meio do expediente. */
  const TOKEN_FILE = process.env[cfg.envTokenFile] || cfg.arquivoToken;
  ...
  try {
    ...
    console.error(`[${rotulo} tokenManager] Erro ao ler tokens:`, e.message);
    return {};
  }
  }
  ...
  console.log(`[${rotulo} tokenManager] Tokens Bling salvos em disco ✓`);
  }

  // ── OAuth helpers ────────────────────────────────────────────────────

  function basicAuth() {
  const id  = process.env[cfg.envBlingClientId];
  const sec = process.env[cfg.envBlingClientSecret];
  if (!id || !sec) throw new Error(cfg.envBlingClientId + ' / ' + cfg.envBlingClientSecret + ' não definidos');
  ...
  if (data.error) throw new Error(`${rotulo} OAuth error: ${JSON.stringify(data)}`);
  return data;
  }
  ...
  const redirect_uri = process.env[cfg.envBlingRedirectUri] || '';
  ...
  try {
    console.log(`[${rotulo} tokenManager] Renovando token Bling...`);
    ...
    salvarTokens(data.access_token, data.refresh_token, data.expires_in);   /* expires_in: sem ele o vencimento não é guardado e a economia de chamada não acontece */
    console.log(`[${rotulo} tokenManager] Token Bling renovado ✓`);
  ...
  if (!access_token || access_token.length < 10) {
    console.log(`[${rotulo} tokenManager] Token Bling ausente — renovando`);
    return renovarToken();
  }
  ...
  if (_tok.expira_em) {
    ...
    if (Date.now() >= (_tok.expira_em - MARGEM)) {
      console.log(`[${rotulo} tokenManager] Token perto de vencer — renovando proativo`);
      return renovarToken();
    }
    return access_token;
  }
  ...
  if (resp.status === 401) {
    console.log(`[${rotulo} tokenManager] Token Bling expirado (401) — renovando`);
    return renovarToken();
  }
  ...
  return { garantirToken, renovarToken, gerarTokenInicial };

}

module.exports = { criarTokenManager };
