...
function registrarCustoVigente(ctx, sku, novo, origem, deISO) {
  const k = String(sku || '').trim();
  if (!k) return null;
  /* Codex #358 r4: mudança detectada pela janela de RECUPERAÇÃO (madrugada) valeu no
     dia varrido (ontem), não em hoje — sem a data efetiva, a faixa nascia com o dia
     errado e o backfill das 03:45 aplicava o custo velho às vendas de ontem. */
  const de = (deISO && /^\d{4}-\d{2}-\d{2}$/.test(String(deISO))) ? String(deISO) : _hojeISO();
  /* Codex #358 r3: custo REMOVIDO conclusivamente no Bling precisa FECHAR a faixa
     aberta — senão o backfill das 03:45 (que consulta custoVigenteEm primeiro) segue
     aplicando o valor obsoleto às vendas novas pra sempre. null explícito encerra. */
  if (novo === null) {
    const todas0 = lerVigencias(ctx);
    const lista0 = Array.isArray(todas0[k]) ? todas0[k] : [];
    const aberta0 = lista0.find(f => !f.ate);
    if (!aberta0) return null;
    aberta0.ate = _diaAntes(de);
    aberta0.encerrada_por = origem || 'remocao';
    todas0[k] = lista0;
    gravarVigencias(ctx, todas0);
    return 'faixa-fechada';
  }
  const v = Number(novo);
  if (!(v > 0)) return null;
  ...
  if (aberta) {
    ...
    if (aberta.de === de) { aberta.custo = v; aberta.origem = origem || aberta.origem; aberta.em = new Date().toISOString(); }
    else {
      aberta.ate = _diaAntes(de);
      lista.push({ custo: v, de: de, ate: null, origem: origem || 'bling', em: new Date().toISOString() });
    }
  } else {
    lista.push({ custo: v, de: de, ate: null, origem: origem || 'bling', em: new Date().toISOString() });
  }
...
/* Codex (#186): limpar o cache NÃO basta. Uma requisição do /sku-info que já estava resolvendo
   capturou o mapa ANTIGO; depois de esperar a rede, ela grava o custo velho de volta no cache e no
   arquivo — e ele passa a ser servido por mais 6h. Com o contador, quem começou antes da mudança
   percebe no fim que o mapa mudou e simplesmente NÃO grava. Chave é o CACHE_DIR (por empresa). */
const _epocaCusto = new Map();
function epocaCusto(ctx) { return _epocaCusto.get(String(ctx.CACHE_DIR || '?')) || 0; }
function _bumpEpoca(ctx) { const k = String(ctx.CACHE_DIR || '?'); _epocaCusto.set(k, (_epocaCusto.get(k) || 0) + 1); }

function gravarDeParaSku(ctx, o) {
  _bumpEpoca(ctx);
  ctx.writeJson(ctx.path.join(ctx.CACHE_DIR, '_sku-depara.json'), o || {});
  /* Codex (#186): mesmo motivo do gravarCustosManuais logo acima — mexer no de-para MUDA o custo
     que o SKU antigo enxerga. Sem derrubar o cache, apagar um vínculo deixava o /sku-info
     servindo o custo herdado por até 6h, e ao vencer o fallback o copiava de volta. Cobre gravar
     E apagar, nas 3 empresas, porque toda alteração passa por aqui. */
...
function comCustosManuais(ctx, mapaDoBling) {
  ...
  try {
    ...
    for (const velho of Object.keys(dp)) {
      ...
      if (alvo && Number(alvo.custo) > 0) {
        /* Codex (#186): `ts: alvo.ts || Date.now()` INVENTAVA frescor. Quando o destino não tem
           carimbo (justamente o dado legado que este caminho atende), o registro herdado passava
           na trava de 7 dias do /sku-info mentindo sobre a idade de um custo que ninguém sabe
           quando foi lido — e o refresh do produto nunca acontecia. Sem ts real, fica SEM ts:
           o /sku-info trata como velho e vai buscar, que é o certo. */
        const _h = { custo: Number(alvo.custo), id: alvo.id, depara_de: destino };
        if (alvo.ts) _h.ts = alvo.ts;
        b[velho] = _h;
      }
    }
  } catch (e) {}
...
/* Codex (#185): o registro herdado do de-para é gravado com a grafia da CHAVE guardada, mas o
   /sku-info lia `_ccAll[sku]` com a grafia que chegou na requisição — "Pm1 → DEST" resolvia certo
   dentro da lib e ainda assim a busca por "PM1" não achava o registro e ia consultar o SKU velho
   no Bling. Mesma família dos outros três: normalizar num lugar e todo mundo usa. */
function custoDeSku(mapa, sku) { return _acharChave(mapa || {}, sku); }

module.exports = { epocaCusto, custoDeSku, _acharChave, carregarCatalogoDePara, parsearCustosColados, telaCustosManuais, comCustosManuais, custoManualDe, custoVigenteEm, gravarCustosManuais, gravarDeParaSku, gravarVigencias, lerCustosManuais, lerDeParaSku, lerVigencias, registrarCustoVigente, resolverDeParaSku, resolverNomeSku, sugerirDeParaSku, _hojeISO, _ontemISO, _diaAntes };
