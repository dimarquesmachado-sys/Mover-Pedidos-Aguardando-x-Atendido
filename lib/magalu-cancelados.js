...
function resumirPedido(p) {
  ...
  if (!pagoAprovado && !temNF) { classe = 'nao_pago'; conta_no_faturamento = false; }
  ...
  const sellerEntrega = entregas.map(d => d && d.seller && (d.seller.id || d.seller.code || d.seller.name)).filter(Boolean)[0] || null;
  return {
