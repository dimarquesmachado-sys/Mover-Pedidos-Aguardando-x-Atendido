...
function resumirPedido(p) {
  ...
  const ship = entregas.map(d => d && d.shipping).filter(Boolean)[0] || {};
  const enviadoEm = ship.shipped_at || null;
  const entregueEm = ship.delivered_at || null;
  const canceladoEm = ship.cancelled_at || null;
  const estornoEm = devolucoes.map(d => d.em).filter(Boolean).sort()[0] || canceladoEm || null;
