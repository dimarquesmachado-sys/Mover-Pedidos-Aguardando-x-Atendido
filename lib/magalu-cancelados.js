...
function totalNoPeriodo(resumidos, deISO, ateISO) {
  ...
  const porClasse = {};
  /* Codex #305: a NF já cancelada saía da soma mas continuava no por_classe, então a tela
     mostrava um pedido na quebra que não estava no total — quem confere não fecha a conta. */
  for (const r of dentro) { if (r.conta_no_faturamento && jaCancelada(r)) continue; porClasse[r.classe] = (porClasse[r.classe] || 0) + 1; }
