...
async function verificarUm(m, anterior) {
  try {
    ...
    /* 29/08 (visto em produção): 429 é RATE LIMIT — as 3 provas saem em sequência e batem no
       limite do Bling. É transitório e a mensagem antiga ('verifique se a API mudou') mandava
       o dono caçar problema onde não há. Estado próprio e texto honesto. */
    if (r.status === 429) return { estado: 'indeterminado', erro: 'Bling limitou a taxa de requisições (HTTP 429) — teste adiado, sem relação com o token' };
    if (!r.ok) return { estado: 'indeterminado', erro: 'resposta inconclusiva do Bling (HTTP ' + r.status + ') — pode ser mudança de API; confira se persistir' };
...
async function verificarTodos() {
  ...
  for (const m of MODULOS) {
    /* pausa entre módulos: 3 chamadas coladas batiam no rate limit do Bling e o canário
       provocava o próprio 429 que depois reportava. */
    if (!primeiro) await new Promise(r => setTimeout(r, 3000));
    primeiro = false;
