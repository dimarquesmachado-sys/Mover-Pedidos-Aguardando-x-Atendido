...
async function decidirCustoDoProduto(prod, ctx) {
  ...
  /* reserva de duas camadas — igual pra composição incompleta OU inexistente:
     campos do próprio produto primeiro, endpoint de fornecedores por último */
  const reservaCompleta = async () => {
  ...
  if (comps.length > TETO_COMPONENTES) {
    const reserva = await reservaCompleta();
