...
const CAPACIDADES_CONHECIDAS = new Set([
  ...
  /* 15/09 — 'expedicao': a empresa tem o app onde a equipe bipa na ENTREGA à transportadora.
     Só a Girassol tem. É o que decide pra onde o checkout manda o pedido conferido: com
     Expedição vai pra VERIFICADO e o app move pra DESPACHADOS depois; sem ela, o checkout já
     manda direto pra DESPACHADOS, porque não existe etapa seguinte. Vira capacidade porque
     antes essa intenção vivia só no valor de uma env e parecia engano. */
  'expedicao',
]);
...
function _lerContrato(caminho) {
  ...
  /* Codex #423 (P1, r2): `typeof null === 'object'`, então um contrato com "empresas": null
     ...
     E os dois casos abaixo são ContratoInvalidoError, não Error genérico: o JSON FOI lido —
     é conteúdo que viola regra, o mesmo caso que `_validar` cobre. Só o catch de
     config/empresas.js decide "modo antigo" só pra Error (arquivo ilegível de verdade); um
     Error genérico aqui caía nesse fallback, ignorava EMPRESAS e subia todas as lojas —
     exatamente o achado original do P1. */
  if (!cru || typeof cru !== 'object') {
    throw new ContratoInvalidoError('registro de empresas: o contrato em ' + arq + ' não é um objeto JSON');
  }
  if (!cru.empresas || typeof cru.empresas !== 'object' || Array.isArray(cru.empresas) || !Object.keys(cru.empresas).length) {
    throw new ContratoInvalidoError('registro de empresas: o contrato em ' + arq + ' foi LIDO, mas o bloco `empresas` está ausente, vazio ou não é um objeto');
  }
  return cru;
}
...
module.exports = {
  ...
  /* vocabulário fechado exposto pra quem precisa ENUMERAR capacidades (ex.: /api/contexto)
     sem manter uma segunda lista — a mesma que `_validar` usa pra rejeitar contrato com
     capacidade desconhecida no boot. Array (não o Set) porque quem consome só itera. */
  CAPACIDADES_CONHECIDAS: Array.from(CAPACIDADES_CONHECIDAS),
};
