...
async function getCidadePorCEP(cep) {
  return _cepResolver.getCidadePorCEP(cep, "[GOOD nfBlingApi]");
}
...
async function _ieViaCNPJa(cnpjLimpo, uf) {
  try {
    ...
    if (!reg) {
      console.log(`[GOOD nfBlingApi] CNPJá CNPJ=${cnpjLimpo}: tem IE mas nenhuma na UF ${uf} — não usar`);
      return null;
    }
    if (!reg.number) {
...
async function getIEPorCNPJ(cnpj, uf) {
  ...
  if (!resultado) {
    console.log(`[GOOD nfBlingApi] IE não veio da CNPJá — tentando SintegraWS [PAGO] (CNPJ=${cnpjLimpo})`);
