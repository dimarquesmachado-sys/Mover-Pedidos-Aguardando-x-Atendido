...
async function getShipmentInfo(token, numeroPedidoLoja) {
  ...
  if (resp.status === 404) {
    ...
    if (packResp.ok) {
      ...
      if (shipmentIdPack) {
        console.log(`[GOOD mlApi] ${numeroPedidoLoja} é PACK (carrinho) → shipment_id=${shipmentIdPack} via /packs`);
        return shipmentIdPack;
      }
      throw new Error(`GOOD ML: pack ${numeroPedidoLoja} sem shipment_id ainda (orders=${(pack?.orders || []).map(o => o.id).join(',') || '—'})`);
    }
  }

  if (!resp.ok) {
    const txt = await resp.text();
    throw new Error(`GOOD ML busca pedido ${numeroPedidoLoja} erro ${resp.status}: ${txt.slice(0, 200)}`);
  }
  const data = await resp.json();
  const shipmentId = data?.shipping?.id;
  if (!shipmentId) throw new Error(`GOOD ML: pedido ${numeroPedidoLoja} sem shipment_id`);
  return shipmentId;
}

// Verifica status + substatus do shipment
async function getShipmentSubstatus(token, shipmentId) {
  ...
  if (!resp.ok) {
    const txt = await resp.text();
    throw new Error(`GOOD ML shipment ${shipmentId} erro ${resp.status}: ${txt.slice(0, 200)}`);
  }
  const data = await resp.json();
  return {
    status:    data?.status,
...
async function baixarXmlNFe(xmlUrl) {
  ...
  if (!resp.ok) throw new Error(`GOOD: Erro ao baixar XML da NF: ${resp.status}`);
...
async function enviarNFeParaML(token, numeroPedidoLoja, nfDetalhe) {
  ...
  console.log(`[GOOD mlApi] Pedido ML ${numeroPedidoLoja} → shipment_id=${shipmentId}`);
  ...
  if (!nfDetalhe.xml) throw new Error(`GOOD NF ${nfDetalhe.id} sem URL de XML`);
  console.log(`[GOOD mlApi] Baixando XML da NF...`);
  const xmlContent = await baixarXmlNFe(nfDetalhe.xml);

  // 4. Enviar XML para o ML
  console.log(`[GOOD mlApi] Enviando XML para shipment ${shipmentId}...`);
  const resp = await fetch(
  ...
  if (!resp.ok) {
    const txt = await resp.text();
    throw new Error(`GOOD ML envio NF shipment=${shipmentId} erro ${resp.status}: ${txt.slice(0, 300)}`);
  }

  const result = await resp.json().catch(() => ({}));
  console.log(`[GOOD mlApi] ✅ NF enviada para shipment ${shipmentId}:`, JSON.stringify(result));
  return result;
}
