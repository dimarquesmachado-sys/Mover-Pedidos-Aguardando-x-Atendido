...
async function _fluxoNFeML(tokenBling) {
  ...
  for (const nfResumo of batch) {
    ...
    try {
      ...
      if (msg.includes('não é invoice_pending') || msg.includes('invoice_pending')) {
        console.log(`[AMB F3-NFeML] NF ${nf.numero} (pedido ML ${numeroPedidoLoja}) — ML não está pendente, nada a fazer`);
        semPendencia++;
        marcarProcessado('F3', nfeId);
      } else {
  ...
  console.log(
    `[AMB F3-NFeML] enviadas=${enviadas} | já-ok=${semPendencia} | ignoradas=${ignoradas} ` +
