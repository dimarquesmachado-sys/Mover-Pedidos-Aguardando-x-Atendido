...
const MAX_NFE = parseInt(process.env.AMB_MAX_NFE_ML || '60');
...
const NF_JANELA_DIAS = parseInt(process.env.AMB_NF_JANELA_DIAS || '5');
...
const MAX_CHECAGENS_PENDENCIA = parseInt(process.env.AMB_F3_MAX_CHECAGENS || '18');
...
async function _fluxoNFeML(tokenBling) {
  ...
  console.log(`[AMB F3-NFeML] janela ${NF_JANELA_DIAS}d (${inicial}→${final}) | ${lista.length} autorizadas | ${pendentes.length} pendentes | processando ${batch.length} (mais recentes)`);

  let mlToken = null;
  try {
    mlToken = await garantirTokenML();
  } catch (e) {
    console.warn('[AMB F3-NFeML] Sem token ML:', e.message, '— abortando rodada');
    return;
  }
  ...
  for (const nfResumo of batch) {
    ...
    try {
      ...
      console.error(`[AMB F3-NFeML] Erro ao buscar detalhe NF ${nfeId}:`, e.message);
      erros++;
      continue;
    }
    ...
    if (!nf.xml) {
      console.warn(`[AMB F3-NFeML] NF ${nfeId} (nº ${nf.numero}) sem XML — pulando`);
      ignoradas++;
      continue;
    }
    ...
    if (!numeroPedidoLoja) {
      console.warn(`[AMB F3-NFeML] NF ${nfeId} (nº ${nf.numero}) sem numeroPedidoLoja — pulando`);
      ignoradas++;
      continue;
    }
    ...
    try {
      ...
      console.log(`[AMB F3-NFeML] ✅ NF ${nf.numero} (pedido ML ${numeroPedidoLoja}) enviada`);
      enviadas++;
      ...
      if (msg.includes('não é invoice_pending') || msg.includes('invoice_pending')) {
        ...
        if (ESTADOS_FINAIS.includes(String(e.mlStatus || ''))) {
          ...
          console.log(`[AMB F3-NFeML] NF ${nf.numero} (pedido ML ${numeroPedidoLoja}) — envio já em "${e.mlStatus}", não pede mais NF — marcando como ok`);
          marcarProcessado('F3', nfeId);
          _semPendenciaCount.delete(nfeId);
        } else {
          ...
          if (tent >= MAX_CHECAGENS_PENDENCIA) {
            console.log(`[AMB F3-NFeML] NF ${nf.numero} (pedido ML ${numeroPedidoLoja}) — sem pendência após ${tent} checagens, marcando como ok`);
            marcarProcessado('F3', nfeId);
            _semPendenciaCount.delete(nfeId);
          } else {
            console.log(`[AMB F3-NFeML] NF ${nf.numero} (pedido ML ${numeroPedidoLoja}) — ML não está pendente (checagem ${tent}/${MAX_CHECAGENS_PENDENCIA}), re-checa no próximo ciclo`);
          }
        }
        semPendencia++;
      } else {
        ...
        if (_perm.permanente) {
          ...
          marcarProcessado('F3', nfeId);
          console.error(`[AMB F3-NFeML] NF ${nf.numero} (pedido ML ${numeroPedidoLoja}) TRAVADA — ${_perm.registro.motivo}. Parei de tentar; resolva no Bling e ela sai da lista.`);
        } else {
          console.error(`[AMB F3-NFeML] Erro ao enviar NF ${nf.numero} (pedido ML ${numeroPedidoLoja}):`, msg);
        }
        erros++;
      }
    }
  ...
  console.log(
    `[AMB F3-NFeML] enviadas=${enviadas} | sem-pendência=${semPendencia} | ignoradas=${ignoradas} ` +
...
async function rotinaNFeML() {
  if (_rodando) {
    console.log('[AMB F3-NFeML] já em execução — pulando');
    return;
  }
  _rodando = true;
  try {
