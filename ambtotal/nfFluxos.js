...
async function corrigirNFsPendentes() {
  if (_rodandoNF) {
    console.log('[AMB corrigirNFs] Já em execução — pulando');
    return;
  }
  _rodandoNF = true;
  ...
  try {
    ...
    console.log(`[AMB corrigirNFs] ${nfs.length} NFs para verificar`);
    ...
    for (const nf of nfs) {
      try {
        ...
        if (minutos < 5) {
          console.log(`[AMB corrigirNFs] NF ${nf.id} tem ${minutos.toFixed(1)} min — aguardando`);
          ignoradas++;
          continue;
        }
        ...
        console.log(`[AMB corrigirNFs] NF ${nf.id} | sit=${detalhe.situacao} | PJ=${isPJ} | IE="${ie}" | CEP=${cep} | UF=${uf}`);
        ...
        if (endereco.length < 2) {
          ...
          console.log(`[AMB corrigirNFs] NF ${nf.id} | endereço curto "${endereco}" -> "${novoEndereco}"`);
        ...
        if (cep) {
          ...
          if (resCidade) {
            ...
            console.log(`[AMB corrigirNFs] NF ${nf.id} | "${cidadeAtual}|${ufAtual}" -> "${resCidade.municipio}|${resCidade.uf}"`);
        ...
        if (isPJ && !ie && uf && idContato) {
          ...
          if (resultado) {
            console.log(`[AMB corrigirNFs] NF ${nf.id} | IE: "${resultado.ie}" contribuinte=${resultado.contribuinte}`);
            ...
            console.log(`[AMB corrigirNFs] NF ${nf.id} | IE não encontrada — intervenção manual`);
          }
        }
        ...
        if (e.code === 401 || e.message === 'TOKEN_EXPIRADO') token = await renovarTokenNF();
        console.error(`[AMB corrigirNFs] Erro na NF ${nf.id}:`, e.message);
        erros++;
      }
    ...
    console.log(`[AMB corrigirNFs] corrigidas=${corrigidas} | ignoradas=${ignoradas} | erros=${erros}`);
