...
function traduzirErroBling(msg) {
  ...
  if (texto.includes('invalid refresh token')) return 'Token inválido. Reautorize em /estoque-girassol/auth/bling.';
...
async function resolverProduto(tipo, valor) {
  ...
  if (tipoBusca === 'SKU') {
    ...
    if (id) {
      ...
      if (p && getSkus(p).some(s => isExactCI(s, valorOriginal))) {
        console.log(`[estoque-girassol SKU-HIT] ${valorOriginal} → ${p.codigo}`);
  ...
  if (idPorEan) {
    ...
    if (p && getEans(p).some(e => isExactDigits(e, valorOriginal))) {
      console.log(`[estoque-girassol EAN-HIT] ${valorOriginal} → ${p.codigo}`);
      return { ok: true, produto: p };
    }
  }
  ...
  for (const url of urlsEan) {
    ...
    for (const item of (data?.data || [])) {
      ...
      const p = await buscarDetalhe(item.id);
      if (p && getEans(p).some(e => isExactDigits(e, valorOriginal))) {
        console.log(`[estoque-girassol EAN-API] ${valorOriginal} → ${p.codigo}`);
        return { ok: true, produto: p };
      }
    }
  }
  ...
  if (eansCarregados) {
    console.log(`[estoque-girassol EAN] EAN ${valorOriginal} não existe no Bling.`);
    return { ok: false, erro: 'Produto não encontrado' };
  }
