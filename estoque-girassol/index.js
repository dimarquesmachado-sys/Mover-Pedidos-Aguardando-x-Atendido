...
async function handleBuscar(req, res, urlObj) {
  ...
  try {
    ...
    console.error('[estoque-girassol /buscar] ERRO:', e.message);
...
async function handleSalvar(req, res, body, urlObj) {
  ...
  try {
    ...
    console.error('[estoque-girassol /salvar] ERRO:', e.message);
    return json(res, 200, { ok: false, erro: blingProdutos.traduzirErroBling(e.message) });
  }
}
...
async function handleLogin(req, res, body) {
  ...
  console.log(`[estoque-girassol LOGIN] ${r.usuario} (${r.perfil})`);
...
function routes(readBody) {
  return async function handle(req, res, urlObj) {
    ...
    // ─ Frontend estático ─
    if (method === 'GET' && p === '/estoque-girassol') {
      res.writeHead(302, { Location: '/estoque-girassol/celular' });
      res.end();
      return true;
    }
    if (method === 'GET' && p === '/estoque-girassol/celular') {
      servirArquivo(res, 'celular.html');
      return true;
    }
    if (method === 'GET' && p.startsWith('/estoque-girassol/static/')) {
      const rel = p.replace('/estoque-girassol/static/', '');
    ...
    if (method === 'GET' && p === '/estoque-girassol/health') {
      ...
      return json(res, 200, {
        ...
        blingConfigurado: !!process.env.ESTOQUE_GIRASSOL_BLING_CLIENT_ID && !!process.env.ESTOQUE_GIRASSOL_BLING_CLIENT_SECRET,
    ...
    // ─ LOGIN (compat + api) ─
    if (method === 'POST' && (p === '/estoque-girassol/login' || p === '/estoque-girassol/api/login')) {
    ...
    // ─ LOGOUT ─
    if (method === 'POST' && p === '/estoque-girassol/api/logout') {
    ...
    // ─ BUSCAR (compat + api) ─
    if (method === 'GET' && (p === '/estoque-girassol/buscar' || p === '/estoque-girassol/api/buscar')) {
    ...
    // ─ OAuth Bling ─
    if (method === 'GET' && p === '/estoque-girassol/auth/bling') {
      const cid = process.env.ESTOQUE_GIRASSOL_BLING_CLIENT_ID;
      if (!cid) { json(res, 500, { erro: 'ESTOQUE_GIRASSOL_BLING_CLIENT_ID não configurado' }); return true; }
      ...
      try {
        ...
        html(res, 200, `
          <html><body style="font-family:Arial;padding:40px;text-align:center;">
            ...
            <p><a href="/estoque-girassol/celular">Abrir PWA mobile</a></p>
          </body></html>
        `);
      } catch (e) {
        console.error('[estoque-girassol OAUTH]', e);
        html(res, 500, `<h2>❌ Erro OAuth: ${e.message}</h2>`);
      }
      return true;
    }

    return false; // /estoque-girassol/* desconhecido — deixa cair pro 404 global
  };
}
